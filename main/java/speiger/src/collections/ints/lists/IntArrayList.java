 ...
 * 
 * <p>This implementation is optimized to improve how data is processed with interfaces like {@link IIntArray}, {@link IntStack}
...
public class IntArrayList extends AbstractIntList implements IIntArray, IntStack
{
	...
	public IntArrayList(Collection<? extends Integer> c) {
		this(c.size());
		size = IntIterators.unwrap(data, c.iterator());
	}
	 ...
	 */
	public IntArrayList(int[] a, int length) {
		this(a, 0, length);
	}
	...
	public boolean addAll(int index, Collection<? extends Integer> c) {
		...
		while(add-- != 0) data[index++] = iter.next().intValue();
		return true;
	}
	...
	public boolean addAll(int index, IntCollection c) {
		...
		while(add-- != 0) data[index++] = iter.nextInt();
		return true;
	}
	...
	public void sort(IntComparator c) {
		if(c != null) IntArrays.stableSort(data, size, c);
		else IntArrays.stableSort(data, size);
	}
	...
	public void unstableSort(IntComparator c) {
		if(c != null) IntArrays.unstableSort(data, size, c);
		else IntArrays.unstableSort(data, size);		
	}
	...
	public boolean removeAll(Collection<?> c) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!c.contains(Integer.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	 ...
	 * This function might delegate to a more appropriate function if necessary
	 * @param c the elements that should be kept. If empty, IntArrayList#clear is called.
	 * @return true if the collection was modified
	 * @throws NullPointerException if the collection is null
	 */
	@Override
	...
	public boolean removeIf(Predicate<? super Integer> filter) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!filter.test(Integer.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	...
	public boolean trim(int size) {
		...
		data = value == 0 ? IntArrays.EMPTY_ARRAY : Arrays.copyOf(data, value);
		return true;
	}
	...
	public void clearAndTrim(int size) {
		...
		data = size == 0 ? IntArrays.EMPTY_ARRAY : new int[size];
		this.size = size;
	}
	...
	protected void grow(int capacity) {
		...
		data = Arrays.copyOf(data, data == IntArrays.EMPTY_ARRAY ? Math.max(DEFAULT_ARRAY_SIZE, capacity) : (int)Math.max(Math.min((long)data.length + (data.length >> 1), SanityChecks.MAX_ARRAY_SIZE), capacity));		
	}
	 ...
	 */
	@Override
	public IntStream primitiveStream() { return StreamSupport.intStream(IntSplititerators.createArrayJavaSplititerator(data, size, 16464), false); }
	 ...
	 */
	@Override
	public IntSplititerator spliterator() { return IntSplititerators.createArraySplititerator(data, size, 16464); }
}