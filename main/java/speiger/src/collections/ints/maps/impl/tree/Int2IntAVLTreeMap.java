...
public class Int2IntAVLTreeMap extends AbstractInt2IntMap implements Int2IntNavigableMap
{
	 ...
	 */
	public Int2IntAVLTreeMap(Integer[] keys, Integer[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2IntAVLTreeMap(Integer[] keys, Integer[] values, IntComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2IntAVLTreeMap(int[] keys, int[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2IntAVLTreeMap(int[] keys, int[] values, IntComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Int2IntAVLTreeMap(Map<? extends Integer, ? extends Integer> map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Int2IntAVLTreeMap(Int2IntMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Int2IntAVLTreeMap(Int2IntMap map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Int2IntAVLTreeMap copy() {
		Int2IntAVLTreeMap set = new Int2IntAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractIntSet implements IntNavigableSet
	{
		...
		public IntBidirectionalIterator iterator(int fromElement) {
			if(map instanceof Int2IntAVLTreeMap) return ((Int2IntAVLTreeMap)map).keyIterator(fromElement);
		...
		public IntBidirectionalIterator iterator() {
			if(map instanceof Int2IntAVLTreeMap) return ((Int2IntAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public IntBidirectionalIterator descendingIterator() {
			if(map instanceof Int2IntAVLTreeMap) return ((Int2IntAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Int2IntAVLTreeMap) return ((Int2IntAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Int2IntAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Int2IntAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Int2IntAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Int2IntAVLTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Int2IntAVLTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractInt2IntMap implements Int2IntNavigableMap
	{
		...
		NavigableSubMap(Int2IntAVLTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractIntCollection
	{
		...
		@Override
		public int size() {
			return Int2IntAVLTreeMap.this.size;
		}
