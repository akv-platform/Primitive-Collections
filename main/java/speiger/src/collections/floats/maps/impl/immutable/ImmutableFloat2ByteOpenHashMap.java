 ...
 */
public class ImmutableFloat2ByteOpenHashMap extends AbstractFloat2ByteMap implements Float2ByteOrderedMap
{
	 ...
	 */
	protected ImmutableFloat2ByteOpenHashMap() {}
	 ...
	 */
	public ImmutableFloat2ByteOpenHashMap(Float[] keys, Byte[] values) {
	 ...
	 */
	public ImmutableFloat2ByteOpenHashMap(Float[] keys, Byte[] values, float loadFactor) {
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		init(FloatArrays.unwrap(keys), ByteArrays.unwrap(values), 0, keys.length, loadFactor);
	}
	 ...
	 */
	public ImmutableFloat2ByteOpenHashMap(Map<? extends Float, ? extends Byte> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public ImmutableFloat2ByteOpenHashMap(Map<? extends Float, ? extends Byte> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public ImmutableFloat2ByteOpenHashMap(Float2ByteMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public ImmutableFloat2ByteOpenHashMap(Float2ByteMap map, float loadFactor) {
		float[] keys = new float[map.size()];
		byte[] values = new byte[keys.length];
		int index = 0;
		for(Float2ByteMap.Entry entry : Float2ByteMaps.fastIterable(map)) {
	...
	protected void init(float[] a, byte[] b, int offset, int length, float loadFactor) {
	...
	@Override
	public ImmutableFloat2ByteOpenHashMap copy() {
		ImmutableFloat2ByteOpenHashMap map = new ImmutableFloat2ByteOpenHashMap();
		map.mask = mask;
	...
	@Override
	public boolean replace(float key, byte oldValue, byte newValue) { throw new UnsupportedOperationException(); }
	
	@Override
	public byte replace(float key, byte value) { throw new UnsupportedOperationException(); }
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2ByteMap.Entry) {
					...
					int index = ImmutableFloat2ByteOpenHashMap.this.findIndex(entry.getFloatKey());
					if(index >= 0) return entry.getByteValue() == ImmutableFloat2ByteOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = ImmutableFloat2ByteOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Byte.valueOf(ImmutableFloat2ByteOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
