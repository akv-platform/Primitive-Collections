...
import speiger.src.collections.floats.utils.IFloatArray;
 ...
 * 
 * <p>This implementation is optimized to improve how data is processed with interfaces like {@link IFloatArray}, {@link FloatStack}
...
public class FloatArrayList extends AbstractFloatList implements IFloatArray, FloatStack
{
	...
	public FloatArrayList(Collection<? extends Float> c) {
		this(c.size());
		size = FloatIterators.unwrap(data, c.iterator());
	}
	 ...
	 */
	public FloatArrayList(float[] a, int length) {
		this(a, 0, length);
	}
	...
	public boolean addAll(int index, Collection<? extends Float> c) {
		...
		while(add-- != 0) data[index++] = iter.next().floatValue();
		return true;
	}
	...
	public boolean addAll(int index, FloatCollection c) {
		...
		while(add-- != 0) data[index++] = iter.nextFloat();
		return true;
	}
	...
	public void sort(FloatComparator c) {
		if(c != null) FloatArrays.stableSort(data, size, c);
		else FloatArrays.stableSort(data, size);
	}
	...
	public void unstableSort(FloatComparator c) {
		if(c != null) FloatArrays.unstableSort(data, size, c);
		else FloatArrays.unstableSort(data, size);		
	}
	...
	public void replaceFloats(DoubleUnaryOperator o) {
		Objects.requireNonNull(o);
		for(int i = 0;i<size;i++)
			data[i] = SanityChecks.castToFloat(o.applyAsDouble(data[i]));
	}
	...
	public boolean removeAll(Collection<?> c) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!c.contains(Float.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	 ...
	 * This function might delegate to a more appropriate function if necessary
	 * @param c the elements that should be kept. If empty, FloatArrayList#clear is called.
	 * @return true if the collection was modified
	 * @throws NullPointerException if the collection is null
	 */
	@Override
	...
	public boolean removeIf(Predicate<? super Float> filter) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!filter.test(Float.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	...
	public boolean trim(int size) {
		...
		data = value == 0 ? FloatArrays.EMPTY_ARRAY : Arrays.copyOf(data, value);
		return true;
	}
	...
	public void clearAndTrim(int size) {
		...
		data = size == 0 ? FloatArrays.EMPTY_ARRAY : new float[size];
		this.size = size;
	}
	...
	protected void grow(int capacity) {
		...
		data = Arrays.copyOf(data, data == FloatArrays.EMPTY_ARRAY ? Math.max(DEFAULT_ARRAY_SIZE, capacity) : (int)Math.max(Math.min((long)data.length + (data.length >> 1), SanityChecks.MAX_ARRAY_SIZE), capacity));		
	}
	 ...
	 */
	@Override
	public DoubleStream primitiveStream() { return StreamSupport.doubleStream(FloatSplititerators.createArrayJavaSplititerator(data, size, 16464), false); }
	 ...
	 */
	@Override
	public FloatSplititerator spliterator() { return FloatSplititerators.createArraySplititerator(data, size, 16464); }
}