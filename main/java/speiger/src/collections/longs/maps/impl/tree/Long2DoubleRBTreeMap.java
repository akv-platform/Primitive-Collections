 ...
 */
public class Long2DoubleRBTreeMap extends AbstractLong2DoubleMap implements Long2DoubleNavigableMap
{
	 ...
	 */
	public Long2DoubleRBTreeMap(LongComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Long2DoubleRBTreeMap(Long[] keys, Double[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2DoubleRBTreeMap(Long[] keys, Double[] values, LongComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Long2DoubleRBTreeMap(Map<? extends Long, ? extends Double> map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2DoubleRBTreeMap(Long2DoubleMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Long2DoubleRBTreeMap(Long2DoubleMap map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Long2DoubleRBTreeMap copy() {
		Long2DoubleRBTreeMap set = new Long2DoubleRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractLongSet implements LongNavigableSet
	{
		...
		public LongBidirectionalIterator iterator(long fromElement) {
			if(map instanceof Long2DoubleRBTreeMap) return ((Long2DoubleRBTreeMap)map).keyIterator(fromElement);
		...
		public LongBidirectionalIterator iterator() {
			if(map instanceof Long2DoubleRBTreeMap) return ((Long2DoubleRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public LongBidirectionalIterator descendingIterator() {
			if(map instanceof Long2DoubleRBTreeMap) return ((Long2DoubleRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Long2DoubleRBTreeMap) return ((Long2DoubleRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Long2DoubleRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Long2DoubleRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Long2DoubleRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Long2DoubleRBTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Long2DoubleRBTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Long2DoubleRBTreeMap map;
		final long lo, hi;
		...
		NavigableSubMap(Long2DoubleRBTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractDoubleCollection
	{
		...
		@Override
		public int size() {
			return Long2DoubleRBTreeMap.this.size;
		}
