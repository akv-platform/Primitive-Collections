...
import speiger.src.collections.longs.maps.interfaces.Long2ObjectConcurrentMap;
 ...
 */
public class Long2ObjectConcurrentOpenHashMap<V> extends AbstractLong2ObjectMap<V> implements Long2ObjectConcurrentMap<V>, ITrimmable
{
	/** Segment Limit */
	...
	@Override
	public Long2ObjectConcurrentOpenHashMap<V> copy() {
		Long2ObjectConcurrentOpenHashMap<V> copy = new Long2ObjectConcurrentOpenHashMap<>(false);
	...
	private class MapEntrySet extends AbstractObjectSet<Long2ObjectMap.Entry<V>> implements Long2ObjectMap.FastEntrySet<V> {
		@Override
	...
	private class FastEntryIterator extends MapIterator implements ObjectBidirectionalIterator<Long2ObjectMap.Entry<V>> {
	...
	private class EntryIterator extends MapIterator implements ObjectBidirectionalIterator<Long2ObjectMap.Entry<V>> {
		MapEntry entry;
	...
	protected class MapEntry implements Long2ObjectMap.Entry<V>, Map.Entry<Long, V> {
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				if(obj instanceof Long2ObjectMap.Entry) {
					...
					return getLongKey() == entry.getLongKey() && Objects.equals(getValue(), entry.getValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)obj;
				Object key = entry.getKey();
				Object value = entry.getValue();
				return key instanceof Long && getLongKey() == ((Long)key).longValue() && Objects.equals(getValue(), value);
			}
			return false;
		}
		...
		@Override
		public int hashCode() {
			return Long.hashCode(getLongKey()) ^ Objects.hashCode(getValue());
		}
		
		@Override
		public String toString() {
			return Long.toString(getLongKey()) + "=" + Objects.toString(getValue());
		}
	}
	...
	protected static class Segment<V> extends StampedLock
	{
		...
		protected void forEach(LongObjectConsumer<V> action) {
			long stamp = readLock();
			try {
				int index = firstIndex;
				while(index != -1) {
		...
		protected boolean remove(int hash, Object key, Object value) {
			...
			try
			{
				...
				long current = keys[pos];
				if(current == 0) return false;
				if(Objects.equals(key, Long.valueOf(current)) && Objects.equals(value, values[pos])) {
					removeIndex(pos);
					return true;
				}
				while(true) {
					if((current = keys[pos = (++pos & mask)]) == 0) return false;
					else if(Objects.equals(key, Long.valueOf(current)) && Objects.equals(value, values[pos])){
						removeIndex(pos);
						return true;
					}
				}				
		...
		protected V compute(int hash, long key, LongObjectUnaryOperator<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfAbsent(int hash, long key, Long2ObjectFunction<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V supplyIfAbsent(int hash, long key, ObjectSupplier<V> valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfPresent(int hash, long key, LongObjectUnaryOperator<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
		...
		protected V merge(int hash, long key, V value, ObjectObjectUnaryOperator<V, V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
