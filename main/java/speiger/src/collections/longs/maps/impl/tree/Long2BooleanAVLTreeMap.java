 ...
 */
public class Long2BooleanAVLTreeMap extends AbstractLong2BooleanMap implements Long2BooleanNavigableMap
{
	...
	/** EntrySet Cache */
	protected ObjectSet<Long2BooleanMap.Entry> entrySet;
	 ...
	 */
	public Long2BooleanAVLTreeMap(LongComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Long2BooleanAVLTreeMap(Long[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2BooleanAVLTreeMap(Long[] keys, Boolean[] values, LongComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Long2BooleanAVLTreeMap(Map<? extends Long, ? extends Boolean> map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2BooleanAVLTreeMap(Long2BooleanMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Long2BooleanAVLTreeMap(Long2BooleanMap map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	public void mergeAllBoolean(Long2BooleanMap m, BooleanBooleanUnaryOperator mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		for(Long2BooleanMap.Entry entry : Long2BooleanMaps.fastIterable(m)) {
	...
	@Override
	public Long2BooleanAVLTreeMap copy() {
		Long2BooleanAVLTreeMap set = new Long2BooleanAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractLongSet implements LongNavigableSet
	{
		...
		public KeySet(Long2BooleanNavigableMap map) {
			this.map = map;
		}
		...
		public Long lower(Long e) {
			Long2BooleanMap.Entry node = map.lowerEntry(e.longValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Long floor(Long e) {
			Long2BooleanMap.Entry node = map.floorEntry(e.longValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Long higher(Long e) {
			Long2BooleanMap.Entry node = map.higherEntry(e.longValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Long ceiling(Long e) {
			Long2BooleanMap.Entry node = map.ceilingEntry(e.longValue());
			return node != null ? node.getKey() : null;
		}
		...
		public LongBidirectionalIterator iterator(long fromElement) {
			if(map instanceof Long2BooleanAVLTreeMap) return ((Long2BooleanAVLTreeMap)map).keyIterator(fromElement);
		...
		public LongBidirectionalIterator iterator() {
			if(map instanceof Long2BooleanAVLTreeMap) return ((Long2BooleanAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public LongBidirectionalIterator descendingIterator() {
			if(map instanceof Long2BooleanAVLTreeMap) return ((Long2BooleanAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Long2BooleanAVLTreeMap) return ((Long2BooleanAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Long2BooleanAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Long2BooleanAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Long2BooleanAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Long2BooleanAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Long2BooleanAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractLong2BooleanMap implements Long2BooleanNavigableMap
	{
		final Long2BooleanAVLTreeMap map;
		final long lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;
		
		Long2BooleanNavigableMap inverse;
		LongNavigableSet keySet;
		ObjectSet<Long2BooleanMap.Entry> entrySet;
		BooleanCollection values;
		
		NavigableSubMap(Long2BooleanAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public Long2BooleanMap.Entry lowerEntry(long key) { return subLower(key); }
		@Override
		public Long2BooleanMap.Entry floorEntry(long key) { return subFloor(key); }
		@Override
		public Long2BooleanMap.Entry ceilingEntry(long key) { return subCeiling(key); }
		@Override
		public Long2BooleanMap.Entry higherEntry(long key) { return subHigher(key); }
		...
		public Long2BooleanMap.Entry pollFirstEntry() {
			Node entry = subLowest();
			if(entry != null) {
				Long2BooleanMap.Entry result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Long2BooleanMap.Entry> {
			@Override
			public int size() {
				if (fromStart && toEnd) return map.size();
				int size = 0;
				for(ObjectIterator<Long2BooleanMap.Entry> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
		...
		class DecsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Long2BooleanMap.Entry>
		{
		...
		class AcsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Long2BooleanMap.Entry>
		{
	...
	class Values extends AbstractBooleanCollection
	{
		...
		@Override
		public int size() {
			return Long2BooleanAVLTreeMap.this.size;
		}
	...
	class EntrySet extends AbstractObjectSet<Long2BooleanMap.Entry> {
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Long2BooleanMap.Entry>
	{
	...
	private static final class Node implements Long2BooleanMap.Entry
	{
		long key;
		boolean value;
		int state;
		...
		Node(long key, boolean value, Node parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherKey instanceof Long && otherValue instanceof Boolean && key == ((Long)otherKey).longValue() && value == ((Boolean)otherValue).booleanValue();
			}
			return false;
		}
