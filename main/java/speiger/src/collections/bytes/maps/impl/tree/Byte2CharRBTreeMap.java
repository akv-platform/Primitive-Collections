 ...
 */
public class Byte2CharRBTreeMap extends AbstractByte2CharMap implements Byte2CharNavigableMap
{
	...
	/** EntrySet Cache */
	protected ObjectSet<Byte2CharMap.Entry> entrySet;
	 ...
	 */
	public Byte2CharRBTreeMap(ByteComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Byte2CharRBTreeMap(Byte[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2CharRBTreeMap(Byte[] keys, Character[] values, ByteComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2CharRBTreeMap(byte[] keys, char[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2CharRBTreeMap(byte[] keys, char[] values, ByteComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Byte2CharRBTreeMap(Map<? extends Byte, ? extends Character> map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Byte2CharRBTreeMap(Byte2CharMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Byte2CharRBTreeMap(Byte2CharMap map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	public void mergeAllChar(Byte2CharMap m, CharCharUnaryOperator mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		for(Byte2CharMap.Entry entry : Byte2CharMaps.fastIterable(m)) {
	...
	@Override
	public Byte2CharRBTreeMap copy() {
		Byte2CharRBTreeMap set = new Byte2CharRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	@Override
	public Byte2CharNavigableMap headMap(byte toKey, boolean inclusive) {
		return new AscendingNaivgableSubMap(this, true, (byte)0, true, false, toKey, inclusive);
	}
	
	@Override
	public Byte2CharNavigableMap tailMap(byte fromKey, boolean inclusive) {
	...
	@Override
	public Byte2CharMap.Entry lowerEntry(byte key) {
		Node node = findLowerNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Byte2CharMap.Entry higherEntry(byte key) {
		Node node = findHigherNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Byte2CharMap.Entry floorEntry(byte key) {
		Node node = findFloorNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Byte2CharMap.Entry ceilingEntry(byte key) {
	...
	static class KeySet extends AbstractByteSet implements ByteNavigableSet
	{
		...
		public KeySet(Byte2CharNavigableMap map) {
			this.map = map;
		}
		...
		public Byte lower(Byte e) {
			Byte2CharMap.Entry node = map.lowerEntry(e.byteValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Byte floor(Byte e) {
			Byte2CharMap.Entry node = map.floorEntry(e.byteValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Byte higher(Byte e) {
			Byte2CharMap.Entry node = map.higherEntry(e.byteValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Byte ceiling(Byte e) {
			Byte2CharMap.Entry node = map.ceilingEntry(e.byteValue());
			return node != null ? node.getKey() : null;
		}
		...
		public ByteBidirectionalIterator iterator(byte fromElement) {
			if(map instanceof Byte2CharRBTreeMap) return ((Byte2CharRBTreeMap)map).keyIterator(fromElement);
		...
		public ByteBidirectionalIterator iterator() {
			if(map instanceof Byte2CharRBTreeMap) return ((Byte2CharRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ByteBidirectionalIterator descendingIterator() {
			if(map instanceof Byte2CharRBTreeMap) return ((Byte2CharRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Byte2CharRBTreeMap) return ((Byte2CharRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Byte2CharRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Byte2CharRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Byte2CharRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Byte2CharRBTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Byte2CharRBTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractByte2CharMap implements Byte2CharNavigableMap
	{
		final Byte2CharRBTreeMap map;
		final byte lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;
		
		Byte2CharNavigableMap inverse;
		ByteNavigableSet keySet;
		ObjectSet<Byte2CharMap.Entry> entrySet;
		CharCollection values;
		
		NavigableSubMap(Byte2CharRBTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public Byte2CharMap.Entry lowerEntry(byte key) { return subLower(key); }
		@Override
		public Byte2CharMap.Entry floorEntry(byte key) { return subFloor(key); }
		@Override
		public Byte2CharMap.Entry ceilingEntry(byte key) { return subCeiling(key); }
		@Override
		public Byte2CharMap.Entry higherEntry(byte key) { return subHigher(key); }
		...
		public Byte2CharMap.Entry pollFirstEntry() {
			Node entry = subLowest();
			if(entry != null) {
				Byte2CharMap.Entry result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Byte2CharMap.Entry> {
			@Override
			public int size() {
				if (fromStart && toEnd) return map.size();
				int size = 0;
				for(ObjectIterator<Byte2CharMap.Entry> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
		...
		class DecsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Byte2CharMap.Entry>
		{
		...
		class AcsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Byte2CharMap.Entry>
		{
	...
	class Values extends AbstractCharCollection
	{
		...
		@Override
		public int size() {
			return Byte2CharRBTreeMap.this.size;
		}
	...
	class EntrySet extends AbstractObjectSet<Byte2CharMap.Entry> {
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Byte2CharMap.Entry>
	{
	...
	private static final class Node implements Byte2CharMap.Entry
	{
		...
		Node(byte key, char value, Node parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherKey instanceof Byte && otherValue instanceof Character && key == ((Byte)otherKey).byteValue() && value == ((Character)otherValue).charValue();
			}
			return false;
		}
