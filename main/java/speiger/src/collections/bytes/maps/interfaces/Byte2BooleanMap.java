...
public interface Byte2BooleanMap extends Map<Byte, Boolean>, Byte2BooleanFunction
{
	...
	public default Boolean computeIfAbsent(Byte key, Function<? super Byte, ? extends Boolean> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Boolean.valueOf(computeBooleanIfAbsent(key.byteValue(), mappingFunction instanceof Byte2BooleanFunction ? (Byte2BooleanFunction)mappingFunction : K -> mappingFunction.apply(Byte.valueOf(K)).booleanValue()));
	}
