 ...
 */
public class Short2ByteRBTreeMap extends AbstractShort2ByteMap implements Short2ByteNavigableMap
{
	 ...
	 */
	public Short2ByteRBTreeMap(ShortComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Short2ByteRBTreeMap(Short[] keys, Byte[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Short2ByteRBTreeMap(Short[] keys, Byte[] values, ShortComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Short2ByteRBTreeMap(Map<? extends Short, ? extends Byte> map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2ByteRBTreeMap(Short2ByteMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Short2ByteRBTreeMap(Short2ByteMap map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Short2ByteRBTreeMap copy() {
		Short2ByteRBTreeMap set = new Short2ByteRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractShortSet implements ShortNavigableSet
	{
		...
		public ShortBidirectionalIterator iterator(short fromElement) {
			if(map instanceof Short2ByteRBTreeMap) return ((Short2ByteRBTreeMap)map).keyIterator(fromElement);
		...
		public ShortBidirectionalIterator iterator() {
			if(map instanceof Short2ByteRBTreeMap) return ((Short2ByteRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ShortBidirectionalIterator descendingIterator() {
			if(map instanceof Short2ByteRBTreeMap) return ((Short2ByteRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Short2ByteRBTreeMap) return ((Short2ByteRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Short2ByteRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Short2ByteRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Short2ByteRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Short2ByteRBTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Short2ByteRBTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractShort2ByteMap implements Short2ByteNavigableMap
	{
		...
		NavigableSubMap(Short2ByteRBTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractByteCollection
	{
		...
		@Override
		public int size() {
			return Short2ByteRBTreeMap.this.size;
		}
