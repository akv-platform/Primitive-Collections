...
public class Short2ObjectRBTreeMap<V> extends AbstractShort2ObjectMap<V> implements Short2ObjectNavigableMap<V>
{
	 ...
	 */
	public Short2ObjectRBTreeMap(ShortComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Short2ObjectRBTreeMap(Short[] keys, V[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Short2ObjectRBTreeMap(Short[] keys, V[] values, ShortComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Short2ObjectRBTreeMap(Map<? extends Short, ? extends V> map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2ObjectRBTreeMap(Short2ObjectMap<V> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Short2ObjectRBTreeMap(Short2ObjectMap<V> map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	public Short2ObjectRBTreeMap<V> copy() {
		Short2ObjectRBTreeMap<V> set = new Short2ObjectRBTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<V> extends AbstractShortSet implements ShortNavigableSet
	{
		...
		public ShortBidirectionalIterator iterator(short fromElement) {
			if(map instanceof Short2ObjectRBTreeMap) return ((Short2ObjectRBTreeMap<V>)map).keyIterator(fromElement);
		...
		public ShortBidirectionalIterator iterator() {
			if(map instanceof Short2ObjectRBTreeMap) return ((Short2ObjectRBTreeMap<V>)map).keyIterator();
			return ((NavigableSubMap<V>)map).keyIterator();
		}
		
		@Override
		public ShortBidirectionalIterator descendingIterator() {
			if(map instanceof Short2ObjectRBTreeMap) return ((Short2ObjectRBTreeMap<V>)map).descendingKeyIterator();
			return ((NavigableSubMap<V>)map).descendingKeyIterator();
		}
		
		protected Node<V> start() {
			if(map instanceof Short2ObjectRBTreeMap) return ((Short2ObjectRBTreeMap<V>)map).first;
			return ((NavigableSubMap<V>)map).subLowest();
		}
		
		protected Node<V> end() {
			if(map instanceof Short2ObjectRBTreeMap) return null;
			return ((NavigableSubMap<V>)map).subHighest();
		}
		
		protected Node<V> next(Node<V> entry) {
			if(map instanceof Short2ObjectRBTreeMap) return entry.next();
			return ((NavigableSubMap<V>)map).next(entry);
		}
		
		protected Node<V> previous(Node<V> entry) {
			if(map instanceof Short2ObjectRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Short2ObjectRBTreeMap<V> map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<V> extends NavigableSubMap<V>
	{
		...
		DescendingNaivgableSubMap(Short2ObjectRBTreeMap<V> map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap<V> extends AbstractShort2ObjectMap<V> implements Short2ObjectNavigableMap<V>
	{
		...
		NavigableSubMap(Short2ObjectRBTreeMap<V> map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
