 ...
 */
public class Short2BooleanAVLTreeMap extends AbstractShort2BooleanMap implements Short2BooleanNavigableMap
{
	 ...
	 */
	public Short2BooleanAVLTreeMap(ShortComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Short2BooleanAVLTreeMap(Short[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Short2BooleanAVLTreeMap(Short[] keys, Boolean[] values, ShortComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Short2BooleanAVLTreeMap(Map<? extends Short, ? extends Boolean> map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2BooleanAVLTreeMap(Short2BooleanMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Short2BooleanAVLTreeMap(Short2BooleanMap map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Short2BooleanAVLTreeMap copy() {
		Short2BooleanAVLTreeMap set = new Short2BooleanAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractShortSet implements ShortNavigableSet
	{
		...
		public ShortBidirectionalIterator iterator(short fromElement) {
			if(map instanceof Short2BooleanAVLTreeMap) return ((Short2BooleanAVLTreeMap)map).keyIterator(fromElement);
		...
		public ShortBidirectionalIterator iterator() {
			if(map instanceof Short2BooleanAVLTreeMap) return ((Short2BooleanAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ShortBidirectionalIterator descendingIterator() {
			if(map instanceof Short2BooleanAVLTreeMap) return ((Short2BooleanAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Short2BooleanAVLTreeMap) return ((Short2BooleanAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Short2BooleanAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Short2BooleanAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Short2BooleanAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Short2BooleanAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Short2BooleanAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Short2BooleanAVLTreeMap map;
		final short lo, hi;
		...
		NavigableSubMap(Short2BooleanAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractBooleanCollection
	{
		...
		@Override
		public int size() {
			return Short2BooleanAVLTreeMap.this.size;
		}
