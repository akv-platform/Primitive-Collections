 ...
 */
public class ImmutableDouble2CharOpenHashMap extends AbstractDouble2CharMap implements Double2CharOrderedMap
{
	 ...
	 */
	protected ImmutableDouble2CharOpenHashMap() {}
	 ...
	 */
	public ImmutableDouble2CharOpenHashMap(Double[] keys, Character[] values) {
	 ...
	 */
	public ImmutableDouble2CharOpenHashMap(Double[] keys, Character[] values, float loadFactor) {
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		init(DoubleArrays.unwrap(keys), CharArrays.unwrap(values), 0, keys.length, loadFactor);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public ImmutableDouble2CharOpenHashMap(double[] keys, char[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public ImmutableDouble2CharOpenHashMap(double[] keys, char[] values, float loadFactor) {
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 */
	public ImmutableDouble2CharOpenHashMap(Map<? extends Double, ? extends Character> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public ImmutableDouble2CharOpenHashMap(Map<? extends Double, ? extends Character> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public ImmutableDouble2CharOpenHashMap(Double2CharMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public ImmutableDouble2CharOpenHashMap(Double2CharMap map, float loadFactor) {
		double[] keys = new double[map.size()];
		char[] values = new char[keys.length];
		int index = 0;
		for(Double2CharMap.Entry entry : Double2CharMaps.fastIterable(map)) {
	...
	protected void init(double[] a, char[] b, int offset, int length, float loadFactor) {
	...
	@Override
	public ImmutableDouble2CharOpenHashMap copy() {
		ImmutableDouble2CharOpenHashMap map = new ImmutableDouble2CharOpenHashMap();
		map.mask = mask;
	...
	@Override
	public boolean replace(double key, char oldValue, char newValue) { throw new UnsupportedOperationException(); }
	
	@Override
	public char replace(double key, char value) { throw new UnsupportedOperationException(); }
	...
	private class MapEntrySet extends AbstractObjectSet<Double2CharMap.Entry> implements Double2CharOrderedMap.FastOrderedSet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Double2CharMap.Entry) {
					...
					int index = ImmutableDouble2CharOpenHashMap.this.findIndex(entry.getDoubleKey());
					if(index >= 0) return entry.getCharValue() == ImmutableDouble2CharOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = ImmutableDouble2CharOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Character.valueOf(ImmutableDouble2CharOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		@Override
		public int size() {
			return ImmutableDouble2CharOpenHashMap.this.size();
		}
