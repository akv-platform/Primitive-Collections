...
public class CopyOnWriteDoubleArrayList extends AbstractDoubleList implements ITrimmable, DoubleStack
{
	...
	public double[] extractElements(int from, int to) {
		...
		try {
			...
			if(to == data.length) this.data = Arrays.copyOf(data, data.length - length);
				...
				int size = data.length;
				double[] newElements = new double[size-length];
	...
	private static class COWSubList extends AbstractDoubleList
	{
		...
		public COWSubList(AbstractDoubleList list, ReentrantLock lock, Consumer<double[]> setter, Supplier<double[]> getter, int offset, int from, int to) {
			this.list = list;
			this.lock = lock;
			this.setter = setter;
			this.getter = getter;
		...
		public double swapRemove(int index) {
			...
			lock.lock();
			try {
				double[] data = getter.get();
				double result = data[offset+index];
				double[] newElements = new double[data.length-1];
		...
		public void sort(DoubleComparator c) {
			...
			try {
				double[] data = getter.get();
				double[] newData = Arrays.copyOf(data, data.length);
				if(c != null) DoubleArrays.stableSort(newData, offset, offset+size, c);
				else DoubleArrays.stableSort(newData, offset, offset+size);
		...
		public void unstableSort(DoubleComparator c) {
			...
			try {
				double[] data = getter.get();
				double[] newData = Arrays.copyOf(data, data.length);
				if(c != null) DoubleArrays.unstableSort(newData, offset, offset+size, c);
				else DoubleArrays.unstableSort(newData, offset, offset+size);
				setter.accept(newData);
			}
			finally {
				lock.unlock();
		...
		public boolean removeAll(Collection<?> c) {
			...
			try {
				...
				for(int i= 0,m=data.length;i<m;i++) {
					...
					else if(!c.contains(Double.valueOf(data[i]))) newElements[j++] = data[i];
				}
				if(data.length != j) {
		...
		public boolean removeIf(Predicate<? super Double> filter) {
			...
			try {
				...
				for(int i= 0,m=data.length;i<m;i++) {
					if(i < offset || i >= offset+size) newElements[j++] = data[i];
					else if(!filter.test(Double.valueOf(data[i]))) newElements[j++] = data[i];
				}
				if(data.length != j) {
					setter.accept(Arrays.copyOf(newElements, j));
					size -= data.length - j;
		...
		public void replaceDoubles(DoubleUnaryOperator o) {
			...
			try {
				...
				for(int i = 0,m=size;i<m;i++)
					newData[i+offset] = o.applyAsDouble(newData[i+offset]);
				setter.accept(newData);
			}
			finally {
				lock.unlock();
