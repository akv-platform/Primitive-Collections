 ...
 */
public class Char2BooleanAVLTreeMap extends AbstractChar2BooleanMap implements Char2BooleanNavigableMap
{
	...
	/** EntrySet Cache */
	protected ObjectSet<Char2BooleanMap.Entry> entrySet;
	 ...
	 */
	public Char2BooleanAVLTreeMap(CharComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Char2BooleanAVLTreeMap(Character[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2BooleanAVLTreeMap(Character[] keys, Boolean[] values, CharComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2BooleanAVLTreeMap(char[] keys, boolean[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2BooleanAVLTreeMap(char[] keys, boolean[] values, CharComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Char2BooleanAVLTreeMap(Map<? extends Character, ? extends Boolean> map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2BooleanAVLTreeMap(Char2BooleanMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Char2BooleanAVLTreeMap(Char2BooleanMap map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Char2BooleanAVLTreeMap copy() {
		Char2BooleanAVLTreeMap set = new Char2BooleanAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractCharSet implements CharNavigableSet
	{
		...
		public KeySet(Char2BooleanNavigableMap map) {
			this.map = map;
		}
		...
		public Character lower(Character e) {
			Char2BooleanMap.Entry node = map.lowerEntry(e.charValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Character floor(Character e) {
			Char2BooleanMap.Entry node = map.floorEntry(e.charValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Character higher(Character e) {
			Char2BooleanMap.Entry node = map.higherEntry(e.charValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Character ceiling(Character e) {
			Char2BooleanMap.Entry node = map.ceilingEntry(e.charValue());
			return node != null ? node.getKey() : null;
		}
		...
		public CharBidirectionalIterator iterator(char fromElement) {
			if(map instanceof Char2BooleanAVLTreeMap) return ((Char2BooleanAVLTreeMap)map).keyIterator(fromElement);
		...
		public CharBidirectionalIterator iterator() {
			if(map instanceof Char2BooleanAVLTreeMap) return ((Char2BooleanAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public CharBidirectionalIterator descendingIterator() {
			if(map instanceof Char2BooleanAVLTreeMap) return ((Char2BooleanAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Char2BooleanAVLTreeMap) return ((Char2BooleanAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Char2BooleanAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Char2BooleanAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Char2BooleanAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Char2BooleanAVLTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Char2BooleanAVLTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractChar2BooleanMap implements Char2BooleanNavigableMap
	{
		final Char2BooleanAVLTreeMap map;
		final char lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;
		
		Char2BooleanNavigableMap inverse;
		CharNavigableSet keySet;
		ObjectSet<Char2BooleanMap.Entry> entrySet;
		BooleanCollection values;
		
		NavigableSubMap(Char2BooleanAVLTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public Char2BooleanMap.Entry lowerEntry(char key) { return subLower(key); }
		@Override
		public Char2BooleanMap.Entry floorEntry(char key) { return subFloor(key); }
		@Override
		public Char2BooleanMap.Entry ceilingEntry(char key) { return subCeiling(key); }
		@Override
		public Char2BooleanMap.Entry higherEntry(char key) { return subHigher(key); }
		...
		public Char2BooleanMap.Entry pollFirstEntry() {
			Node entry = subLowest();
			if(entry != null) {
				Char2BooleanMap.Entry result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Char2BooleanMap.Entry> {
			@Override
			public int size() {
				if (fromStart && toEnd) return map.size();
				int size = 0;
				for(ObjectIterator<Char2BooleanMap.Entry> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
		...
		class DecsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Char2BooleanMap.Entry>
		{
		...
		class AcsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Char2BooleanMap.Entry>
		{
	...
	class Values extends AbstractBooleanCollection
	{
		...
		@Override
		public int size() {
			return Char2BooleanAVLTreeMap.this.size;
		}
	...
	class EntrySet extends AbstractObjectSet<Char2BooleanMap.Entry> {
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Char2BooleanMap.Entry>
	{
	...
	private static final class Node implements Char2BooleanMap.Entry
	{
		char key;
		boolean value;
		int state;
		...
		Node(char key, boolean value, Node parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherKey instanceof Character && otherValue instanceof Boolean && key == ((Character)otherKey).charValue() && value == ((Boolean)otherValue).booleanValue();
			}
			return false;
		}
