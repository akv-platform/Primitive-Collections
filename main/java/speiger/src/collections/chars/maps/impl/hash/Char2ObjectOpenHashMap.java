 ...
 */
public class Char2ObjectOpenHashMap<V> extends AbstractChar2ObjectMap<V> implements ITrimmable
{
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (char)0 && Objects.equals(value, values[i])) return true;
		return false;
	}
	...
	public Char2ObjectOpenHashMap<V> copy() {
		Char2ObjectOpenHashMap<V> map = new Char2ObjectOpenHashMap<>(0, loadFactor);
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2ObjectMap.Entry) {
					...
					if(index >= 0) return Objects.equals(entry.getValue(), Char2ObjectOpenHashMap.this.values[index]);
				}
				else {
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public boolean matchesAny(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.getBoolean(values[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.getBoolean(values[i])) return false;
			}
			return true;
		}
		...
		public V findFirst(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.getBoolean(values[i])) return values[i];
			}
			return null;
		}
		...
		public int count(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (char)0 && filter.getBoolean(values[i])) result++;
			}
			return result;
		}
	}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2ObjectOpenHashMap.this.remove(wrapped.getChar(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
