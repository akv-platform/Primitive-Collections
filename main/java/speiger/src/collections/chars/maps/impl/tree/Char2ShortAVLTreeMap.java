 ...
 */
public class Char2ShortAVLTreeMap extends AbstractChar2ShortMap implements Char2ShortNavigableMap
{
	 ...
	 */
	public Char2ShortAVLTreeMap(CharComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Char2ShortAVLTreeMap(Character[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2ShortAVLTreeMap(Character[] keys, Short[] values, CharComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2ShortAVLTreeMap(char[] keys, short[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2ShortAVLTreeMap(char[] keys, short[] values, CharComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Char2ShortAVLTreeMap(Map<? extends Character, ? extends Short> map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2ShortAVLTreeMap(Char2ShortMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Char2ShortAVLTreeMap(Char2ShortMap map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Char2ShortAVLTreeMap copy() {
		Char2ShortAVLTreeMap set = new Char2ShortAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractCharSet implements CharNavigableSet
	{
		...
		public CharBidirectionalIterator iterator(char fromElement) {
			if(map instanceof Char2ShortAVLTreeMap) return ((Char2ShortAVLTreeMap)map).keyIterator(fromElement);
		...
		public CharBidirectionalIterator iterator() {
			if(map instanceof Char2ShortAVLTreeMap) return ((Char2ShortAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public CharBidirectionalIterator descendingIterator() {
			if(map instanceof Char2ShortAVLTreeMap) return ((Char2ShortAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Char2ShortAVLTreeMap) return ((Char2ShortAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Char2ShortAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Char2ShortAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Char2ShortAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Char2ShortAVLTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Char2ShortAVLTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Char2ShortAVLTreeMap map;
		final char lo, hi;
		...
		NavigableSubMap(Char2ShortAVLTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractShortCollection
	{
		...
		@Override
		public int size() {
			return Char2ShortAVLTreeMap.this.size;
		}
