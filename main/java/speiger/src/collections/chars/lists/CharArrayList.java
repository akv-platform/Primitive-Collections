 ...
 * 
 * <p>This implementation is optimized to improve how data is processed with interfaces like {@link ICharArray}, {@link CharStack}
...
public class CharArrayList extends AbstractCharList implements ICharArray, CharStack
{
	...
	public CharArrayList(Collection<? extends Character> c) {
		this(c.size());
		size = CharIterators.unwrap(data, c.iterator());
	}
	 ...
	 */
	public CharArrayList(char[] a, int length) {
		this(a, 0, length);
	}
	...
	public boolean addAll(int index, Collection<? extends Character> c) {
		...
		while(add-- != 0) data[index++] = iter.next().charValue();
		return true;
	}
	...
	public boolean addAll(int index, CharCollection c) {
		...
		while(add-- != 0) data[index++] = iter.nextChar();
		return true;
	}
	...
	public void sort(CharComparator c) {
		if(c != null) CharArrays.stableSort(data, size, c);
		else CharArrays.stableSort(data, size);
	}
	...
	public void unstableSort(CharComparator c) {
		if(c != null) CharArrays.unstableSort(data, size, c);
		else CharArrays.unstableSort(data, size);		
	}
	...
	public void replaceChars(IntUnaryOperator o) {
		Objects.requireNonNull(o);
		for(int i = 0;i<size;i++)
			data[i] = SanityChecks.castToChar(o.applyAsInt(data[i]));
	}
	...
	public boolean removeAll(Collection<?> c) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!c.contains(Character.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	 ...
	 * This function might delegate to a more appropriate function if necessary
	 * @param c the elements that should be kept. If empty, CharArrayList#clear is called.
	 * @return true if the collection was modified
	 * @throws NullPointerException if the collection is null
	 */
	@Override
	...
	public boolean removeIf(Predicate<? super Character> filter) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!filter.test(Character.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	...
	public void size(int size) {
		if(size > data.length)
			...
			Arrays.fill(data, size, size(), (char)0);
		this.size = size;
	}
	...
	public boolean trim(int size) {
		...
		data = value == 0 ? CharArrays.EMPTY_ARRAY : Arrays.copyOf(data, value);
		return true;
	}
	...
	public void clearAndTrim(int size) {
		...
		data = size == 0 ? CharArrays.EMPTY_ARRAY : new char[size];
		this.size = size;
	}
	...
	protected void grow(int capacity) {
		...
		data = Arrays.copyOf(data, data == CharArrays.EMPTY_ARRAY ? Math.max(DEFAULT_ARRAY_SIZE, capacity) : (int)Math.max(Math.min((long)data.length + (data.length >> 1), SanityChecks.MAX_ARRAY_SIZE), capacity));		
	}
	 ...
	 */
	@Override
	public IntStream primitiveStream() { return StreamSupport.intStream(CharSplititerators.createArrayJavaSplititerator(data, size, 16464), false); }
	 ...
	 */
	@Override
	public CharSplititerator spliterator() { return CharSplititerators.createArraySplititerator(data, size, 16464); }
}