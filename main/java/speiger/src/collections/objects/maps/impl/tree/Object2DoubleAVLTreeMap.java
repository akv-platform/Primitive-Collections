 ...
 */
public class Object2DoubleAVLTreeMap<T> extends AbstractObject2DoubleMap<T> implements Object2DoubleNavigableMap<T>
{
	 ...
	 */
	public Object2DoubleAVLTreeMap(Comparator<T> comp) {
		comparator = comp;
	}
	 ...
	 */
	public Object2DoubleAVLTreeMap(T[] keys, Double[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Object2DoubleAVLTreeMap(T[] keys, Double[] values, Comparator<T> comp) {
		comparator = comp;
	 ...
	 */
	public Object2DoubleAVLTreeMap(Map<? extends T, ? extends Double> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Object2DoubleAVLTreeMap(Map<? extends T, ? extends Double> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Object2DoubleAVLTreeMap(Object2DoubleMap<T> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Object2DoubleAVLTreeMap(Object2DoubleMap<T> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Object2DoubleAVLTreeMap<T> copy() {
		Object2DoubleAVLTreeMap<T> set = new Object2DoubleAVLTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<T> extends AbstractObjectSet<T> implements ObjectNavigableSet<T>
	{
		...
		public ObjectBidirectionalIterator<T> iterator(T fromElement) {
			if(map instanceof Object2DoubleAVLTreeMap) return ((Object2DoubleAVLTreeMap<T>)map).keyIterator(fromElement);
		...
		public ObjectBidirectionalIterator<T> iterator() {
			if(map instanceof Object2DoubleAVLTreeMap) return ((Object2DoubleAVLTreeMap<T>)map).keyIterator();
			return ((NavigableSubMap<T>)map).keyIterator();
		}
		
		@Override
		public ObjectBidirectionalIterator<T> descendingIterator() {
			if(map instanceof Object2DoubleAVLTreeMap) return ((Object2DoubleAVLTreeMap<T>)map).descendingKeyIterator();
			return ((NavigableSubMap<T>)map).descendingKeyIterator();
		}
		
		protected Node<T> start() {
			if(map instanceof Object2DoubleAVLTreeMap) return ((Object2DoubleAVLTreeMap<T>)map).first;
			return ((NavigableSubMap<T>)map).subLowest();
		}
		
		protected Node<T> end() {
			if(map instanceof Object2DoubleAVLTreeMap) return null;
			return ((NavigableSubMap<T>)map).subHighest();
		}
		
		protected Node<T> next(Node<T> entry) {
			if(map instanceof Object2DoubleAVLTreeMap) return entry.next();
			return ((NavigableSubMap<T>)map).next(entry);
		}
		
		protected Node<T> previous(Node<T> entry) {
			if(map instanceof Object2DoubleAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Object2DoubleAVLTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<T> extends NavigableSubMap<T>
	{
		...
		DescendingNaivgableSubMap(Object2DoubleAVLTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Object2DoubleAVLTreeMap<T> map;
		final T lo, hi;
		...
		NavigableSubMap(Object2DoubleAVLTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractDoubleCollection
	{
		...
		@Override
		public int size() {
			return Object2DoubleAVLTreeMap.this.size;
		}
