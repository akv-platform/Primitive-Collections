 ...
 */
public class Int2ObjectAVLTreeMap<V> extends AbstractInt2ObjectMap<V> implements Int2ObjectNavigableMap<V>
{
	 ...
	 */
	public Int2ObjectAVLTreeMap(Integer[] keys, V[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2ObjectAVLTreeMap(Integer[] keys, V[] values, IntComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2ObjectAVLTreeMap(int[] keys, V[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2ObjectAVLTreeMap(int[] keys, V[] values, IntComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Int2ObjectAVLTreeMap(Map<? extends Integer, ? extends V> map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Int2ObjectAVLTreeMap(Int2ObjectMap<V> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Int2ObjectAVLTreeMap(Int2ObjectMap<V> map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	public Int2ObjectAVLTreeMap<V> copy() {
		Int2ObjectAVLTreeMap<V> set = new Int2ObjectAVLTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<V> extends AbstractIntSet implements IntNavigableSet
	{
		...
		public IntBidirectionalIterator iterator(int fromElement) {
			if(map instanceof Int2ObjectAVLTreeMap) return ((Int2ObjectAVLTreeMap<V>)map).keyIterator(fromElement);
		...
		public IntBidirectionalIterator iterator() {
			if(map instanceof Int2ObjectAVLTreeMap) return ((Int2ObjectAVLTreeMap<V>)map).keyIterator();
			return ((NavigableSubMap<V>)map).keyIterator();
		}
		
		@Override
		public IntBidirectionalIterator descendingIterator() {
			if(map instanceof Int2ObjectAVLTreeMap) return ((Int2ObjectAVLTreeMap<V>)map).descendingKeyIterator();
			return ((NavigableSubMap<V>)map).descendingKeyIterator();
		}
		
		protected Node<V> start() {
			if(map instanceof Int2ObjectAVLTreeMap) return ((Int2ObjectAVLTreeMap<V>)map).first;
			return ((NavigableSubMap<V>)map).subLowest();
		}
		
		protected Node<V> end() {
			if(map instanceof Int2ObjectAVLTreeMap) return null;
			return ((NavigableSubMap<V>)map).subHighest();
		}
		
		protected Node<V> next(Node<V> entry) {
			if(map instanceof Int2ObjectAVLTreeMap) return entry.next();
			return ((NavigableSubMap<V>)map).next(entry);
		}
		
		protected Node<V> previous(Node<V> entry) {
			if(map instanceof Int2ObjectAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Int2ObjectAVLTreeMap<V> map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<V> extends NavigableSubMap<V>
	{
		...
		DescendingNaivgableSubMap(Int2ObjectAVLTreeMap<V> map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Int2ObjectAVLTreeMap<V> map;
		final int lo, hi;
		...
		NavigableSubMap(Int2ObjectAVLTreeMap<V> map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractObjectCollection<V>
	{
		...
		@Override
		public int size() {
			return Int2ObjectAVLTreeMap.this.size;
		}
