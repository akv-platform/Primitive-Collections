...
public class Int2DoubleArrayMap extends AbstractInt2DoubleMap implements Int2DoubleOrderedMap
{
	 ...
	 */
	public Int2DoubleArrayMap(Integer[] keys, Double[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in length
	 */
	public Int2DoubleArrayMap(Integer[] keys, Double[] values, int length) {
		this(length);
	 ...
	 */
	public Int2DoubleArrayMap(int[] keys, double[] values) {
		this(keys, values, keys.length);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2DoubleArrayMap(int[] keys, double[] values, int length) {
		this(length);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		putAll(keys, values, 0, length);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Int2DoubleArrayMap(Int2DoubleMap map) {
		this(map.size());
		for(ObjectIterator<Int2DoubleMap.Entry> iter = Int2DoubleMaps.fastIterator(map);iter.hasNext();size++) {
	...
	public Int2DoubleArrayMap copy() {
		Int2DoubleArrayMap map = new Int2DoubleArrayMap();
		map.size = size;
	...
	protected void insertIndex(int index, int key, double value) {
		grow(size+1);
		if(index != size) {
	...
	protected int findIndex(int key, double value) {
	...
	protected int findIndex(Object key, Object value) {
		for(int i = size-1;i>=0;i--)
			if(Objects.equals(key, Integer.valueOf(keys[i])) && Objects.equals(value, Double.valueOf(values[i]))) return i;
		return -1;		
	}
	...
	private class MapEntrySet extends AbstractObjectSet<Int2DoubleMap.Entry> implements Int2DoubleOrderedMap.FastOrderedSet {
		...
		public boolean moveToFirst(Int2DoubleMap.Entry o) {
			return Int2DoubleArrayMap.this.moveToFirst(o.getIntKey());
		}
		
		@Override
		public boolean moveToLast(Int2DoubleMap.Entry o) {
			return Int2DoubleArrayMap.this.moveToLast(o.getIntKey());
		}
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Int2DoubleMap.Entry) {
					...
					int index = Int2DoubleArrayMap.this.findIndex(entry.getIntKey());
					if(index >= 0) return Double.doubleToLongBits(entry.getDoubleValue()) == Double.doubleToLongBits(Int2DoubleArrayMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Int2DoubleArrayMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Double.valueOf(Int2DoubleArrayMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Int2DoubleMap.Entry) {
					Int2DoubleMap.Entry entry = (Int2DoubleMap.Entry)o;
					return Int2DoubleArrayMap.this.remove(entry.getIntKey(), entry.getDoubleValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Int2DoubleArrayMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	...
	private class KeySet extends AbstractIntSet implements IntOrderedSet {
		...
		public boolean remove(int o) {
			int oldSize = size;
			Int2DoubleArrayMap.this.remove(o);
			return size != oldSize;
		}
		...
		@Override
		public boolean moveToFirst(int o) { return Int2DoubleArrayMap.this.moveToFirst(o); }
		@Override
		public boolean moveToLast(int o) { return Int2DoubleArrayMap.this.moveToLast(o); }
		@Override
		...
		@Override
		public int size() { return Int2DoubleArrayMap.this.size(); }
		@Override
		public void clear() { Int2DoubleArrayMap.this.clear(); }
		@Override
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Int2DoubleMap.Entry> {
		...
		@Override
		public void set(Int2DoubleMap.Entry e) { throw new UnsupportedOperationException(); }
		@Override
		public void add(Int2DoubleMap.Entry e) { throw new UnsupportedOperationException(); }
	}
	...
	private class MapEntry implements Int2DoubleMap.Entry, Map.Entry<Integer, Double> {
		int index = -1;
		...
		@Override
		public int hashCode() {
			return Integer.hashCode(keys[index]) ^ Double.hashCode(values[index]);
		}
