...
public class Int2DoubleOpenHashMap extends AbstractInt2DoubleMap implements ITrimmable
{
	...
	public Int2DoubleOpenHashMap copy() {
		Int2DoubleOpenHashMap map = new Int2DoubleOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2DoubleMap.Entry> implements Int2DoubleMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Int2DoubleMap.Entry) {
					...
					int index = Int2DoubleOpenHashMap.this.findIndex(entry.getIntKey());
					if(index >= 0) return Double.doubleToLongBits(entry.getDoubleValue()) == Double.doubleToLongBits(Int2DoubleOpenHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Int2DoubleOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Double.valueOf(Int2DoubleOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Int2DoubleMap.Entry) {
					Int2DoubleMap.Entry entry = (Int2DoubleMap.Entry)o;
					return Int2DoubleOpenHashMap.this.remove(entry.getIntKey(), entry.getDoubleValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Int2DoubleOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractIntSet {
		...
		public boolean remove(int o) {
			int oldSize = size;
			Int2DoubleOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Int2DoubleOpenHashMap.this.remove(wrapped.getInt(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
