 ...
 */
public class Int2ByteOpenHashMap extends AbstractInt2ByteMap implements ITrimmable
{
	 ...
	 */
	public Int2ByteOpenHashMap(int minCapacity) {
	 ...
	 */
	public Int2ByteOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Int2ByteOpenHashMap(Integer[] keys, Byte[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Int2ByteOpenHashMap(Integer[] keys, Byte[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2ByteOpenHashMap(int[] keys, byte[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Int2ByteOpenHashMap(int[] keys, byte[] values, float loadFactor) {
		this(keys.length, loadFactor);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Int2ByteOpenHashMap(Map<? extends Integer, ? extends Byte> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Int2ByteOpenHashMap(Int2ByteMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Int2ByteOpenHashMap(Int2ByteMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != 0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Byte.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Int2ByteOpenHashMap copy() {
		Int2ByteOpenHashMap map = new Int2ByteOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2ByteMap.Entry> implements Int2ByteMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Int2ByteMap.Entry) {
					...
					int index = Int2ByteOpenHashMap.this.findIndex(entry.getIntKey());
					if(index >= 0) return entry.getByteValue() == Int2ByteOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Int2ByteOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Byte.valueOf(Int2ByteOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Int2ByteMap.Entry) {
					Int2ByteMap.Entry entry = (Int2ByteMap.Entry)o;
					return Int2ByteOpenHashMap.this.remove(entry.getIntKey(), entry.getByteValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Int2ByteOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractIntSet {
		...
		public boolean remove(int o) {
			int oldSize = size;
			Int2ByteOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Int2ByteOpenHashMap.this.remove(wrapped.getInt(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
