...
public class Int2FloatAVLTreeMap extends AbstractInt2FloatMap implements Int2FloatNavigableMap
{
	 ...
	 */
	public Int2FloatAVLTreeMap(Integer[] keys, Float[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2FloatAVLTreeMap(Integer[] keys, Float[] values, IntComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2FloatAVLTreeMap(int[] keys, float[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2FloatAVLTreeMap(int[] keys, float[] values, IntComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Int2FloatAVLTreeMap(Map<? extends Integer, ? extends Float> map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Int2FloatAVLTreeMap(Int2FloatMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Int2FloatAVLTreeMap(Int2FloatMap map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Int2FloatAVLTreeMap copy() {
		Int2FloatAVLTreeMap set = new Int2FloatAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractIntSet implements IntNavigableSet
	{
		...
		public IntBidirectionalIterator iterator(int fromElement) {
			if(map instanceof Int2FloatAVLTreeMap) return ((Int2FloatAVLTreeMap)map).keyIterator(fromElement);
		...
		public IntBidirectionalIterator iterator() {
			if(map instanceof Int2FloatAVLTreeMap) return ((Int2FloatAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public IntBidirectionalIterator descendingIterator() {
			if(map instanceof Int2FloatAVLTreeMap) return ((Int2FloatAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Int2FloatAVLTreeMap) return ((Int2FloatAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Int2FloatAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Int2FloatAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Int2FloatAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Int2FloatAVLTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Int2FloatAVLTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractInt2FloatMap implements Int2FloatNavigableMap
	{
		...
		NavigableSubMap(Int2FloatAVLTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractFloatCollection
	{
		...
		@Override
		public int size() {
			return Int2FloatAVLTreeMap.this.size;
		}
