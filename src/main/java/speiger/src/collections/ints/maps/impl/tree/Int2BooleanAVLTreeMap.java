 ...
 */
public class Int2BooleanAVLTreeMap extends AbstractInt2BooleanMap implements Int2BooleanNavigableMap
{
	 ...
	 */
	public Int2BooleanAVLTreeMap(IntComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Int2BooleanAVLTreeMap(Integer[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2BooleanAVLTreeMap(Integer[] keys, Boolean[] values, IntComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2BooleanAVLTreeMap(int[] keys, boolean[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2BooleanAVLTreeMap(int[] keys, boolean[] values, IntComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Int2BooleanAVLTreeMap(Map<? extends Integer, ? extends Boolean> map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Int2BooleanAVLTreeMap(Int2BooleanMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Int2BooleanAVLTreeMap(Int2BooleanMap map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Int2BooleanAVLTreeMap copy() {
		Int2BooleanAVLTreeMap set = new Int2BooleanAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractIntSet implements IntNavigableSet
	{
		...
		public IntBidirectionalIterator iterator(int fromElement) {
			if(map instanceof Int2BooleanAVLTreeMap) return ((Int2BooleanAVLTreeMap)map).keyIterator(fromElement);
		...
		public IntBidirectionalIterator iterator() {
			if(map instanceof Int2BooleanAVLTreeMap) return ((Int2BooleanAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public IntBidirectionalIterator descendingIterator() {
			if(map instanceof Int2BooleanAVLTreeMap) return ((Int2BooleanAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Int2BooleanAVLTreeMap) return ((Int2BooleanAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Int2BooleanAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Int2BooleanAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Int2BooleanAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Int2BooleanAVLTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Int2BooleanAVLTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Int2BooleanAVLTreeMap map;
		final int lo, hi;
		...
		NavigableSubMap(Int2BooleanAVLTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractBooleanCollection
	{
		...
		@Override
		public int size() {
			return Int2BooleanAVLTreeMap.this.size;
		}
