...
public class Int2DoubleRBTreeMap extends AbstractInt2DoubleMap implements Int2DoubleNavigableMap
{
	 ...
	 */
	public Int2DoubleRBTreeMap(Integer[] keys, Double[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2DoubleRBTreeMap(Integer[] keys, Double[] values, IntComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2DoubleRBTreeMap(int[] keys, double[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Int2DoubleRBTreeMap(int[] keys, double[] values, IntComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Int2DoubleRBTreeMap(Map<? extends Integer, ? extends Double> map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Int2DoubleRBTreeMap(Int2DoubleMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Int2DoubleRBTreeMap(Int2DoubleMap map, IntComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Int2DoubleRBTreeMap copy() {
		Int2DoubleRBTreeMap set = new Int2DoubleRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractIntSet implements IntNavigableSet
	{
		...
		public IntBidirectionalIterator iterator(int fromElement) {
			if(map instanceof Int2DoubleRBTreeMap) return ((Int2DoubleRBTreeMap)map).keyIterator(fromElement);
		...
		public IntBidirectionalIterator iterator() {
			if(map instanceof Int2DoubleRBTreeMap) return ((Int2DoubleRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public IntBidirectionalIterator descendingIterator() {
			if(map instanceof Int2DoubleRBTreeMap) return ((Int2DoubleRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Int2DoubleRBTreeMap) return ((Int2DoubleRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Int2DoubleRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Int2DoubleRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Int2DoubleRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Int2DoubleRBTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Int2DoubleRBTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractInt2DoubleMap implements Int2DoubleNavigableMap
	{
		...
		NavigableSubMap(Int2DoubleRBTreeMap map, boolean fromStart, int lo, boolean loInclusive, boolean toEnd, int hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractDoubleCollection
	{
		...
		@Override
		public int size() {
			return Int2DoubleRBTreeMap.this.size;
		}
