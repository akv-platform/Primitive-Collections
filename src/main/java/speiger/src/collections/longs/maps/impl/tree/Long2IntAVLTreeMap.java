...
public class Long2IntAVLTreeMap extends AbstractLong2IntMap implements Long2IntNavigableMap
{
	...
	/** EntrySet Cache */
	protected ObjectSet<Long2IntMap.Entry> entrySet;
	 ...
	 */
	public Long2IntAVLTreeMap(Long[] keys, Integer[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2IntAVLTreeMap(Long[] keys, Integer[] values, LongComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2IntAVLTreeMap(long[] keys, int[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2IntAVLTreeMap(long[] keys, int[] values, LongComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Long2IntAVLTreeMap(Map<? extends Long, ? extends Integer> map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2IntAVLTreeMap(Long2IntMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Long2IntAVLTreeMap(Long2IntMap map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Long2IntAVLTreeMap copy() {
		Long2IntAVLTreeMap set = new Long2IntAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractLongSet implements LongNavigableSet
	{
		...
		public KeySet(Long2IntNavigableMap map) {
			this.map = map;
		}
		...
		public Long lower(Long e) {
			Long2IntMap.Entry node = map.lowerEntry(e.longValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Long floor(Long e) {
			Long2IntMap.Entry node = map.floorEntry(e.longValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Long higher(Long e) {
			Long2IntMap.Entry node = map.higherEntry(e.longValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Long ceiling(Long e) {
			Long2IntMap.Entry node = map.ceilingEntry(e.longValue());
			return node != null ? node.getKey() : null;
		}
		...
		public LongBidirectionalIterator iterator(long fromElement) {
			if(map instanceof Long2IntAVLTreeMap) return ((Long2IntAVLTreeMap)map).keyIterator(fromElement);
		...
		public LongBidirectionalIterator iterator() {
			if(map instanceof Long2IntAVLTreeMap) return ((Long2IntAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public LongBidirectionalIterator descendingIterator() {
			if(map instanceof Long2IntAVLTreeMap) return ((Long2IntAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Long2IntAVLTreeMap) return ((Long2IntAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Long2IntAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Long2IntAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Long2IntAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Long2IntAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Long2IntAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractLong2IntMap implements Long2IntNavigableMap
	{
		final Long2IntAVLTreeMap map;
		final long lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;
		
		Long2IntNavigableMap inverse;
		LongNavigableSet keySet;
		ObjectSet<Long2IntMap.Entry> entrySet;
		IntCollection values;
		
		NavigableSubMap(Long2IntAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public Long2IntMap.Entry lowerEntry(long key) { return subLower(key); }
		@Override
		public Long2IntMap.Entry floorEntry(long key) { return subFloor(key); }
		@Override
		public Long2IntMap.Entry ceilingEntry(long key) { return subCeiling(key); }
		@Override
		public Long2IntMap.Entry higherEntry(long key) { return subHigher(key); }
		...
		@Override
		public Long2IntNavigableMap copy() { throw new UnsupportedOperationException(); }
		...
		public Long2IntMap.Entry pollFirstEntry() {
			Node entry = subLowest();
			if(entry != null) {
				Long2IntMap.Entry result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Long2IntMap.Entry> {
			@Override
			public int size() {
				if (fromStart && toEnd) return map.size();
				int size = 0;
				for(ObjectIterator<Long2IntMap.Entry> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
		...
		class DecsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Long2IntMap.Entry>
		{
		...
		class AcsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Long2IntMap.Entry>
		{
	...
	class Values extends AbstractIntCollection
	{
		...
		@Override
		public int size() {
			return Long2IntAVLTreeMap.this.size;
		}
	...
	class EntrySet extends AbstractObjectSet<Long2IntMap.Entry> {
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Long2IntMap.Entry>
	{
	...
	private static final class Node implements Long2IntMap.Entry
	{
		long key;
		int value;
		int state;
		...
		Node(long key, int value, Node parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherKey instanceof Long && otherValue instanceof Integer && key == ((Long)otherKey).longValue() && value == ((Integer)otherValue).intValue();
			}
			return false;
		}
