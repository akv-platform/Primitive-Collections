 ...
 */
public class Long2ObjectOpenHashMap<V> extends AbstractLong2ObjectMap<V> implements ITrimmable
{
	 ...
	 */
	public Long2ObjectOpenHashMap(int minCapacity) {
	 ...
	 */
	public Long2ObjectOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Long2ObjectOpenHashMap(Long[] keys, V[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Long2ObjectOpenHashMap(Long[] keys, V[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Long2ObjectOpenHashMap(Map<? extends Long, ? extends V> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2ObjectOpenHashMap(Long2ObjectMap<V> map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Long2ObjectOpenHashMap(Long2ObjectMap<V> map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	@Override
	public Long2ObjectOpenHashMap<V> copy() {
		Long2ObjectOpenHashMap<V> map = new Long2ObjectOpenHashMap<>(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ObjectMap.Entry<V>> implements Long2ObjectMap.FastEntrySet<V> {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Long2ObjectMap.Entry) {
					...
					int index = Long2ObjectOpenHashMap.this.findIndex(entry.getLongKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Long2ObjectOpenHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Long2ObjectOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Long2ObjectOpenHashMap.this.values[index]);
				}
			}
			return false;
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Long2ObjectMap.Entry) {
					Long2ObjectMap.Entry<V> entry = (Long2ObjectMap.Entry<V>)o;
					return Long2ObjectOpenHashMap.this.remove(entry.getLongKey(), entry.getValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Long2ObjectOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractLongSet {
		...
		public boolean remove(long o) {
			int oldSize = size;
			Long2ObjectOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2ObjectOpenHashMap.this.remove(wrapped.getLong(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
