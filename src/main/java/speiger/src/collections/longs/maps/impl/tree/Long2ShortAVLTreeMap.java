...
public class Long2ShortAVLTreeMap extends AbstractLong2ShortMap implements Long2ShortNavigableMap
{
	 ...
	 */
	public Long2ShortAVLTreeMap(LongComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Long2ShortAVLTreeMap(Long[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2ShortAVLTreeMap(Long[] keys, Short[] values, LongComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Long2ShortAVLTreeMap(Map<? extends Long, ? extends Short> map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2ShortAVLTreeMap(Long2ShortMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Long2ShortAVLTreeMap(Long2ShortMap map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	public Long2ShortAVLTreeMap copy() {
		Long2ShortAVLTreeMap set = new Long2ShortAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractLongSet implements LongNavigableSet
	{
		...
		public LongBidirectionalIterator iterator(long fromElement) {
			if(map instanceof Long2ShortAVLTreeMap) return ((Long2ShortAVLTreeMap)map).keyIterator(fromElement);
		...
		public LongBidirectionalIterator iterator() {
			if(map instanceof Long2ShortAVLTreeMap) return ((Long2ShortAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public LongBidirectionalIterator descendingIterator() {
			if(map instanceof Long2ShortAVLTreeMap) return ((Long2ShortAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Long2ShortAVLTreeMap) return ((Long2ShortAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Long2ShortAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Long2ShortAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Long2ShortAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Long2ShortAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Long2ShortAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractLong2ShortMap implements Long2ShortNavigableMap
	{
		...
		NavigableSubMap(Long2ShortAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
