 ...
 */
public class ImmutableLong2FloatOpenHashMap extends AbstractLong2FloatMap implements Long2FloatOrderedMap
{
	 ...
	 */
	protected ImmutableLong2FloatOpenHashMap() {}
	 ...
	 */
	public ImmutableLong2FloatOpenHashMap(Long[] keys, Float[] values) {
	 ...
	 */
	public ImmutableLong2FloatOpenHashMap(Long[] keys, Float[] values, float loadFactor) {
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		init(LongArrays.unwrap(keys), FloatArrays.unwrap(values), 0, keys.length, loadFactor);
	}
	 ...
	 */
	public ImmutableLong2FloatOpenHashMap(Map<? extends Long, ? extends Float> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public ImmutableLong2FloatOpenHashMap(Map<? extends Long, ? extends Float> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public ImmutableLong2FloatOpenHashMap(Long2FloatMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public ImmutableLong2FloatOpenHashMap(Long2FloatMap map, float loadFactor) {
		long[] keys = new long[map.size()];
		float[] values = new float[keys.length];
		int index = 0;
		for(Long2FloatMap.Entry entry : Long2FloatMaps.fastIterable(map)) {
	...
	protected void init(long[] a, float[] b, int offset, int length, float loadFactor) {
	...
	@Override
	public ImmutableLong2FloatOpenHashMap copy() {
		ImmutableLong2FloatOpenHashMap map = new ImmutableLong2FloatOpenHashMap();
		map.mask = mask;
	...
	@Override
	public boolean replace(long key, float oldValue, float newValue) { throw new UnsupportedOperationException(); }
	
	@Override
	public float replace(long key, float value) { throw new UnsupportedOperationException(); }
	...
	private class MapEntrySet extends AbstractObjectSet<Long2FloatMap.Entry> implements Long2FloatOrderedMap.FastOrderedSet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Long2FloatMap.Entry) {
					...
					int index = ImmutableLong2FloatOpenHashMap.this.findIndex(entry.getLongKey());
					if(index >= 0) return Float.floatToIntBits(entry.getFloatValue()) == Float.floatToIntBits(ImmutableLong2FloatOpenHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = ImmutableLong2FloatOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Float.valueOf(ImmutableLong2FloatOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		@Override
		public int size() {
			return ImmutableLong2FloatOpenHashMap.this.size();
		}
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Long2FloatMap.Entry> {
		...
		@Override
		public void set(Long2FloatMap.Entry entry) { throw new UnsupportedOperationException(); }

		@Override
		public void add(Long2FloatMap.Entry entry) { throw new UnsupportedOperationException(); }
	}
