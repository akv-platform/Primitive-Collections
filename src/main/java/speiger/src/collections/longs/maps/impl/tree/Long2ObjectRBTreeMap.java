 ...
 */
public class Long2ObjectRBTreeMap<V> extends AbstractLong2ObjectMap<V> implements Long2ObjectNavigableMap<V>
{
	 ...
	 */
	public Long2ObjectRBTreeMap(Long[] keys, V[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2ObjectRBTreeMap(Long[] keys, V[] values, LongComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Long2ObjectRBTreeMap(Map<? extends Long, ? extends V> map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2ObjectRBTreeMap(Long2ObjectMap<V> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Long2ObjectRBTreeMap(Long2ObjectMap<V> map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	public Long2ObjectRBTreeMap<V> copy() {
		Long2ObjectRBTreeMap<V> set = new Long2ObjectRBTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<V> extends AbstractLongSet implements LongNavigableSet
	{
		...
		public LongBidirectionalIterator iterator(long fromElement) {
			if(map instanceof Long2ObjectRBTreeMap) return ((Long2ObjectRBTreeMap<V>)map).keyIterator(fromElement);
		...
		public LongBidirectionalIterator iterator() {
			if(map instanceof Long2ObjectRBTreeMap) return ((Long2ObjectRBTreeMap<V>)map).keyIterator();
			return ((NavigableSubMap<V>)map).keyIterator();
		}
		
		@Override
		public LongBidirectionalIterator descendingIterator() {
			if(map instanceof Long2ObjectRBTreeMap) return ((Long2ObjectRBTreeMap<V>)map).descendingKeyIterator();
			return ((NavigableSubMap<V>)map).descendingKeyIterator();
		}
		
		protected Node<V> start() {
			if(map instanceof Long2ObjectRBTreeMap) return ((Long2ObjectRBTreeMap<V>)map).first;
			return ((NavigableSubMap<V>)map).subLowest();
		}
		
		protected Node<V> end() {
			if(map instanceof Long2ObjectRBTreeMap) return null;
			return ((NavigableSubMap<V>)map).subHighest();
		}
		
		protected Node<V> next(Node<V> entry) {
			if(map instanceof Long2ObjectRBTreeMap) return entry.next();
			return ((NavigableSubMap<V>)map).next(entry);
		}
		
		protected Node<V> previous(Node<V> entry) {
			if(map instanceof Long2ObjectRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Long2ObjectRBTreeMap<V> map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<V> extends NavigableSubMap<V>
	{
		...
		DescendingNaivgableSubMap(Long2ObjectRBTreeMap<V> map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Long2ObjectRBTreeMap<V> map;
		final long lo, hi;
		...
		NavigableSubMap(Long2ObjectRBTreeMap<V> map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractObjectCollection<V>
	{
		...
		@Override
		public int size() {
			return Long2ObjectRBTreeMap.this.size;
		}
