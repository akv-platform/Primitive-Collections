 ...
 * 
 */
public class Long2LongConcurrentOpenHashMap extends AbstractLong2LongMap implements Long2LongConcurrentMap
{
	/** Segment Limit */
	...
	protected static class Segment extends StampedLock
	{
		...
		protected long computeIfAbsent(int hash, long key, Long2LongFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
