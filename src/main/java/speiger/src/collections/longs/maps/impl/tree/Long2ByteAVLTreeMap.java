 ...
 */
public class Long2ByteAVLTreeMap extends AbstractLong2ByteMap implements Long2ByteNavigableMap
{
	 ...
	 */
	public Long2ByteAVLTreeMap(LongComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Long2ByteAVLTreeMap(Long[] keys, Byte[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2ByteAVLTreeMap(Long[] keys, Byte[] values, LongComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Long2ByteAVLTreeMap(Map<? extends Long, ? extends Byte> map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2ByteAVLTreeMap(Long2ByteMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Long2ByteAVLTreeMap(Long2ByteMap map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Long2ByteAVLTreeMap copy() {
		Long2ByteAVLTreeMap set = new Long2ByteAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractLongSet implements LongNavigableSet
	{
		...
		public LongBidirectionalIterator iterator(long fromElement) {
			if(map instanceof Long2ByteAVLTreeMap) return ((Long2ByteAVLTreeMap)map).keyIterator(fromElement);
		...
		public LongBidirectionalIterator iterator() {
			if(map instanceof Long2ByteAVLTreeMap) return ((Long2ByteAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public LongBidirectionalIterator descendingIterator() {
			if(map instanceof Long2ByteAVLTreeMap) return ((Long2ByteAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Long2ByteAVLTreeMap) return ((Long2ByteAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Long2ByteAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Long2ByteAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Long2ByteAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Long2ByteAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Long2ByteAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractLong2ByteMap implements Long2ByteNavigableMap
	{
		...
		NavigableSubMap(Long2ByteAVLTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractByteCollection
	{
		...
		@Override
		public int size() {
			return Long2ByteAVLTreeMap.this.size;
		}
