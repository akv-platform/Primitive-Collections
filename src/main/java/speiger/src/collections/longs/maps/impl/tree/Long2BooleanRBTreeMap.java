 ...
 */
public class Long2BooleanRBTreeMap extends AbstractLong2BooleanMap implements Long2BooleanNavigableMap
{
	 ...
	 */
	public Long2BooleanRBTreeMap(LongComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Long2BooleanRBTreeMap(Long[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2BooleanRBTreeMap(Long[] keys, Boolean[] values, LongComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Long2BooleanRBTreeMap(Map<? extends Long, ? extends Boolean> map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2BooleanRBTreeMap(Long2BooleanMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Long2BooleanRBTreeMap(Long2BooleanMap map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Long2BooleanRBTreeMap copy() {
		Long2BooleanRBTreeMap set = new Long2BooleanRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractLongSet implements LongNavigableSet
	{
		...
		public LongBidirectionalIterator iterator(long fromElement) {
			if(map instanceof Long2BooleanRBTreeMap) return ((Long2BooleanRBTreeMap)map).keyIterator(fromElement);
		...
		public LongBidirectionalIterator iterator() {
			if(map instanceof Long2BooleanRBTreeMap) return ((Long2BooleanRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public LongBidirectionalIterator descendingIterator() {
			if(map instanceof Long2BooleanRBTreeMap) return ((Long2BooleanRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Long2BooleanRBTreeMap) return ((Long2BooleanRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Long2BooleanRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Long2BooleanRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Long2BooleanRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Long2BooleanRBTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Long2BooleanRBTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Long2BooleanRBTreeMap map;
		final long lo, hi;
		...
		NavigableSubMap(Long2BooleanRBTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractBooleanCollection
	{
		...
		@Override
		public int size() {
			return Long2BooleanRBTreeMap.this.size;
		}
