...
public class Long2CharOpenHashMap extends AbstractLong2CharMap implements ITrimmable
{
	 ...
	 */
	public Long2CharOpenHashMap(int minCapacity) {
	 ...
	 */
	public Long2CharOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Long2CharOpenHashMap(Long[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Long2CharOpenHashMap(Long[] keys, Character[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2CharOpenHashMap(long[] keys, char[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Long2CharOpenHashMap(long[] keys, char[] values, float loadFactor) {
		this(keys.length, loadFactor);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Long2CharOpenHashMap(Map<? extends Long, ? extends Character> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2CharOpenHashMap(Long2CharMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Long2CharOpenHashMap(Long2CharMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public Long2CharOpenHashMap copy() {
		Long2CharOpenHashMap map = new Long2CharOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2CharMap.Entry> implements Long2CharMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Long2CharMap.Entry) {
					...
					int index = Long2CharOpenHashMap.this.findIndex(entry.getLongKey());
					if(index >= 0) return entry.getCharValue() == Long2CharOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Long2CharOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Character.valueOf(Long2CharOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				...
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Long2CharOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractLongSet {
		...
		public boolean remove(long o) {
			int oldSize = size;
			Long2CharOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2CharOpenHashMap.this.remove(wrapped.getLong(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
