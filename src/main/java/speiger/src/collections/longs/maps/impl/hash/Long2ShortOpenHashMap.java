 ...
 */
public class Long2ShortOpenHashMap extends AbstractLong2ShortMap implements ITrimmable
{
	 ...
	 */
	public Long2ShortOpenHashMap(int minCapacity) {
	 ...
	 */
	public Long2ShortOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Long2ShortOpenHashMap(Long[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Long2ShortOpenHashMap(Long[] keys, Short[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Long2ShortOpenHashMap(Map<? extends Long, ? extends Short> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2ShortOpenHashMap(Long2ShortMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Long2ShortOpenHashMap(Long2ShortMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	@Override
	public Long2ShortOpenHashMap copy() {
		Long2ShortOpenHashMap map = new Long2ShortOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ShortMap.Entry> implements Long2ShortMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Long2ShortMap.Entry) {
					...
					int index = Long2ShortOpenHashMap.this.findIndex(entry.getLongKey());
					if(index >= 0) return entry.getShortValue() == Long2ShortOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Long2ShortOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short.valueOf(Long2ShortOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Long2ShortMap.Entry) {
					Long2ShortMap.Entry entry = (Long2ShortMap.Entry)o;
					return Long2ShortOpenHashMap.this.remove(entry.getLongKey(), entry.getShortValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Long2ShortOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractLongSet {
		...
		public boolean remove(long o) {
			int oldSize = size;
			Long2ShortOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Long2ShortOpenHashMap.this.remove(wrapped.getLong(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
