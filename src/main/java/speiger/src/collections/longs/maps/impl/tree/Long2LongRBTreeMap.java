 ...
 */
public class Long2LongRBTreeMap extends AbstractLong2LongMap implements Long2LongNavigableMap
{
	 ...
	 */
	public Long2LongRBTreeMap(LongComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Long2LongRBTreeMap(Long[] keys, Long[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Long2LongRBTreeMap(Long[] keys, Long[] values, LongComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Long2LongRBTreeMap(Map<? extends Long, ? extends Long> map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Long2LongRBTreeMap(Long2LongMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Long2LongRBTreeMap(Long2LongMap map, LongComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Long2LongRBTreeMap copy() {
		Long2LongRBTreeMap set = new Long2LongRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractLongSet implements LongNavigableSet
	{
		...
		public LongBidirectionalIterator iterator(long fromElement) {
			if(map instanceof Long2LongRBTreeMap) return ((Long2LongRBTreeMap)map).keyIterator(fromElement);
		...
		public LongBidirectionalIterator iterator() {
			if(map instanceof Long2LongRBTreeMap) return ((Long2LongRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public LongBidirectionalIterator descendingIterator() {
			if(map instanceof Long2LongRBTreeMap) return ((Long2LongRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Long2LongRBTreeMap) return ((Long2LongRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Long2LongRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Long2LongRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Long2LongRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Long2LongRBTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Long2LongRBTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractLong2LongMap implements Long2LongNavigableMap
	{
		...
		NavigableSubMap(Long2LongRBTreeMap map, boolean fromStart, long lo, boolean loInclusive, boolean toEnd, long hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractLongCollection
	{
		...
		@Override
		public int size() {
			return Long2LongRBTreeMap.this.size;
		}
