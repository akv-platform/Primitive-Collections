...
public interface Float2FloatMap extends Map<Float, Float>, Float2FloatFunction
{
	...
	public default Float computeIfAbsent(Float key, Function<? super Float, ? extends Float> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Float.valueOf(computeFloatIfAbsent(key.floatValue(), mappingFunction instanceof Float2FloatFunction ? (Float2FloatFunction)mappingFunction : K -> mappingFunction.apply(Float.valueOf(K)).floatValue()));
	}
