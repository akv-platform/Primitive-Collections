 ...
 */
public class Float2CharAVLTreeMap extends AbstractFloat2CharMap implements Float2CharNavigableMap
{
	...
	/** EntrySet Cache */
	protected ObjectSet<Float2CharMap.Entry> entrySet;
	 ...
	 */
	public Float2CharAVLTreeMap(FloatComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Float2CharAVLTreeMap(Float[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2CharAVLTreeMap(Float[] keys, Character[] values, FloatComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2CharAVLTreeMap(float[] keys, char[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2CharAVLTreeMap(float[] keys, char[] values, FloatComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Float2CharAVLTreeMap(Map<? extends Float, ? extends Character> map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Float2CharAVLTreeMap(Float2CharMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Float2CharAVLTreeMap(Float2CharMap map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Float2CharAVLTreeMap copy() {
		Float2CharAVLTreeMap set = new Float2CharAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractFloatSet implements FloatNavigableSet
	{
		...
		public KeySet(Float2CharNavigableMap map) {
			this.map = map;
		}
		...
		public FloatBidirectionalIterator iterator(float fromElement) {
			if(map instanceof Float2CharAVLTreeMap) return ((Float2CharAVLTreeMap)map).keyIterator(fromElement);
		...
		public FloatBidirectionalIterator iterator() {
			if(map instanceof Float2CharAVLTreeMap) return ((Float2CharAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public FloatBidirectionalIterator descendingIterator() {
			if(map instanceof Float2CharAVLTreeMap) return ((Float2CharAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Float2CharAVLTreeMap) return ((Float2CharAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Float2CharAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Float2CharAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Float2CharAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Float2CharAVLTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Float2CharAVLTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractFloat2CharMap implements Float2CharNavigableMap
	{
		final Float2CharAVLTreeMap map;
		final float lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;
		
		Float2CharNavigableMap inverse;
		FloatNavigableSet keySet;
		ObjectSet<Float2CharMap.Entry> entrySet;
		CharCollection values;
		
		NavigableSubMap(Float2CharAVLTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public Float2CharMap.Entry lowerEntry(float key) { return subLower(key); }
		@Override
		public Float2CharMap.Entry floorEntry(float key) { return subFloor(key); }
		@Override
		public Float2CharMap.Entry ceilingEntry(float key) { return subCeiling(key); }
		@Override
		public Float2CharMap.Entry higherEntry(float key) { return subHigher(key); }
		...
		@Override
		public Float2CharNavigableMap copy() { throw new UnsupportedOperationException(); }
		...
		public Float2CharMap.Entry pollFirstEntry() {
			Node entry = subLowest();
			if(entry != null) {
				Float2CharMap.Entry result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Float2CharMap.Entry> {
			@Override
			public int size() {
				if (fromStart && toEnd) return map.size();
				int size = 0;
				for(ObjectIterator<Float2CharMap.Entry> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
		...
		class DecsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Float2CharMap.Entry>
		{
		...
		class AcsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Float2CharMap.Entry>
		{
	...
	class Values extends AbstractCharCollection
	{
		...
		@Override
		public int size() {
			return Float2CharAVLTreeMap.this.size;
		}
	...
	class EntrySet extends AbstractObjectSet<Float2CharMap.Entry> {
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Float2CharMap.Entry>
	{
	...
	private static final class Node implements Float2CharMap.Entry
	{
		float key;
		char value;
		int state;
		...
		Node(float key, char value, Node parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherKey instanceof Float && otherValue instanceof Character && Float.floatToIntBits(key) == Float.floatToIntBits(((Float)otherKey).floatValue()) && value == ((Character)otherValue).charValue();
			}
			return false;
		}
