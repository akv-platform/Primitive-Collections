 ...
 */
public class ImmutableFloat2IntOpenHashMap extends AbstractFloat2IntMap implements Float2IntOrderedMap
{
	 ...
	 */
	protected ImmutableFloat2IntOpenHashMap() {}
	 ...
	 */
	public ImmutableFloat2IntOpenHashMap(Float[] keys, Integer[] values) {
	 ...
	 */
	public ImmutableFloat2IntOpenHashMap(Float[] keys, Integer[] values, float loadFactor) {
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		init(FloatArrays.unwrap(keys), IntArrays.unwrap(values), 0, keys.length, loadFactor);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public ImmutableFloat2IntOpenHashMap(float[] keys, int[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public ImmutableFloat2IntOpenHashMap(float[] keys, int[] values, float loadFactor) {
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 */
	public ImmutableFloat2IntOpenHashMap(Map<? extends Float, ? extends Integer> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public ImmutableFloat2IntOpenHashMap(Map<? extends Float, ? extends Integer> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public ImmutableFloat2IntOpenHashMap(Float2IntMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public ImmutableFloat2IntOpenHashMap(Float2IntMap map, float loadFactor) {
		float[] keys = new float[map.size()];
		int[] values = new int[keys.length];
		int index = 0;
		for(Float2IntMap.Entry entry : Float2IntMaps.fastIterable(map)) {
	...
	protected void init(float[] a, int[] b, int offset, int length, float loadFactor) {
	...
	@Override
	public ImmutableFloat2IntOpenHashMap copy() {
		ImmutableFloat2IntOpenHashMap map = new ImmutableFloat2IntOpenHashMap();
		map.mask = mask;
	...
	@Override
	public boolean replace(float key, int oldValue, int newValue) { throw new UnsupportedOperationException(); }
	
	@Override
	public int replace(float key, int value) { throw new UnsupportedOperationException(); }
	...
	private class MapEntrySet extends AbstractObjectSet<Float2IntMap.Entry> implements Float2IntOrderedMap.FastOrderedSet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2IntMap.Entry) {
					...
					int index = ImmutableFloat2IntOpenHashMap.this.findIndex(entry.getFloatKey());
					if(index >= 0) return entry.getIntValue() == ImmutableFloat2IntOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = ImmutableFloat2IntOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Integer.valueOf(ImmutableFloat2IntOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		@Override
		public int size() {
			return ImmutableFloat2IntOpenHashMap.this.size();
		}
