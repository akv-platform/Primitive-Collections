 ...
 */
public class Float2ShortOpenHashMap extends AbstractFloat2ShortMap implements ITrimmable
{
	 ...
	 */
	public Float2ShortOpenHashMap(int minCapacity) {
	 ...
	 */
	public Float2ShortOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Float2ShortOpenHashMap(Float[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Float2ShortOpenHashMap(Float[] keys, Short[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Float2ShortOpenHashMap(Map<? extends Float, ? extends Short> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Float2ShortOpenHashMap(Float2ShortMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Float2ShortOpenHashMap(Float2ShortMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(Float.floatToIntBits(keys[i]) != 0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Short.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Float2ShortOpenHashMap copy() {
		Float2ShortOpenHashMap map = new Float2ShortOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2ShortMap.Entry> implements Float2ShortMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2ShortMap.Entry) {
					...
					int index = Float2ShortOpenHashMap.this.findIndex(entry.getFloatKey());
					if(index >= 0) return entry.getShortValue() == Float2ShortOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Float2ShortOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short.valueOf(Float2ShortOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2ShortMap.Entry) {
					Float2ShortMap.Entry entry = (Float2ShortMap.Entry)o;
					return Float2ShortOpenHashMap.this.remove(entry.getFloatKey(), entry.getShortValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Float2ShortOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractFloatSet {
		...
		public boolean remove(float o) {
			int oldSize = size;
			Float2ShortOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Float2ShortOpenHashMap.this.remove(wrapped.getFloat(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
