 ...
 */
public class Float2FloatOpenHashMap extends AbstractFloat2FloatMap implements ITrimmable
{
	 ...
	 */
	public Float2FloatOpenHashMap(int minCapacity) {
	 ...
	 */
	public Float2FloatOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Float2FloatOpenHashMap(Float[] keys, Float[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Float2FloatOpenHashMap(Float[] keys, Float[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Float2FloatOpenHashMap(Map<? extends Float, ? extends Float> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Float2FloatOpenHashMap(Float2FloatMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Float2FloatOpenHashMap(Float2FloatMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(float value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(Float.floatToIntBits(keys[i]) != 0 && Float.floatToIntBits(values[i]) == Float.floatToIntBits(value)) return true;
		return false;
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(Float.floatToIntBits(keys[i]) != 0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Float.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Float2FloatOpenHashMap copy() {
		Float2FloatOpenHashMap map = new Float2FloatOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2FloatMap.Entry> implements Float2FloatMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2FloatMap.Entry) {
					...
					int index = Float2FloatOpenHashMap.this.findIndex(entry.getFloatKey());
					if(index >= 0) return Float.floatToIntBits(entry.getFloatValue()) == Float.floatToIntBits(Float2FloatOpenHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Float2FloatOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Float.valueOf(Float2FloatOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2FloatMap.Entry) {
					Float2FloatMap.Entry entry = (Float2FloatMap.Entry)o;
					return Float2FloatOpenHashMap.this.remove(entry.getFloatKey(), entry.getFloatValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Float2FloatOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractFloatSet {
		...
		public boolean remove(float o) {
			int oldSize = size;
			Float2FloatOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Float2FloatOpenHashMap.this.remove(wrapped.getFloat(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
