 ...
 */
public class Float2LongRBTreeMap extends AbstractFloat2LongMap implements Float2LongNavigableMap
{
	 ...
	 */
	public Float2LongRBTreeMap(FloatComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Float2LongRBTreeMap(Float[] keys, Long[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2LongRBTreeMap(Float[] keys, Long[] values, FloatComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Float2LongRBTreeMap(Map<? extends Float, ? extends Long> map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Float2LongRBTreeMap(Float2LongMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Float2LongRBTreeMap(Float2LongMap map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Float2LongRBTreeMap copy() {
		Float2LongRBTreeMap set = new Float2LongRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	@Override
	public Float2LongNavigableMap subMap(float fromKey, boolean fromInclusive, float toKey, boolean toInclusive) {
		return new AscendingNaivgableSubMap(this, false, fromKey, fromInclusive, false, toKey, toInclusive);
	}
	
	@Override
	public Float2LongNavigableMap headMap(float toKey, boolean inclusive) {
		return new AscendingNaivgableSubMap(this, true, 0F, true, false, toKey, inclusive);
	}
	
	@Override
	public Float2LongNavigableMap tailMap(float fromKey, boolean inclusive) {
	...
	@Override
	public Float2LongMap.Entry lowerEntry(float key) {
		Node node = findLowerNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Float2LongMap.Entry higherEntry(float key) {
		Node node = findHigherNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Float2LongMap.Entry floorEntry(float key) {
		Node node = findFloorNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Float2LongMap.Entry ceilingEntry(float key) {
	...
	static class KeySet extends AbstractFloatSet implements FloatNavigableSet
	{
		...
		public KeySet(Float2LongNavigableMap map) {
			this.map = map;
		}
		...
		public FloatBidirectionalIterator iterator(float fromElement) {
			if(map instanceof Float2LongRBTreeMap) return ((Float2LongRBTreeMap)map).keyIterator(fromElement);
		...
		public FloatBidirectionalIterator iterator() {
			if(map instanceof Float2LongRBTreeMap) return ((Float2LongRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public FloatBidirectionalIterator descendingIterator() {
			if(map instanceof Float2LongRBTreeMap) return ((Float2LongRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Float2LongRBTreeMap) return ((Float2LongRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Float2LongRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Float2LongRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Float2LongRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Float2LongRBTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Float2LongRBTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractFloat2LongMap implements Float2LongNavigableMap
	{
		final Float2LongRBTreeMap map;
		final float lo, hi;
		...
		NavigableSubMap(Float2LongRBTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public Float2LongMap.Entry lowerEntry(float key) { return subLower(key); }
		@Override
		public Float2LongMap.Entry floorEntry(float key) { return subFloor(key); }
		@Override
		public Float2LongMap.Entry ceilingEntry(float key) { return subCeiling(key); }
		@Override
		public Float2LongMap.Entry higherEntry(float key) { return subHigher(key); }
		...
		public Float2LongMap.Entry pollFirstEntry() {
			Node entry = subLowest();
			if(entry != null) {
				Float2LongMap.Entry result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Float2LongMap.Entry> {
			...
			public int size() {
				...
				int size = 0;
				for(ObjectIterator<Float2LongMap.Entry> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
		...
		class DecsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Float2LongMap.Entry>
		{
		...
		class AcsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Float2LongMap.Entry>
		{
	...
	class Values extends AbstractLongCollection
	{
		...
		@Override
		public int size() {
			return Float2LongRBTreeMap.this.size;
		}
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Float2LongMap.Entry>
	{
	...
	private static final class Node implements Float2LongMap.Entry
	{
		...
		Node(float key, long value, Node parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherKey instanceof Float && otherValue instanceof Long && Float.floatToIntBits(key) == Float.floatToIntBits(((Float)otherKey).floatValue()) && value == ((Long)otherValue).longValue();
			}
			return false;
		}
