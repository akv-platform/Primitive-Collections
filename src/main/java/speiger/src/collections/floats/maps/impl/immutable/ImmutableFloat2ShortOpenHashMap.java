 ...
 */
public class ImmutableFloat2ShortOpenHashMap extends AbstractFloat2ShortMap implements Float2ShortOrderedMap
{
	 ...
	 */
	protected ImmutableFloat2ShortOpenHashMap() {}
	 ...
	 */
	public ImmutableFloat2ShortOpenHashMap(Float[] keys, Short[] values) {
	 ...
	 */
	public ImmutableFloat2ShortOpenHashMap(Float[] keys, Short[] values, float loadFactor) {
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		init(FloatArrays.unwrap(keys), ShortArrays.unwrap(values), 0, keys.length, loadFactor);
	}
	 ...
	 */
	public ImmutableFloat2ShortOpenHashMap(Map<? extends Float, ? extends Short> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public ImmutableFloat2ShortOpenHashMap(Map<? extends Float, ? extends Short> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public ImmutableFloat2ShortOpenHashMap(Float2ShortMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public ImmutableFloat2ShortOpenHashMap(Float2ShortMap map, float loadFactor) {
		float[] keys = new float[map.size()];
		short[] values = new short[keys.length];
		int index = 0;
		for(Float2ShortMap.Entry entry : Float2ShortMaps.fastIterable(map)) {
	...
	protected void init(float[] a, short[] b, int offset, int length, float loadFactor) {
	...
	@Override
	public ImmutableFloat2ShortOpenHashMap copy() {
		ImmutableFloat2ShortOpenHashMap map = new ImmutableFloat2ShortOpenHashMap();
		map.mask = mask;
	...
	@Override
	public boolean replace(float key, short oldValue, short newValue) { throw new UnsupportedOperationException(); }
	
	@Override
	public short replace(float key, short value) { throw new UnsupportedOperationException(); }
	...
	private class MapEntrySet extends AbstractObjectSet<Float2ShortMap.Entry> implements Float2ShortOrderedMap.FastOrderedSet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2ShortMap.Entry) {
					...
					int index = ImmutableFloat2ShortOpenHashMap.this.findIndex(entry.getFloatKey());
					if(index >= 0) return entry.getShortValue() == ImmutableFloat2ShortOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = ImmutableFloat2ShortOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short.valueOf(ImmutableFloat2ShortOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		@Override
		public int size() {
			return ImmutableFloat2ShortOpenHashMap.this.size();
		}
