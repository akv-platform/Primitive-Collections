...
public interface Float2BooleanMap extends Map<Float, Boolean>, Float2BooleanFunction
{
	...
	public default Boolean computeIfAbsent(Float key, Function<? super Float, ? extends Boolean> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		return Boolean.valueOf(computeBooleanIfAbsent(key.floatValue(), mappingFunction instanceof Float2BooleanFunction ? (Float2BooleanFunction)mappingFunction : K -> mappingFunction.apply(Float.valueOf(K)).booleanValue()));
	}
