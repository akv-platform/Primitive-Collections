...
public class Float2IntArrayMap extends AbstractFloat2IntMap implements Float2IntOrderedMap
{
	 ...
	 */
	public Float2IntArrayMap(Float[] keys, Integer[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in length
	 */
	public Float2IntArrayMap(Float[] keys, Integer[] values, int length) {
		this(length);
	 ...
	 */
	public Float2IntArrayMap(float[] keys, int[] values) {
		this(keys, values, keys.length);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2IntArrayMap(float[] keys, int[] values, int length) {
		this(length);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		putAll(keys, values, 0, length);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Float2IntArrayMap(Float2IntMap map) {
		this(map.size());
		for(ObjectIterator<Float2IntMap.Entry> iter = Float2IntMaps.fastIterator(map);iter.hasNext();size++) {
	...
	public Float2IntArrayMap copy() {
		Float2IntArrayMap map = new Float2IntArrayMap();
		map.size = size;
	...
	protected void insertIndex(int index, float key, int value) {
		grow(size+1);
		if(index != size) {
	...
	protected int findIndex(float key, int value) {
	...
	protected int findIndex(Object key, Object value) {
		for(int i = size-1;i>=0;i--)
			if(Objects.equals(key, Float.valueOf(keys[i])) && Objects.equals(value, Integer.valueOf(values[i]))) return i;
		return -1;		
	}
	...
	private class MapEntrySet extends AbstractObjectSet<Float2IntMap.Entry> implements Float2IntOrderedMap.FastOrderedSet {
		...
		public boolean moveToFirst(Float2IntMap.Entry o) {
			return Float2IntArrayMap.this.moveToFirst(o.getFloatKey());
		}
		
		@Override
		public boolean moveToLast(Float2IntMap.Entry o) {
			return Float2IntArrayMap.this.moveToLast(o.getFloatKey());
		}
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2IntMap.Entry) {
					...
					int index = Float2IntArrayMap.this.findIndex(entry.getFloatKey());
					if(index >= 0) return entry.getIntValue() == Float2IntArrayMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Float2IntArrayMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Integer.valueOf(Float2IntArrayMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				...
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Float2IntArrayMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	...
	private class KeySet extends AbstractFloatSet implements FloatOrderedSet {
		...
		public boolean remove(float o) {
			int oldSize = size;
			Float2IntArrayMap.this.remove(o);
			return size != oldSize;
		}
		...
		@Override
		public boolean moveToFirst(float o) { return Float2IntArrayMap.this.moveToFirst(o); }
		@Override
		public boolean moveToLast(float o) { return Float2IntArrayMap.this.moveToLast(o); }
		@Override
		...
		@Override
		public int size() { return Float2IntArrayMap.this.size(); }
		@Override
		public void clear() { Float2IntArrayMap.this.clear(); }
		@Override
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Float2IntMap.Entry> {
		...
		@Override
		public void set(Float2IntMap.Entry e) { throw new UnsupportedOperationException(); }
		@Override
		public void add(Float2IntMap.Entry e) { throw new UnsupportedOperationException(); }
	}
	...
	private class MapEntry implements Float2IntMap.Entry, Map.Entry<Float, Integer> {
		int index = -1;
