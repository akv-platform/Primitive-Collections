...
public class Float2CharRBTreeMap extends AbstractFloat2CharMap implements Float2CharNavigableMap
{
	 ...
	 */
	public Float2CharRBTreeMap(Float[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2CharRBTreeMap(Float[] keys, Character[] values, FloatComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2CharRBTreeMap(float[] keys, char[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2CharRBTreeMap(float[] keys, char[] values, FloatComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Float2CharRBTreeMap(Map<? extends Float, ? extends Character> map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Float2CharRBTreeMap(Float2CharMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Float2CharRBTreeMap(Float2CharMap map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Float2CharRBTreeMap copy() {
		Float2CharRBTreeMap set = new Float2CharRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractFloatSet implements FloatNavigableSet
	{
		...
		public FloatBidirectionalIterator iterator(float fromElement) {
			if(map instanceof Float2CharRBTreeMap) return ((Float2CharRBTreeMap)map).keyIterator(fromElement);
		...
		public FloatBidirectionalIterator iterator() {
			if(map instanceof Float2CharRBTreeMap) return ((Float2CharRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public FloatBidirectionalIterator descendingIterator() {
			if(map instanceof Float2CharRBTreeMap) return ((Float2CharRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Float2CharRBTreeMap) return ((Float2CharRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Float2CharRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Float2CharRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Float2CharRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Float2CharRBTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Float2CharRBTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractFloat2CharMap implements Float2CharNavigableMap
	{
		...
		NavigableSubMap(Float2CharRBTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractCharCollection
	{
		...
		@Override
		public int size() {
			return Float2CharRBTreeMap.this.size;
		}
