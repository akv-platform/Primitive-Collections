 ...
 */
public class Float2ObjectOpenCustomHashMap<V> extends AbstractFloat2ObjectMap<V> implements ITrimmable
{
	 ...
	 */
	public Float2ObjectOpenCustomHashMap(FloatStrategy strategy) {
	 ...
	 */
	public Float2ObjectOpenCustomHashMap(int minCapacity, FloatStrategy strategy) {
	 ...
	 */
	public Float2ObjectOpenCustomHashMap(int minCapacity, float loadFactor, FloatStrategy strategy) {
	 ...
	 */
	public Float2ObjectOpenCustomHashMap(Float[] keys, V[] values, FloatStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Float2ObjectOpenCustomHashMap(Float[] keys, V[] values, float loadFactor, FloatStrategy strategy) {
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Float2ObjectOpenCustomHashMap(Map<? extends Float, ? extends V> map, FloatStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Float2ObjectOpenCustomHashMap(Map<? extends Float, ? extends V> map, float loadFactor, FloatStrategy strategy) {
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Float2ObjectOpenCustomHashMap(Float2ObjectMap<V> map, FloatStrategy strategy) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Float2ObjectOpenCustomHashMap(Float2ObjectMap<V> map, float loadFactor, FloatStrategy strategy) {
		this(map.size(), loadFactor, strategy);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(!strategy.equals(keys[i], 0F) && Objects.equals(value, values[i])) return true;
		return false;
	}
	...
	@Override
	public Float2ObjectOpenCustomHashMap<V> copy() {
		Float2ObjectOpenCustomHashMap<V> map = new Float2ObjectOpenCustomHashMap<>(0, loadFactor, strategy);
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2ObjectMap.Entry<V>> implements Float2ObjectMap.FastEntrySet<V> {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2ObjectMap.Entry) {
					...
					int index = Float2ObjectOpenCustomHashMap.this.findIndex(entry.getFloatKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Float2ObjectOpenCustomHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					if(!(entry.getKey() instanceof Float)) return false;
					int index = Float2ObjectOpenCustomHashMap.this.findIndex((Float)entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Float2ObjectOpenCustomHashMap.this.values[index]);
				}
			}
			return false;
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Float2ObjectMap.Entry) {
					Float2ObjectMap.Entry<V> entry = (Float2ObjectMap.Entry<V>)o;
					return Float2ObjectOpenCustomHashMap.this.remove(entry.getFloatKey(), entry.getValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Float2ObjectOpenCustomHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractFloatSet {
		...
		public boolean remove(float o) {
			int oldSize = size;
			Float2ObjectOpenCustomHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public boolean matchesAny(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0F) && filter.getBoolean(values[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0F) && filter.getBoolean(values[i])) return false;
			}
			return true;
		}
		...
		public V findFirst(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0F) && filter.getBoolean(values[i])) return values[i];
			}
			return null;
		}
		...
		public int count(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0F) && filter.getBoolean(values[i])) result++;
			}
			return result;
		}
	}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Float2ObjectOpenCustomHashMap.this.remove(wrapped.getFloat(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
