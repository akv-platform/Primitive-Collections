 ...
 */
public class Float2ObjectAVLTreeMap<V> extends AbstractFloat2ObjectMap<V> implements Float2ObjectNavigableMap<V>
{
	 ...
	 */
	public Float2ObjectAVLTreeMap(Float[] keys, V[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2ObjectAVLTreeMap(Float[] keys, V[] values, FloatComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Float2ObjectAVLTreeMap(Map<? extends Float, ? extends V> map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Float2ObjectAVLTreeMap(Float2ObjectMap<V> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Float2ObjectAVLTreeMap(Float2ObjectMap<V> map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Float2ObjectAVLTreeMap<V> copy() {
		Float2ObjectAVLTreeMap<V> set = new Float2ObjectAVLTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<V> extends AbstractFloatSet implements FloatNavigableSet
	{
		...
		public FloatBidirectionalIterator iterator(float fromElement) {
			if(map instanceof Float2ObjectAVLTreeMap) return ((Float2ObjectAVLTreeMap<V>)map).keyIterator(fromElement);
		...
		public FloatBidirectionalIterator iterator() {
			if(map instanceof Float2ObjectAVLTreeMap) return ((Float2ObjectAVLTreeMap<V>)map).keyIterator();
			return ((NavigableSubMap<V>)map).keyIterator();
		}
		
		@Override
		public FloatBidirectionalIterator descendingIterator() {
			if(map instanceof Float2ObjectAVLTreeMap) return ((Float2ObjectAVLTreeMap<V>)map).descendingKeyIterator();
			return ((NavigableSubMap<V>)map).descendingKeyIterator();
		}
		
		protected Node<V> start() {
			if(map instanceof Float2ObjectAVLTreeMap) return ((Float2ObjectAVLTreeMap<V>)map).first;
			return ((NavigableSubMap<V>)map).subLowest();
		}
		
		protected Node<V> end() {
			if(map instanceof Float2ObjectAVLTreeMap) return null;
			return ((NavigableSubMap<V>)map).subHighest();
		}
		
		protected Node<V> next(Node<V> entry) {
			if(map instanceof Float2ObjectAVLTreeMap) return entry.next();
			return ((NavigableSubMap<V>)map).next(entry);
		}
		
		protected Node<V> previous(Node<V> entry) {
			if(map instanceof Float2ObjectAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Float2ObjectAVLTreeMap<V> map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<V> extends NavigableSubMap<V>
	{
		...
		DescendingNaivgableSubMap(Float2ObjectAVLTreeMap<V> map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Float2ObjectAVLTreeMap<V> map;
		final float lo, hi;
		...
		NavigableSubMap(Float2ObjectAVLTreeMap<V> map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractObjectCollection<V>
	{
		...
		@Override
		public int size() {
			return Float2ObjectAVLTreeMap.this.size;
		}
