 ...
 */
public class Float2ShortRBTreeMap extends AbstractFloat2ShortMap implements Float2ShortNavigableMap
{
	 ...
	 */
	public Float2ShortRBTreeMap(FloatComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Float2ShortRBTreeMap(Float[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Float2ShortRBTreeMap(Float[] keys, Short[] values, FloatComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Float2ShortRBTreeMap(Map<? extends Float, ? extends Short> map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Float2ShortRBTreeMap(Float2ShortMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Float2ShortRBTreeMap(Float2ShortMap map, FloatComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Float2ShortRBTreeMap copy() {
		Float2ShortRBTreeMap set = new Float2ShortRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractFloatSet implements FloatNavigableSet
	{
		...
		public Float lower(Float e) {
			Float2ShortMap.Entry node = map.lowerEntry(e.floatValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Float floor(Float e) {
			Float2ShortMap.Entry node = map.floorEntry(e.floatValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Float higher(Float e) {
			Float2ShortMap.Entry node = map.higherEntry(e.floatValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Float ceiling(Float e) {
			Float2ShortMap.Entry node = map.ceilingEntry(e.floatValue());
			return node != null ? node.getKey() : null;
		}
		...
		public FloatBidirectionalIterator iterator(float fromElement) {
			if(map instanceof Float2ShortRBTreeMap) return ((Float2ShortRBTreeMap)map).keyIterator(fromElement);
		...
		public FloatBidirectionalIterator iterator() {
			if(map instanceof Float2ShortRBTreeMap) return ((Float2ShortRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public FloatBidirectionalIterator descendingIterator() {
			if(map instanceof Float2ShortRBTreeMap) return ((Float2ShortRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Float2ShortRBTreeMap) return ((Float2ShortRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Float2ShortRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Float2ShortRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Float2ShortRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Float2ShortRBTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Float2ShortRBTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Float2ShortRBTreeMap map;
		final float lo, hi;
		...
		NavigableSubMap(Float2ShortRBTreeMap map, boolean fromStart, float lo, boolean loInclusive, boolean toEnd, float hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractShortCollection
	{
		...
		@Override
		public int size() {
			return Float2ShortRBTreeMap.this.size;
		}
