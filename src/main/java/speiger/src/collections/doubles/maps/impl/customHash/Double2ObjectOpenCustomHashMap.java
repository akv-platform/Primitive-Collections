 ...
 */
public class Double2ObjectOpenCustomHashMap<V> extends AbstractDouble2ObjectMap<V> implements ITrimmable
{
	 ...
	 */
	public Double2ObjectOpenCustomHashMap(DoubleStrategy strategy) {
	 ...
	 */
	public Double2ObjectOpenCustomHashMap(int minCapacity, DoubleStrategy strategy) {
	 ...
	 */
	public Double2ObjectOpenCustomHashMap(int minCapacity, float loadFactor, DoubleStrategy strategy) {
	 ...
	 */
	public Double2ObjectOpenCustomHashMap(Double[] keys, V[] values, DoubleStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Double2ObjectOpenCustomHashMap(Double[] keys, V[] values, float loadFactor, DoubleStrategy strategy) {
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Double2ObjectOpenCustomHashMap(Map<? extends Double, ? extends V> map, DoubleStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Double2ObjectOpenCustomHashMap(Map<? extends Double, ? extends V> map, float loadFactor, DoubleStrategy strategy) {
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Double2ObjectOpenCustomHashMap(Double2ObjectMap<V> map, DoubleStrategy strategy) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Double2ObjectOpenCustomHashMap(Double2ObjectMap<V> map, float loadFactor, DoubleStrategy strategy) {
		this(map.size(), loadFactor, strategy);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(!strategy.equals(keys[i], 0D) && Objects.equals(value, values[i])) return true;
		return false;
	}
	...
	@Override
	public Double2ObjectOpenCustomHashMap<V> copy() {
		Double2ObjectOpenCustomHashMap<V> map = new Double2ObjectOpenCustomHashMap<>(0, loadFactor, strategy);
	...
	public void mergeAll(Double2ObjectMap<V> m, ObjectObjectUnaryOperator<V, V> mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		for(Double2ObjectMap.Entry<V> entry : Double2ObjectMaps.fastIterable(m)) {
	...
	private final class MapEntrySet extends AbstractObjectSet<Double2ObjectMap.Entry<V>> implements Double2ObjectMap.FastEntrySet<V> {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Double2ObjectMap.Entry) {
					...
					int index = Double2ObjectOpenCustomHashMap.this.findIndex(entry.getDoubleKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Double2ObjectOpenCustomHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					if(!(entry.getKey() instanceof Double)) return false;
					int index = Double2ObjectOpenCustomHashMap.this.findIndex((Double)entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Double2ObjectOpenCustomHashMap.this.values[index]);
				}
			}
			return false;
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Double2ObjectMap.Entry) {
					Double2ObjectMap.Entry<V> entry = (Double2ObjectMap.Entry<V>)o;
					return Double2ObjectOpenCustomHashMap.this.remove(entry.getDoubleKey(), entry.getValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Double2ObjectOpenCustomHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractDoubleSet {
		...
		public boolean remove(double o) {
			int oldSize = size;
			Double2ObjectOpenCustomHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public boolean matchesAny(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0D) && filter.getBoolean(values[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0D) && filter.getBoolean(values[i])) return false;
			}
			return true;
		}
		...
		public V findFirst(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0D) && filter.getBoolean(values[i])) return values[i];
			}
			return null;
		}
		...
		public int count(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], 0D) && filter.getBoolean(values[i])) result++;
			}
			return result;
		}
	}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Double2ObjectOpenCustomHashMap.this.remove(wrapped.getDouble(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
