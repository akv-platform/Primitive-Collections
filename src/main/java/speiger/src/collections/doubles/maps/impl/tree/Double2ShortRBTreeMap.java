...
public class Double2ShortRBTreeMap extends AbstractDouble2ShortMap implements Double2ShortNavigableMap
{
	 ...
	 */
	public Double2ShortRBTreeMap(Double[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Double2ShortRBTreeMap(Double[] keys, Short[] values, DoubleComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Double2ShortRBTreeMap(Map<? extends Double, ? extends Short> map, DoubleComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Double2ShortRBTreeMap(Double2ShortMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Double2ShortRBTreeMap(Double2ShortMap map, DoubleComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Double2ShortRBTreeMap copy() {
		Double2ShortRBTreeMap set = new Double2ShortRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractDoubleSet implements DoubleNavigableSet
	{
		...
		public DoubleBidirectionalIterator iterator(double fromElement) {
			if(map instanceof Double2ShortRBTreeMap) return ((Double2ShortRBTreeMap)map).keyIterator(fromElement);
		...
		public DoubleBidirectionalIterator iterator() {
			if(map instanceof Double2ShortRBTreeMap) return ((Double2ShortRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public DoubleBidirectionalIterator descendingIterator() {
			if(map instanceof Double2ShortRBTreeMap) return ((Double2ShortRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Double2ShortRBTreeMap) return ((Double2ShortRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Double2ShortRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Double2ShortRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Double2ShortRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Double2ShortRBTreeMap map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Double2ShortRBTreeMap map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Double2ShortRBTreeMap map;
		final double lo, hi;
		...
		NavigableSubMap(Double2ShortRBTreeMap map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractShortCollection
	{
		...
		@Override
		public int size() {
			return Double2ShortRBTreeMap.this.size;
		}
