...
public class Double2CharAVLTreeMap extends AbstractDouble2CharMap implements Double2CharNavigableMap
{
	 ...
	 */
	public Double2CharAVLTreeMap(Double[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Double2CharAVLTreeMap(Double[] keys, Character[] values, DoubleComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Double2CharAVLTreeMap(double[] keys, char[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Double2CharAVLTreeMap(double[] keys, char[] values, DoubleComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Double2CharAVLTreeMap(Map<? extends Double, ? extends Character> map, DoubleComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Double2CharAVLTreeMap(Double2CharMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Double2CharAVLTreeMap(Double2CharMap map, DoubleComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Double2CharAVLTreeMap copy() {
		Double2CharAVLTreeMap set = new Double2CharAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractDoubleSet implements DoubleNavigableSet
	{
		...
		public DoubleBidirectionalIterator iterator(double fromElement) {
			if(map instanceof Double2CharAVLTreeMap) return ((Double2CharAVLTreeMap)map).keyIterator(fromElement);
		...
		public DoubleBidirectionalIterator iterator() {
			if(map instanceof Double2CharAVLTreeMap) return ((Double2CharAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public DoubleBidirectionalIterator descendingIterator() {
			if(map instanceof Double2CharAVLTreeMap) return ((Double2CharAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Double2CharAVLTreeMap) return ((Double2CharAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Double2CharAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Double2CharAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Double2CharAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Double2CharAVLTreeMap map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Double2CharAVLTreeMap map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Double2CharAVLTreeMap map;
		final double lo, hi;
		...
		NavigableSubMap(Double2CharAVLTreeMap map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractCharCollection
	{
		...
		@Override
		public int size() {
			return Double2CharAVLTreeMap.this.size;
		}
