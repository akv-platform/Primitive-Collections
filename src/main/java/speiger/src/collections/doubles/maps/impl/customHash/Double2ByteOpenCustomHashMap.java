...
public class Double2ByteOpenCustomHashMap extends AbstractDouble2ByteMap implements ITrimmable
{
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(!strategy.equals(keys[i], 0D) && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Byte.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Double2ByteOpenCustomHashMap copy() {
		Double2ByteOpenCustomHashMap map = new Double2ByteOpenCustomHashMap(0, loadFactor, strategy);
	...
	private final class MapEntrySet extends AbstractObjectSet<Double2ByteMap.Entry> implements Double2ByteMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Double2ByteMap.Entry) {
					...
					int index = Double2ByteOpenCustomHashMap.this.findIndex(entry.getDoubleKey());
					if(index >= 0) return entry.getByteValue() == Double2ByteOpenCustomHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					if(!(entry.getKey() instanceof Double)) return false;
					int index = Double2ByteOpenCustomHashMap.this.findIndex((Double)entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Byte.valueOf(Double2ByteOpenCustomHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Double2ByteMap.Entry) {
					Double2ByteMap.Entry entry = (Double2ByteMap.Entry)o;
					return Double2ByteOpenCustomHashMap.this.remove(entry.getDoubleKey(), entry.getByteValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Double2ByteOpenCustomHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractDoubleSet {
		...
		public boolean remove(double o) {
			int oldSize = size;
			Double2ByteOpenCustomHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Double2ByteOpenCustomHashMap.this.remove(wrapped.getDouble(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
