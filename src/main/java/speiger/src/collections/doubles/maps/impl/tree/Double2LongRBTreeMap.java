...
public class Double2LongRBTreeMap extends AbstractDouble2LongMap implements Double2LongNavigableMap
{
	 ...
	 */
	public Double2LongRBTreeMap(Double[] keys, Long[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Double2LongRBTreeMap(Double[] keys, Long[] values, DoubleComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Double2LongRBTreeMap(Map<? extends Double, ? extends Long> map, DoubleComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Double2LongRBTreeMap(Double2LongMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Double2LongRBTreeMap(Double2LongMap map, DoubleComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Double2LongRBTreeMap copy() {
		Double2LongRBTreeMap set = new Double2LongRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractDoubleSet implements DoubleNavigableSet
	{
		...
		public DoubleBidirectionalIterator iterator(double fromElement) {
			if(map instanceof Double2LongRBTreeMap) return ((Double2LongRBTreeMap)map).keyIterator(fromElement);
		...
		public DoubleBidirectionalIterator iterator() {
			if(map instanceof Double2LongRBTreeMap) return ((Double2LongRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public DoubleBidirectionalIterator descendingIterator() {
			if(map instanceof Double2LongRBTreeMap) return ((Double2LongRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Double2LongRBTreeMap) return ((Double2LongRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Double2LongRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Double2LongRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Double2LongRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Double2LongRBTreeMap map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Double2LongRBTreeMap map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Double2LongRBTreeMap map;
		final double lo, hi;
		...
		NavigableSubMap(Double2LongRBTreeMap map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractLongCollection
	{
		...
		@Override
		public int size() {
			return Double2LongRBTreeMap.this.size;
		}
