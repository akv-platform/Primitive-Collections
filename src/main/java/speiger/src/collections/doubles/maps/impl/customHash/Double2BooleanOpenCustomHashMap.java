...
public class Double2BooleanOpenCustomHashMap extends AbstractDouble2BooleanMap implements ITrimmable
{
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(!strategy.equals(keys[i], 0D) && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Boolean.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Double2BooleanOpenCustomHashMap copy() {
		Double2BooleanOpenCustomHashMap map = new Double2BooleanOpenCustomHashMap(0, loadFactor, strategy);
	...
	private final class MapEntrySet extends AbstractObjectSet<Double2BooleanMap.Entry> implements Double2BooleanMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Double2BooleanMap.Entry) {
					...
					int index = Double2BooleanOpenCustomHashMap.this.findIndex(entry.getDoubleKey());
					if(index >= 0) return entry.getBooleanValue() == Double2BooleanOpenCustomHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					if(!(entry.getKey() instanceof Double)) return false;
					int index = Double2BooleanOpenCustomHashMap.this.findIndex((Double)entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Boolean.valueOf(Double2BooleanOpenCustomHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Double2BooleanMap.Entry) {
					Double2BooleanMap.Entry entry = (Double2BooleanMap.Entry)o;
					return Double2BooleanOpenCustomHashMap.this.remove(entry.getDoubleKey(), entry.getBooleanValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Double2BooleanOpenCustomHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractDoubleSet {
		...
		public boolean remove(double o) {
			int oldSize = size;
			Double2BooleanOpenCustomHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Double2BooleanOpenCustomHashMap.this.remove(wrapped.getDouble(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
