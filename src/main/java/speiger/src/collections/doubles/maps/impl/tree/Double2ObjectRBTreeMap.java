...
public class Double2ObjectRBTreeMap<V> extends AbstractDouble2ObjectMap<V> implements Double2ObjectNavigableMap<V>
{
	...
	public Double2ObjectRBTreeMap<V> copy() {
		Double2ObjectRBTreeMap<V> set = new Double2ObjectRBTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<V> extends AbstractDoubleSet implements DoubleNavigableSet
	{
		...
		public DoubleBidirectionalIterator iterator(double fromElement) {
			if(map instanceof Double2ObjectRBTreeMap) return ((Double2ObjectRBTreeMap<V>)map).keyIterator(fromElement);
		...
		public DoubleBidirectionalIterator iterator() {
			if(map instanceof Double2ObjectRBTreeMap) return ((Double2ObjectRBTreeMap<V>)map).keyIterator();
			return ((NavigableSubMap<V>)map).keyIterator();
		}
		
		@Override
		public DoubleBidirectionalIterator descendingIterator() {
			if(map instanceof Double2ObjectRBTreeMap) return ((Double2ObjectRBTreeMap<V>)map).descendingKeyIterator();
			return ((NavigableSubMap<V>)map).descendingKeyIterator();
		}
		
		protected Node<V> start() {
			if(map instanceof Double2ObjectRBTreeMap) return ((Double2ObjectRBTreeMap<V>)map).first;
			return ((NavigableSubMap<V>)map).subLowest();
		}
		
		protected Node<V> end() {
			if(map instanceof Double2ObjectRBTreeMap) return null;
			return ((NavigableSubMap<V>)map).subHighest();
		}
		
		protected Node<V> next(Node<V> entry) {
			if(map instanceof Double2ObjectRBTreeMap) return entry.next();
			return ((NavigableSubMap<V>)map).next(entry);
		}
		
		protected Node<V> previous(Node<V> entry) {
			if(map instanceof Double2ObjectRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Double2ObjectRBTreeMap<V> map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<V> extends NavigableSubMap<V>
	{
		...
		DescendingNaivgableSubMap(Double2ObjectRBTreeMap<V> map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Double2ObjectRBTreeMap<V> map;
		final double lo, hi;
		...
		NavigableSubMap(Double2ObjectRBTreeMap<V> map, boolean fromStart, double lo, boolean loInclusive, boolean toEnd, double hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractObjectCollection<V>
	{
		...
		@Override
		public int size() {
			return Double2ObjectRBTreeMap.this.size;
		}
