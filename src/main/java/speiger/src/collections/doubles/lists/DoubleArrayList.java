 ...
 * 
 * <p>This implementation is optimized to improve how data is processed with interfaces like {@link IDoubleArray}, {@link DoubleStack}
...
public class DoubleArrayList extends AbstractDoubleList implements IDoubleArray, DoubleStack
{
	...
	public DoubleArrayList(Collection<? extends Double> c) {
		this(c.size());
		size = DoubleIterators.unwrap(data, c.iterator());
	}
	 ...
	 */
	public DoubleArrayList(double[] a, int length) {
		this(a, 0, length);
	}
	...
	public boolean addAll(int index, Collection<? extends Double> c) {
		...
		while(add-- != 0) data[index++] = iter.next().doubleValue();
		return true;
	}
	...
	public boolean addAll(int index, DoubleCollection c) {
		...
		while(add-- != 0) data[index++] = iter.nextDouble();
		return true;
	}
	...
	public void sort(DoubleComparator c) {
		if(c != null) DoubleArrays.stableSort(data, size, c);
		else DoubleArrays.stableSort(data, size);
	}
	...
	public void unstableSort(DoubleComparator c) {
		if(c != null) DoubleArrays.unstableSort(data, size, c);
		else DoubleArrays.unstableSort(data, size);		
	}
	...
	public boolean removeAll(Collection<?> c) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!c.contains(Double.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	 ...
	 * This function might delegate to a more appropriate function if necessary
	 * @param c the elements that should be kept. If empty, DoubleArrayList#clear is called.
	 * @return true if the collection was modified
	 * @throws NullPointerException if the collection is null
	 */
	@Override
	...
	public boolean removeIf(Predicate<? super Double> filter) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!filter.test(Double.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	...
	public boolean trim(int size) {
		...
		data = value == 0 ? DoubleArrays.EMPTY_ARRAY : Arrays.copyOf(data, value);
		return true;
	}
	...
	public void clearAndTrim(int size) {
		...
		data = size == 0 ? DoubleArrays.EMPTY_ARRAY : new double[size];
		this.size = size;
	}
	...
	protected void grow(int capacity) {
		...
		data = Arrays.copyOf(data, data == DoubleArrays.EMPTY_ARRAY ? Math.max(DEFAULT_ARRAY_SIZE, capacity) : (int)Math.max(Math.min((long)data.length + (data.length >> 1), SanityChecks.MAX_ARRAY_SIZE), capacity));		
	}
	 ...
	 */
	@Override
	public DoubleStream primitiveStream() { return StreamSupport.doubleStream(DoubleSplititerators.createArrayJavaSplititerator(data, size, 16464), false); }
	 ...
	 */
	@Override
	public DoubleSplititerator spliterator() { return DoubleSplititerators.createArraySplititerator(data, size, 16464); }
}