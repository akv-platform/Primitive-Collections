 ...
 */
public class Short2ByteOpenHashMap extends AbstractShort2ByteMap implements ITrimmable
{
	 ...
	 */
	public Short2ByteOpenHashMap(int minCapacity) {
	 ...
	 */
	public Short2ByteOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Short2ByteOpenHashMap(Short[] keys, Byte[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Short2ByteOpenHashMap(Short[] keys, Byte[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Short2ByteOpenHashMap(Map<? extends Short, ? extends Byte> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2ByteOpenHashMap(Short2ByteMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Short2ByteOpenHashMap(Short2ByteMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (short)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Byte.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Short2ByteOpenHashMap copy() {
		Short2ByteOpenHashMap map = new Short2ByteOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Short2ByteMap.Entry> implements Short2ByteMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Short2ByteMap.Entry) {
					...
					int index = Short2ByteOpenHashMap.this.findIndex(entry.getShortKey());
					if(index >= 0) return entry.getByteValue() == Short2ByteOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Short2ByteOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Byte.valueOf(Short2ByteOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Short2ByteMap.Entry) {
					Short2ByteMap.Entry entry = (Short2ByteMap.Entry)o;
					return Short2ByteOpenHashMap.this.remove(entry.getShortKey(), entry.getByteValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Short2ByteOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractShortSet {
		...
		public boolean remove(short o) {
			int oldSize = size;
			Short2ByteOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Short2ByteOpenHashMap.this.remove(wrapped.getShort(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
