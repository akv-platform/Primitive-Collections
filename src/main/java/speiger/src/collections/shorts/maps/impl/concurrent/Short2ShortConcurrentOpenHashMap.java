 ...
 * 
 */
public class Short2ShortConcurrentOpenHashMap extends AbstractShort2ShortMap implements Short2ShortConcurrentMap
{
	/** Segment Limit */
	...
	protected static class Segment extends StampedLock
	{
		...
		protected short computeIfAbsent(int hash, short key, Short2ShortFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
