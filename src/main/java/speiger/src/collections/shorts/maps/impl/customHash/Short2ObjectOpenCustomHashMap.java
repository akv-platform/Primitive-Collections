 ...
 */
public class Short2ObjectOpenCustomHashMap<V> extends AbstractShort2ObjectMap<V> implements ITrimmable
{
	 ...
	 */
	public Short2ObjectOpenCustomHashMap(ShortStrategy strategy) {
	 ...
	 */
	public Short2ObjectOpenCustomHashMap(int minCapacity, ShortStrategy strategy) {
	 ...
	 */
	public Short2ObjectOpenCustomHashMap(int minCapacity, float loadFactor, ShortStrategy strategy) {
	 ...
	 */
	public Short2ObjectOpenCustomHashMap(Short[] keys, V[] values, ShortStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Short2ObjectOpenCustomHashMap(Short[] keys, V[] values, float loadFactor, ShortStrategy strategy) {
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Short2ObjectOpenCustomHashMap(Map<? extends Short, ? extends V> map, ShortStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Short2ObjectOpenCustomHashMap(Map<? extends Short, ? extends V> map, float loadFactor, ShortStrategy strategy) {
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Short2ObjectOpenCustomHashMap(Short2ObjectMap<V> map, ShortStrategy strategy) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Short2ObjectOpenCustomHashMap(Short2ObjectMap<V> map, float loadFactor, ShortStrategy strategy) {
		this(map.size(), loadFactor, strategy);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(!strategy.equals(keys[i], (short)0) && Objects.equals(value, values[i])) return true;
		return false;
	}
	...
	@Override
	public Short2ObjectOpenCustomHashMap<V> copy() {
		Short2ObjectOpenCustomHashMap<V> map = new Short2ObjectOpenCustomHashMap<>(0, loadFactor, strategy);
	...
	private final class MapEntrySet extends AbstractObjectSet<Short2ObjectMap.Entry<V>> implements Short2ObjectMap.FastEntrySet<V> {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Short2ObjectMap.Entry) {
					...
					int index = Short2ObjectOpenCustomHashMap.this.findIndex(entry.getShortKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short2ObjectOpenCustomHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					if(!(entry.getKey() instanceof Short)) return false;
					int index = Short2ObjectOpenCustomHashMap.this.findIndex((Short)entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short2ObjectOpenCustomHashMap.this.values[index]);
				}
			}
			return false;
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Short2ObjectMap.Entry) {
					Short2ObjectMap.Entry<V> entry = (Short2ObjectMap.Entry<V>)o;
					return Short2ObjectOpenCustomHashMap.this.remove(entry.getShortKey(), entry.getValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Short2ObjectOpenCustomHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractShortSet {
		...
		public boolean remove(short o) {
			int oldSize = size;
			Short2ObjectOpenCustomHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public boolean matchesAny(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (short)0) && filter.getBoolean(values[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (short)0) && filter.getBoolean(values[i])) return false;
			}
			return true;
		}
		...
		public V findFirst(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (short)0) && filter.getBoolean(values[i])) return values[i];
			}
			return null;
		}
		...
		public int count(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (short)0) && filter.getBoolean(values[i])) result++;
			}
			return result;
		}
	}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Short2ObjectOpenCustomHashMap.this.remove(wrapped.getShort(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
