 ...
 */
public class Short2IntAVLTreeMap extends AbstractShort2IntMap implements Short2IntNavigableMap
{
	 ...
	 */
	public Short2IntAVLTreeMap(ShortComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Short2IntAVLTreeMap(Short[] keys, Integer[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Short2IntAVLTreeMap(Short[] keys, Integer[] values, ShortComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Short2IntAVLTreeMap(short[] keys, int[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Short2IntAVLTreeMap(short[] keys, int[] values, ShortComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Short2IntAVLTreeMap(Map<? extends Short, ? extends Integer> map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2IntAVLTreeMap(Short2IntMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Short2IntAVLTreeMap(Short2IntMap map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Short2IntAVLTreeMap copy() {
		Short2IntAVLTreeMap set = new Short2IntAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractShortSet implements ShortNavigableSet
	{
		...
		public ShortBidirectionalIterator iterator(short fromElement) {
			if(map instanceof Short2IntAVLTreeMap) return ((Short2IntAVLTreeMap)map).keyIterator(fromElement);
		...
		public ShortBidirectionalIterator iterator() {
			if(map instanceof Short2IntAVLTreeMap) return ((Short2IntAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ShortBidirectionalIterator descendingIterator() {
			if(map instanceof Short2IntAVLTreeMap) return ((Short2IntAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Short2IntAVLTreeMap) return ((Short2IntAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Short2IntAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Short2IntAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Short2IntAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Short2IntAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Short2IntAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractShort2IntMap implements Short2IntNavigableMap
	{
		...
		NavigableSubMap(Short2IntAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractIntCollection
	{
		...
		@Override
		public int size() {
			return Short2IntAVLTreeMap.this.size;
		}
