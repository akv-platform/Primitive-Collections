 ...
 */
public class Short2FloatAVLTreeMap extends AbstractShort2FloatMap implements Short2FloatNavigableMap
{
	 ...
	 */
	public Short2FloatAVLTreeMap(ShortComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Short2FloatAVLTreeMap(Short[] keys, Float[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Short2FloatAVLTreeMap(Short[] keys, Float[] values, ShortComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Short2FloatAVLTreeMap(Map<? extends Short, ? extends Float> map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2FloatAVLTreeMap(Short2FloatMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Short2FloatAVLTreeMap(Short2FloatMap map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Short2FloatAVLTreeMap copy() {
		Short2FloatAVLTreeMap set = new Short2FloatAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractShortSet implements ShortNavigableSet
	{
		...
		public ShortBidirectionalIterator iterator(short fromElement) {
			if(map instanceof Short2FloatAVLTreeMap) return ((Short2FloatAVLTreeMap)map).keyIterator(fromElement);
		...
		public ShortBidirectionalIterator iterator() {
			if(map instanceof Short2FloatAVLTreeMap) return ((Short2FloatAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ShortBidirectionalIterator descendingIterator() {
			if(map instanceof Short2FloatAVLTreeMap) return ((Short2FloatAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Short2FloatAVLTreeMap) return ((Short2FloatAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Short2FloatAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Short2FloatAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Short2FloatAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Short2FloatAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Short2FloatAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Short2FloatAVLTreeMap map;
		final short lo, hi;
		...
		NavigableSubMap(Short2FloatAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractFloatCollection
	{
		...
		@Override
		public int size() {
			return Short2FloatAVLTreeMap.this.size;
		}
