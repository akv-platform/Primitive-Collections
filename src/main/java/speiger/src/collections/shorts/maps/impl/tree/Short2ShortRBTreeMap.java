 ...
 */
public class Short2ShortRBTreeMap extends AbstractShort2ShortMap implements Short2ShortNavigableMap
{
	 ...
	 */
	public Short2ShortRBTreeMap(ShortComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Short2ShortRBTreeMap(Short[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Short2ShortRBTreeMap(Short[] keys, Short[] values, ShortComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Short2ShortRBTreeMap(Map<? extends Short, ? extends Short> map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2ShortRBTreeMap(Short2ShortMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Short2ShortRBTreeMap(Short2ShortMap map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Short2ShortRBTreeMap copy() {
		Short2ShortRBTreeMap set = new Short2ShortRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractShortSet implements ShortNavigableSet
	{
		...
		public ShortBidirectionalIterator iterator(short fromElement) {
			if(map instanceof Short2ShortRBTreeMap) return ((Short2ShortRBTreeMap)map).keyIterator(fromElement);
		...
		public ShortBidirectionalIterator iterator() {
			if(map instanceof Short2ShortRBTreeMap) return ((Short2ShortRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ShortBidirectionalIterator descendingIterator() {
			if(map instanceof Short2ShortRBTreeMap) return ((Short2ShortRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Short2ShortRBTreeMap) return ((Short2ShortRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Short2ShortRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Short2ShortRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Short2ShortRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Short2ShortRBTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Short2ShortRBTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Short2ShortRBTreeMap map;
		final short lo, hi;
		...
		NavigableSubMap(Short2ShortRBTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractShortCollection
	{
		...
		@Override
		public int size() {
			return Short2ShortRBTreeMap.this.size;
		}
