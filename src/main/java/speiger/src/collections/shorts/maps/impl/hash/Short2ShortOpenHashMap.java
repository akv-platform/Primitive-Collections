 ...
 */
public class Short2ShortOpenHashMap extends AbstractShort2ShortMap implements ITrimmable
{
	 ...
	 */
	public Short2ShortOpenHashMap(int minCapacity) {
	 ...
	 */
	public Short2ShortOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Short2ShortOpenHashMap(Short[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Short2ShortOpenHashMap(Short[] keys, Short[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Short2ShortOpenHashMap(Map<? extends Short, ? extends Short> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2ShortOpenHashMap(Short2ShortMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Short2ShortOpenHashMap(Short2ShortMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (short)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Short.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Short2ShortOpenHashMap copy() {
		Short2ShortOpenHashMap map = new Short2ShortOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Short2ShortMap.Entry> implements Short2ShortMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Short2ShortMap.Entry) {
					...
					int index = Short2ShortOpenHashMap.this.findIndex(entry.getShortKey());
					if(index >= 0) return entry.getShortValue() == Short2ShortOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Short2ShortOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short.valueOf(Short2ShortOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Short2ShortMap.Entry) {
					Short2ShortMap.Entry entry = (Short2ShortMap.Entry)o;
					return Short2ShortOpenHashMap.this.remove(entry.getShortKey(), entry.getShortValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Short2ShortOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractShortSet {
		...
		public boolean remove(short o) {
			int oldSize = size;
			Short2ShortOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Short2ShortOpenHashMap.this.remove(wrapped.getShort(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
