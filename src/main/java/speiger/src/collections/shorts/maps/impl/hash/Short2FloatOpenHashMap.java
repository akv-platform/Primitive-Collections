 ...
 */
public class Short2FloatOpenHashMap extends AbstractShort2FloatMap implements ITrimmable
{
	 ...
	 */
	public Short2FloatOpenHashMap(int minCapacity) {
	 ...
	 */
	public Short2FloatOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Short2FloatOpenHashMap(Short[] keys, Float[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Short2FloatOpenHashMap(Short[] keys, Float[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Short2FloatOpenHashMap(Map<? extends Short, ? extends Float> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2FloatOpenHashMap(Short2FloatMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Short2FloatOpenHashMap(Short2FloatMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(float value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (short)0 && Float.floatToIntBits(values[i]) == Float.floatToIntBits(value)) return true;
		return false;
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (short)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Float.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Short2FloatOpenHashMap copy() {
		Short2FloatOpenHashMap map = new Short2FloatOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Short2FloatMap.Entry> implements Short2FloatMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Short2FloatMap.Entry) {
					...
					int index = Short2FloatOpenHashMap.this.findIndex(entry.getShortKey());
					if(index >= 0) return Float.floatToIntBits(entry.getFloatValue()) == Float.floatToIntBits(Short2FloatOpenHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Short2FloatOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Float.valueOf(Short2FloatOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Short2FloatMap.Entry) {
					Short2FloatMap.Entry entry = (Short2FloatMap.Entry)o;
					return Short2FloatOpenHashMap.this.remove(entry.getShortKey(), entry.getFloatValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Short2FloatOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractShortSet {
		...
		public boolean remove(short o) {
			int oldSize = size;
			Short2FloatOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Short2FloatOpenHashMap.this.remove(wrapped.getShort(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
