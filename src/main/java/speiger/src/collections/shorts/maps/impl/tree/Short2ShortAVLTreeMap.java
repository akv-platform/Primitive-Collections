 ...
 */
public class Short2ShortAVLTreeMap extends AbstractShort2ShortMap implements Short2ShortNavigableMap
{
	...
	/** EntrySet Cache */
	protected ObjectSet<Short2ShortMap.Entry> entrySet;
	 ...
	 */
	public Short2ShortAVLTreeMap(ShortComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Short2ShortAVLTreeMap(Short[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Short2ShortAVLTreeMap(Short[] keys, Short[] values, ShortComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Short2ShortAVLTreeMap(Map<? extends Short, ? extends Short> map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Short2ShortAVLTreeMap(Short2ShortMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Short2ShortAVLTreeMap(Short2ShortMap map, ShortComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	public void mergeAllShort(Short2ShortMap m, ShortShortUnaryOperator mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		for(Short2ShortMap.Entry entry : Short2ShortMaps.fastIterable(m)) {
	...
	@Override
	public Short2ShortAVLTreeMap copy() {
		Short2ShortAVLTreeMap set = new Short2ShortAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractShortSet implements ShortNavigableSet
	{
		...
		public KeySet(Short2ShortNavigableMap map) {
			this.map = map;
		}
		...
		public Short lower(Short e) {
			Short2ShortMap.Entry node = map.lowerEntry(e.shortValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Short floor(Short e) {
			Short2ShortMap.Entry node = map.floorEntry(e.shortValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Short higher(Short e) {
			Short2ShortMap.Entry node = map.higherEntry(e.shortValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Short ceiling(Short e) {
			Short2ShortMap.Entry node = map.ceilingEntry(e.shortValue());
			return node != null ? node.getKey() : null;
		}
		...
		public ShortBidirectionalIterator iterator(short fromElement) {
			if(map instanceof Short2ShortAVLTreeMap) return ((Short2ShortAVLTreeMap)map).keyIterator(fromElement);
		...
		public ShortBidirectionalIterator iterator() {
			if(map instanceof Short2ShortAVLTreeMap) return ((Short2ShortAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ShortBidirectionalIterator descendingIterator() {
			if(map instanceof Short2ShortAVLTreeMap) return ((Short2ShortAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Short2ShortAVLTreeMap) return ((Short2ShortAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Short2ShortAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Short2ShortAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Short2ShortAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Short2ShortAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Short2ShortAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractShort2ShortMap implements Short2ShortNavigableMap
	{
		final Short2ShortAVLTreeMap map;
		final short lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;
		
		Short2ShortNavigableMap inverse;
		ShortNavigableSet keySet;
		ObjectSet<Short2ShortMap.Entry> entrySet;
		ShortCollection values;
		
		NavigableSubMap(Short2ShortAVLTreeMap map, boolean fromStart, short lo, boolean loInclusive, boolean toEnd, short hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public Short2ShortMap.Entry lowerEntry(short key) { return subLower(key); }
		@Override
		public Short2ShortMap.Entry floorEntry(short key) { return subFloor(key); }
		@Override
		public Short2ShortMap.Entry ceilingEntry(short key) { return subCeiling(key); }
		@Override
		public Short2ShortMap.Entry higherEntry(short key) { return subHigher(key); }
		...
		public Short2ShortMap.Entry pollFirstEntry() {
			Node entry = subLowest();
			if(entry != null) {
				Short2ShortMap.Entry result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Short2ShortMap.Entry> {
			@Override
			public int size() {
				if (fromStart && toEnd) return map.size();
				int size = 0;
				for(ObjectIterator<Short2ShortMap.Entry> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
	...
	class Values extends AbstractShortCollection
	{
		...
		@Override
		public int size() {
			return Short2ShortAVLTreeMap.this.size;
		}
	...
	class EntrySet extends AbstractObjectSet<Short2ShortMap.Entry> {
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Short2ShortMap.Entry>
	{
	...
	private static final class Node implements Short2ShortMap.Entry
	{
		short key;
		short value;
		int state;
		...
		Node(short key, short value, Node parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherKey instanceof Short && otherValue instanceof Short && key == ((Short)otherKey).shortValue() && value == ((Short)otherValue).shortValue();
			}
			return false;
		}
