 ...
 * 
 * <p>This implementation is optimized to improve how data is processed with interfaces like {@link IShortArray}, {@link ShortStack}
...
public class ShortArrayList extends AbstractShortList implements IShortArray, ShortStack
{
	...
	public ShortArrayList(Collection<? extends Short> c) {
		this(c.size());
		size = ShortIterators.unwrap(data, c.iterator());
	}
	 ...
	 */
	public ShortArrayList(short[] a, int length) {
		this(a, 0, length);
	}
	...
	public boolean addAll(int index, Collection<? extends Short> c) {
		...
		while(add-- != 0) data[index++] = iter.next().shortValue();
		return true;
	}
	...
	public boolean addAll(int index, ShortCollection c) {
		...
		while(add-- != 0) data[index++] = iter.nextShort();
		return true;
	}
	...
	public void sort(ShortComparator c) {
		if(c != null) ShortArrays.stableSort(data, size, c);
		else ShortArrays.stableSort(data, size);
	}
	...
	public void unstableSort(ShortComparator c) {
		if(c != null) ShortArrays.unstableSort(data, size, c);
		else ShortArrays.unstableSort(data, size);		
	}
	...
	public void replaceShorts(IntUnaryOperator o) {
		Objects.requireNonNull(o);
		for(int i = 0;i<size;i++)
			data[i] = SanityChecks.castToShort(o.applyAsInt(data[i]));
	}
	...
	public boolean removeAll(Collection<?> c) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!c.contains(Short.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	 ...
	 * This function might delegate to a more appropriate function if necessary
	 * @param c the elements that should be kept. If empty, ShortArrayList#clear is called.
	 * @return true if the collection was modified
	 * @throws NullPointerException if the collection is null
	 */
	@Override
	...
	public boolean removeIf(Predicate<? super Short> filter) {
		...
		int j = 0;
		for(int i = 0;i<size;i++) {
			if(!filter.test(Short.valueOf(data[i]))) data[j++] = data[i];
			else modified = true;
		}
		size = j;
		return modified;
	...
	public void size(int size) {
		if(size > data.length)
			...
			Arrays.fill(data, size, size(), (short)0);
		this.size = size;
	}
	...
	public boolean trim(int size) {
		...
		data = value == 0 ? ShortArrays.EMPTY_ARRAY : Arrays.copyOf(data, value);
		return true;
	}
	...
	public void clearAndTrim(int size) {
		...
		data = size == 0 ? ShortArrays.EMPTY_ARRAY : new short[size];
		this.size = size;
	}
	...
	protected void grow(int capacity) {
		...
		data = Arrays.copyOf(data, data == ShortArrays.EMPTY_ARRAY ? Math.max(DEFAULT_ARRAY_SIZE, capacity) : (int)Math.max(Math.min((long)data.length + (data.length >> 1), SanityChecks.MAX_ARRAY_SIZE), capacity));		
	}
	 ...
	 */
	@Override
	public IntStream primitiveStream() { return StreamSupport.intStream(ShortSplititerators.createArrayJavaSplititerator(data, size, 16464), false); }
	 ...
	 */
	@Override
	public ShortSplititerator spliterator() { return ShortSplititerators.createArraySplititerator(data, size, 16464); }
}