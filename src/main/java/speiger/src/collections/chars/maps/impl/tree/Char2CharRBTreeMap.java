 ...
 */
public class Char2CharRBTreeMap extends AbstractChar2CharMap implements Char2CharNavigableMap
{
	 ...
	 */
	public Char2CharRBTreeMap(CharComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Char2CharRBTreeMap(Character[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2CharRBTreeMap(Character[] keys, Character[] values, CharComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2CharRBTreeMap(char[] keys, char[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2CharRBTreeMap(char[] keys, char[] values, CharComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Char2CharRBTreeMap(Map<? extends Character, ? extends Character> map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2CharRBTreeMap(Char2CharMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Char2CharRBTreeMap(Char2CharMap map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Char2CharRBTreeMap copy() {
		Char2CharRBTreeMap set = new Char2CharRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractCharSet implements CharNavigableSet
	{
		...
		public CharBidirectionalIterator iterator(char fromElement) {
			if(map instanceof Char2CharRBTreeMap) return ((Char2CharRBTreeMap)map).keyIterator(fromElement);
		...
		public CharBidirectionalIterator iterator() {
			if(map instanceof Char2CharRBTreeMap) return ((Char2CharRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public CharBidirectionalIterator descendingIterator() {
			if(map instanceof Char2CharRBTreeMap) return ((Char2CharRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Char2CharRBTreeMap) return ((Char2CharRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Char2CharRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Char2CharRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Char2CharRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Char2CharRBTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Char2CharRBTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractChar2CharMap implements Char2CharNavigableMap
	{
		...
		NavigableSubMap(Char2CharRBTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractCharCollection
	{
		...
		@Override
		public int size() {
			return Char2CharRBTreeMap.this.size;
		}
