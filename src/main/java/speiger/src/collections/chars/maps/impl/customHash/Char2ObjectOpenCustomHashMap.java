 ...
 */
public class Char2ObjectOpenCustomHashMap<V> extends AbstractChar2ObjectMap<V> implements ITrimmable
{
	 ...
	 */
	public Char2ObjectOpenCustomHashMap(CharStrategy strategy) {
	 ...
	 */
	public Char2ObjectOpenCustomHashMap(int minCapacity, CharStrategy strategy) {
	 ...
	 */
	public Char2ObjectOpenCustomHashMap(int minCapacity, float loadFactor, CharStrategy strategy) {
	 ...
	 */
	public Char2ObjectOpenCustomHashMap(Character[] keys, V[] values, CharStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2ObjectOpenCustomHashMap(Character[] keys, V[] values, float loadFactor, CharStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2ObjectOpenCustomHashMap(char[] keys, V[] values, CharStrategy strategy) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2ObjectOpenCustomHashMap(char[] keys, V[] values, float loadFactor, CharStrategy strategy) {
		this(keys.length, loadFactor, strategy);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Char2ObjectOpenCustomHashMap(Map<? extends Character, ? extends V> map, CharStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2ObjectOpenCustomHashMap(Map<? extends Character, ? extends V> map, float loadFactor, CharStrategy strategy) {
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Char2ObjectOpenCustomHashMap(Char2ObjectMap<V> map, CharStrategy strategy) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Char2ObjectOpenCustomHashMap(Char2ObjectMap<V> map, float loadFactor, CharStrategy strategy) {
		this(map.size(), loadFactor, strategy);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(!strategy.equals(keys[i], (char)0) && Objects.equals(value, values[i])) return true;
		return false;
	}
	...
	@Override
	public Char2ObjectOpenCustomHashMap<V> copy() {
		Char2ObjectOpenCustomHashMap<V> map = new Char2ObjectOpenCustomHashMap<>(0, loadFactor, strategy);
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2ObjectMap.Entry<V>> implements Char2ObjectMap.FastEntrySet<V> {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2ObjectMap.Entry) {
					...
					int index = Char2ObjectOpenCustomHashMap.this.findIndex(entry.getCharKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Char2ObjectOpenCustomHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					if(!(entry.getKey() instanceof Character)) return false;
					int index = Char2ObjectOpenCustomHashMap.this.findIndex((Character)entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Char2ObjectOpenCustomHashMap.this.values[index]);
				}
			}
			return false;
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2ObjectMap.Entry) {
					Char2ObjectMap.Entry<V> entry = (Char2ObjectMap.Entry<V>)o;
					return Char2ObjectOpenCustomHashMap.this.remove(entry.getCharKey(), entry.getValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Char2ObjectOpenCustomHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractCharSet {
		...
		public boolean remove(char o) {
			int oldSize = size;
			Char2ObjectOpenCustomHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public boolean matchesAny(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (char)0) && filter.getBoolean(values[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (char)0) && filter.getBoolean(values[i])) return false;
			}
			return true;
		}
		...
		public V findFirst(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (char)0) && filter.getBoolean(values[i])) return values[i];
			}
			return null;
		}
		...
		public int count(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(!strategy.equals(keys[i], (char)0) && filter.getBoolean(values[i])) result++;
			}
			return result;
		}
	}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2ObjectOpenCustomHashMap.this.remove(wrapped.getChar(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
