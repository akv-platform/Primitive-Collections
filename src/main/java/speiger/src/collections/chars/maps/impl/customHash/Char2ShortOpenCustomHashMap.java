 ...
 */
public class Char2ShortOpenCustomHashMap extends AbstractChar2ShortMap implements ITrimmable
{
	 ...
	 */
	public Char2ShortOpenCustomHashMap(CharStrategy strategy) {
	 ...
	 */
	public Char2ShortOpenCustomHashMap(int minCapacity, CharStrategy strategy) {
	 ...
	 */
	public Char2ShortOpenCustomHashMap(int minCapacity, float loadFactor, CharStrategy strategy) {
	 ...
	 */
	public Char2ShortOpenCustomHashMap(Character[] keys, Short[] values, CharStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2ShortOpenCustomHashMap(Character[] keys, Short[] values, float loadFactor, CharStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2ShortOpenCustomHashMap(char[] keys, short[] values, CharStrategy strategy) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2ShortOpenCustomHashMap(char[] keys, short[] values, float loadFactor, CharStrategy strategy) {
		this(keys.length, loadFactor, strategy);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Char2ShortOpenCustomHashMap(Map<? extends Character, ? extends Short> map, CharStrategy strategy) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2ShortOpenCustomHashMap(Map<? extends Character, ? extends Short> map, float loadFactor, CharStrategy strategy) {
	 ...
	 * @throws NullPointerException if Strategy is null
	 */
	public Char2ShortOpenCustomHashMap(Char2ShortMap map, CharStrategy strategy) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Char2ShortOpenCustomHashMap(Char2ShortMap map, float loadFactor, CharStrategy strategy) {
		this(map.size(), loadFactor, strategy);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(!strategy.equals(keys[i], (char)0) && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Short.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Char2ShortOpenCustomHashMap copy() {
		Char2ShortOpenCustomHashMap map = new Char2ShortOpenCustomHashMap(0, loadFactor, strategy);
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2ShortMap.Entry> implements Char2ShortMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2ShortMap.Entry) {
					...
					int index = Char2ShortOpenCustomHashMap.this.findIndex(entry.getCharKey());
					if(index >= 0) return entry.getShortValue() == Char2ShortOpenCustomHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					if(!(entry.getKey() instanceof Character)) return false;
					int index = Char2ShortOpenCustomHashMap.this.findIndex((Character)entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short.valueOf(Char2ShortOpenCustomHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2ShortMap.Entry) {
					Char2ShortMap.Entry entry = (Char2ShortMap.Entry)o;
					return Char2ShortOpenCustomHashMap.this.remove(entry.getCharKey(), entry.getShortValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Char2ShortOpenCustomHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractCharSet {
		...
		public boolean remove(char o) {
			int oldSize = size;
			Char2ShortOpenCustomHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2ShortOpenCustomHashMap.this.remove(wrapped.getChar(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
