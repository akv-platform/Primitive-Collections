 ...
 */
public class Char2CharOpenHashMap extends AbstractChar2CharMap implements ITrimmable
{
	 ...
	 */
	public Char2CharOpenHashMap(int minCapacity) {
	 ...
	 */
	public Char2CharOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Char2CharOpenHashMap(Character[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2CharOpenHashMap(Character[] keys, Character[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2CharOpenHashMap(char[] keys, char[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2CharOpenHashMap(char[] keys, char[] values, float loadFactor) {
		this(keys.length, loadFactor);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2CharOpenHashMap(Map<? extends Character, ? extends Character> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2CharOpenHashMap(Char2CharMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Char2CharOpenHashMap(Char2CharMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (char)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Character.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Char2CharOpenHashMap copy() {
		Char2CharOpenHashMap map = new Char2CharOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2CharMap.Entry> implements Char2CharMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2CharMap.Entry) {
					...
					int index = Char2CharOpenHashMap.this.findIndex(entry.getCharKey());
					if(index >= 0) return entry.getCharValue() == Char2CharOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Char2CharOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Character.valueOf(Char2CharOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2CharMap.Entry) {
					Char2CharMap.Entry entry = (Char2CharMap.Entry)o;
					return Char2CharOpenHashMap.this.remove(entry.getCharKey(), entry.getCharValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Char2CharOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractCharSet {
		...
		public boolean remove(char o) {
			int oldSize = size;
			Char2CharOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2CharOpenHashMap.this.remove(wrapped.getChar(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
