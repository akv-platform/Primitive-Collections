 ...
 */
public class Char2BooleanOpenHashMap extends AbstractChar2BooleanMap implements ITrimmable
{
	 ...
	 */
	public Char2BooleanOpenHashMap(int minCapacity) {
	 ...
	 */
	public Char2BooleanOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Char2BooleanOpenHashMap(Character[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2BooleanOpenHashMap(Character[] keys, Boolean[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2BooleanOpenHashMap(char[] keys, boolean[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2BooleanOpenHashMap(char[] keys, boolean[] values, float loadFactor) {
		this(keys.length, loadFactor);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2BooleanOpenHashMap(Map<? extends Character, ? extends Boolean> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2BooleanOpenHashMap(Char2BooleanMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Char2BooleanOpenHashMap(Char2BooleanMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (char)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Boolean.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Char2BooleanOpenHashMap copy() {
		Char2BooleanOpenHashMap map = new Char2BooleanOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2BooleanMap.Entry> implements Char2BooleanMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2BooleanMap.Entry) {
					...
					int index = Char2BooleanOpenHashMap.this.findIndex(entry.getCharKey());
					if(index >= 0) return entry.getBooleanValue() == Char2BooleanOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Char2BooleanOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Boolean.valueOf(Char2BooleanOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2BooleanMap.Entry) {
					Char2BooleanMap.Entry entry = (Char2BooleanMap.Entry)o;
					return Char2BooleanOpenHashMap.this.remove(entry.getCharKey(), entry.getBooleanValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Char2BooleanOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractCharSet {
		...
		public boolean remove(char o) {
			int oldSize = size;
			Char2BooleanOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2BooleanOpenHashMap.this.remove(wrapped.getChar(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
