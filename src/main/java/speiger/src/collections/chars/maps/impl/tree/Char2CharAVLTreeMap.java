 ...
 */
public class Char2CharAVLTreeMap extends AbstractChar2CharMap implements Char2CharNavigableMap
{
	...
	/** EntrySet Cache */
	protected ObjectSet<Char2CharMap.Entry> entrySet;
	 ...
	 */
	public Char2CharAVLTreeMap(CharComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Char2CharAVLTreeMap(Character[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2CharAVLTreeMap(Character[] keys, Character[] values, CharComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2CharAVLTreeMap(char[] keys, char[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2CharAVLTreeMap(char[] keys, char[] values, CharComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Char2CharAVLTreeMap(Map<? extends Character, ? extends Character> map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2CharAVLTreeMap(Char2CharMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Char2CharAVLTreeMap(Char2CharMap map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public char computeCharIfAbsent(char key, Char2CharFunction mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		Node entry = findNode(key);
		if(entry == null) {
	...
	public void mergeAllChar(Char2CharMap m, CharCharUnaryOperator mappingFunction) {
		Objects.requireNonNull(mappingFunction);
		for(Char2CharMap.Entry entry : Char2CharMaps.fastIterable(m)) {
	...
	@Override
	public Char2CharAVLTreeMap copy() {
		Char2CharAVLTreeMap set = new Char2CharAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	@Override
	public Char2CharNavigableMap descendingMap() {
	...
	@Override
	public Char2CharNavigableMap subMap(char fromKey, boolean fromInclusive, char toKey, boolean toInclusive) {
		return new AscendingNaivgableSubMap(this, false, fromKey, fromInclusive, false, toKey, toInclusive);
	}
	
	@Override
	public Char2CharNavigableMap headMap(char toKey, boolean inclusive) {
		return new AscendingNaivgableSubMap(this, true, (char)0, true, false, toKey, inclusive);
	}
	
	@Override
	public Char2CharNavigableMap tailMap(char fromKey, boolean inclusive) {
	...
	@Override
	public Char2CharMap.Entry lowerEntry(char key) {
		Node node = findLowerNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Char2CharMap.Entry higherEntry(char key) {
		Node node = findHigherNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Char2CharMap.Entry floorEntry(char key) {
		Node node = findFloorNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Char2CharMap.Entry ceilingEntry(char key) {
	...
	static class KeySet extends AbstractCharSet implements CharNavigableSet
	{
		...
		public KeySet(Char2CharNavigableMap map) {
			this.map = map;
		}
		...
		public CharBidirectionalIterator iterator(char fromElement) {
			if(map instanceof Char2CharAVLTreeMap) return ((Char2CharAVLTreeMap)map).keyIterator(fromElement);
		...
		public CharBidirectionalIterator iterator() {
			if(map instanceof Char2CharAVLTreeMap) return ((Char2CharAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public CharBidirectionalIterator descendingIterator() {
			if(map instanceof Char2CharAVLTreeMap) return ((Char2CharAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Char2CharAVLTreeMap) return ((Char2CharAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Char2CharAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Char2CharAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Char2CharAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Char2CharAVLTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Char2CharAVLTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractChar2CharMap implements Char2CharNavigableMap
	{
		final Char2CharAVLTreeMap map;
		final char lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;
		
		Char2CharNavigableMap inverse;
		CharNavigableSet keySet;
		ObjectSet<Char2CharMap.Entry> entrySet;
		CharCollection values;
		
		NavigableSubMap(Char2CharAVLTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public Char2CharMap.Entry lowerEntry(char key) { return subLower(key); }
		@Override
		public Char2CharMap.Entry floorEntry(char key) { return subFloor(key); }
		@Override
		public Char2CharMap.Entry ceilingEntry(char key) { return subCeiling(key); }
		@Override
		public Char2CharMap.Entry higherEntry(char key) { return subHigher(key); }
		...
		@Override
		public Char2CharNavigableMap copy() { throw new UnsupportedOperationException(); }
		...
		public Char2CharMap.Entry pollFirstEntry() {
			Node entry = subLowest();
			if(entry != null) {
				Char2CharMap.Entry result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Char2CharMap.Entry> {
			@Override
			public int size() {
				if (fromStart && toEnd) return map.size();
				int size = 0;
				for(ObjectIterator<Char2CharMap.Entry> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
	...
	class Values extends AbstractCharCollection
	{
		...
		@Override
		public int size() {
			return Char2CharAVLTreeMap.this.size;
		}
	...
	class EntrySet extends AbstractObjectSet<Char2CharMap.Entry> {
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Char2CharMap.Entry>
	{
	...
	private static final class Node implements Char2CharMap.Entry
	{
		char key;
		char value;
		int state;
		...
		Node(char key, char value, Node parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherKey instanceof Character && otherValue instanceof Character && key == ((Character)otherKey).charValue() && value == ((Character)otherValue).charValue();
			}
			return false;
		}
