 ...
 */
public class Char2LongRBTreeMap extends AbstractChar2LongMap implements Char2LongNavigableMap
{
	...
	/** EntrySet Cache */
	protected ObjectSet<Char2LongMap.Entry> entrySet;
	 ...
	 */
	public Char2LongRBTreeMap(CharComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Char2LongRBTreeMap(Character[] keys, Long[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2LongRBTreeMap(Character[] keys, Long[] values, CharComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2LongRBTreeMap(char[] keys, long[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2LongRBTreeMap(char[] keys, long[] values, CharComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Char2LongRBTreeMap(Map<? extends Character, ? extends Long> map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2LongRBTreeMap(Char2LongMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Char2LongRBTreeMap(Char2LongMap map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Char2LongRBTreeMap copy() {
		Char2LongRBTreeMap set = new Char2LongRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractCharSet implements CharNavigableSet
	{
		...
		public KeySet(Char2LongNavigableMap map) {
			this.map = map;
		}
		...
		public Character lower(Character e) {
			Char2LongMap.Entry node = map.lowerEntry(e.charValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Character floor(Character e) {
			Char2LongMap.Entry node = map.floorEntry(e.charValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Character higher(Character e) {
			Char2LongMap.Entry node = map.higherEntry(e.charValue());
			return node != null ? node.getKey() : null;
		}
		
		@Override
		public Character ceiling(Character e) {
			Char2LongMap.Entry node = map.ceilingEntry(e.charValue());
			return node != null ? node.getKey() : null;
		}
		...
		public CharBidirectionalIterator iterator(char fromElement) {
			if(map instanceof Char2LongRBTreeMap) return ((Char2LongRBTreeMap)map).keyIterator(fromElement);
		...
		public CharBidirectionalIterator iterator() {
			if(map instanceof Char2LongRBTreeMap) return ((Char2LongRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public CharBidirectionalIterator descendingIterator() {
			if(map instanceof Char2LongRBTreeMap) return ((Char2LongRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Char2LongRBTreeMap) return ((Char2LongRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Char2LongRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Char2LongRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Char2LongRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Char2LongRBTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Char2LongRBTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractChar2LongMap implements Char2LongNavigableMap
	{
		final Char2LongRBTreeMap map;
		final char lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;
		
		Char2LongNavigableMap inverse;
		CharNavigableSet keySet;
		ObjectSet<Char2LongMap.Entry> entrySet;
		LongCollection values;
		
		NavigableSubMap(Char2LongRBTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public Char2LongMap.Entry lowerEntry(char key) { return subLower(key); }
		@Override
		public Char2LongMap.Entry floorEntry(char key) { return subFloor(key); }
		@Override
		public Char2LongMap.Entry ceilingEntry(char key) { return subCeiling(key); }
		@Override
		public Char2LongMap.Entry higherEntry(char key) { return subHigher(key); }
		...
		public Char2LongMap.Entry pollFirstEntry() {
			Node entry = subLowest();
			if(entry != null) {
				Char2LongMap.Entry result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Char2LongMap.Entry> {
			@Override
			public int size() {
				if (fromStart && toEnd) return map.size();
				int size = 0;
				for(ObjectIterator<Char2LongMap.Entry> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
		...
		class DecsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Char2LongMap.Entry>
		{
		...
		class AcsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Char2LongMap.Entry>
		{
	...
	class Values extends AbstractLongCollection
	{
		...
		@Override
		public int size() {
			return Char2LongRBTreeMap.this.size;
		}
	...
	class EntrySet extends AbstractObjectSet<Char2LongMap.Entry> {
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Char2LongMap.Entry>
	{
	...
	private static final class Node implements Char2LongMap.Entry
	{
		...
		Node(char key, long value, Node parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherKey instanceof Character && otherValue instanceof Long && key == ((Character)otherKey).charValue() && value == ((Long)otherValue).longValue();
			}
			return false;
		}
