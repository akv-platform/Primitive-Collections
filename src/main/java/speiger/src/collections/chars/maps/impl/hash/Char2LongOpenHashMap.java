...
public class Char2LongOpenHashMap extends AbstractChar2LongMap implements ITrimmable
{
	 ...
	 */
	public Char2LongOpenHashMap(int minCapacity) {
	 ...
	 */
	public Char2LongOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Char2LongOpenHashMap(Character[] keys, Long[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2LongOpenHashMap(Character[] keys, Long[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2LongOpenHashMap(char[] keys, long[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2LongOpenHashMap(char[] keys, long[] values, float loadFactor) {
		this(keys.length, loadFactor);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2LongOpenHashMap(Map<? extends Character, ? extends Long> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2LongOpenHashMap(Char2LongMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Char2LongOpenHashMap(Char2LongMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (char)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Long.valueOf(values[i])))) return true;
		return false;
	}
	...
	public Char2LongOpenHashMap copy() {
		Char2LongOpenHashMap map = new Char2LongOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2LongMap.Entry> implements Char2LongMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2LongMap.Entry) {
					...
					int index = Char2LongOpenHashMap.this.findIndex(entry.getCharKey());
					if(index >= 0) return entry.getLongValue() == Char2LongOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Char2LongOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Long.valueOf(Char2LongOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				...
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Char2LongOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractCharSet {
		...
		public boolean remove(char o) {
			int oldSize = size;
			Char2LongOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2LongOpenHashMap.this.remove(wrapped.getChar(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
