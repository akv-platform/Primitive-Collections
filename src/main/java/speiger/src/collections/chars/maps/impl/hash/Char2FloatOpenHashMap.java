...
public class Char2FloatOpenHashMap extends AbstractChar2FloatMap implements ITrimmable
{
	 ...
	 */
	public Char2FloatOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Char2FloatOpenHashMap(Character[] keys, Float[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2FloatOpenHashMap(Character[] keys, Float[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2FloatOpenHashMap(char[] keys, float[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2FloatOpenHashMap(char[] keys, float[] values, float loadFactor) {
		this(keys.length, loadFactor);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2FloatOpenHashMap(Map<? extends Character, ? extends Float> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2FloatOpenHashMap(Char2FloatMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Char2FloatOpenHashMap(Char2FloatMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(float value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (char)0 && Float.floatToIntBits(values[i]) == Float.floatToIntBits(value)) return true;
		return false;
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (char)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Float.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Char2FloatOpenHashMap copy() {
		Char2FloatOpenHashMap map = new Char2FloatOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2FloatMap.Entry> implements Char2FloatMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2FloatMap.Entry) {
					...
					int index = Char2FloatOpenHashMap.this.findIndex(entry.getCharKey());
					if(index >= 0) return Float.floatToIntBits(entry.getFloatValue()) == Float.floatToIntBits(Char2FloatOpenHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Char2FloatOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Float.valueOf(Char2FloatOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2FloatMap.Entry) {
					Char2FloatMap.Entry entry = (Char2FloatMap.Entry)o;
					return Char2FloatOpenHashMap.this.remove(entry.getCharKey(), entry.getFloatValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Char2FloatOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractCharSet {
		...
		public boolean remove(char o) {
			int oldSize = size;
			Char2FloatOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2FloatOpenHashMap.this.remove(wrapped.getChar(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
