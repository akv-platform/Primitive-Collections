 ...
 * 
 */
public class Char2CharConcurrentOpenHashMap extends AbstractChar2CharMap implements Char2CharConcurrentMap
{
	/** Segment Limit */
	...
	protected static class Segment extends StampedLock
	{
		...
		protected char computeIfAbsent(int hash, char key, Char2CharFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
