...
public class Char2BooleanRBTreeMap extends AbstractChar2BooleanMap implements Char2BooleanNavigableMap
{
	 ...
	 */
	public Char2BooleanRBTreeMap(Character[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2BooleanRBTreeMap(Character[] keys, Boolean[] values, CharComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2BooleanRBTreeMap(char[] keys, boolean[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2BooleanRBTreeMap(char[] keys, boolean[] values, CharComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Char2BooleanRBTreeMap(Map<? extends Character, ? extends Boolean> map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2BooleanRBTreeMap(Char2BooleanMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Char2BooleanRBTreeMap(Char2BooleanMap map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Char2BooleanRBTreeMap copy() {
		Char2BooleanRBTreeMap set = new Char2BooleanRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractCharSet implements CharNavigableSet
	{
		...
		public CharBidirectionalIterator iterator(char fromElement) {
			if(map instanceof Char2BooleanRBTreeMap) return ((Char2BooleanRBTreeMap)map).keyIterator(fromElement);
		...
		public CharBidirectionalIterator iterator() {
			if(map instanceof Char2BooleanRBTreeMap) return ((Char2BooleanRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public CharBidirectionalIterator descendingIterator() {
			if(map instanceof Char2BooleanRBTreeMap) return ((Char2BooleanRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Char2BooleanRBTreeMap) return ((Char2BooleanRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Char2BooleanRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Char2BooleanRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Char2BooleanRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Char2BooleanRBTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Char2BooleanRBTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Char2BooleanRBTreeMap map;
		final char lo, hi;
		...
		NavigableSubMap(Char2BooleanRBTreeMap map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractBooleanCollection
	{
		...
		@Override
		public int size() {
			return Char2BooleanRBTreeMap.this.size;
		}
