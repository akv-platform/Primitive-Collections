 ...
 */
public class Char2ByteOpenHashMap extends AbstractChar2ByteMap implements ITrimmable
{
	 ...
	 */
	public Char2ByteOpenHashMap(int minCapacity) {
	 ...
	 */
	public Char2ByteOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Char2ByteOpenHashMap(Character[] keys, Byte[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2ByteOpenHashMap(Character[] keys, Byte[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2ByteOpenHashMap(char[] keys, byte[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2ByteOpenHashMap(char[] keys, byte[] values, float loadFactor) {
		this(keys.length, loadFactor);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Char2ByteOpenHashMap(Map<? extends Character, ? extends Byte> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2ByteOpenHashMap(Char2ByteMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Char2ByteOpenHashMap(Char2ByteMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (char)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Byte.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Char2ByteOpenHashMap copy() {
		Char2ByteOpenHashMap map = new Char2ByteOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2ByteMap.Entry> implements Char2ByteMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2ByteMap.Entry) {
					...
					int index = Char2ByteOpenHashMap.this.findIndex(entry.getCharKey());
					if(index >= 0) return entry.getByteValue() == Char2ByteOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Char2ByteOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Byte.valueOf(Char2ByteOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Char2ByteMap.Entry) {
					Char2ByteMap.Entry entry = (Char2ByteMap.Entry)o;
					return Char2ByteOpenHashMap.this.remove(entry.getCharKey(), entry.getByteValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Char2ByteOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractCharSet {
		...
		public boolean remove(char o) {
			int oldSize = size;
			Char2ByteOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Char2ByteOpenHashMap.this.remove(wrapped.getChar(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
