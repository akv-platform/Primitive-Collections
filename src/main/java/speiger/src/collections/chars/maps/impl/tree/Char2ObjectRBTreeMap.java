 ...
 */
public class Char2ObjectRBTreeMap<V> extends AbstractChar2ObjectMap<V> implements Char2ObjectNavigableMap<V>
{
	 ...
	 */
	public Char2ObjectRBTreeMap(Character[] keys, V[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2ObjectRBTreeMap(Character[] keys, V[] values, CharComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2ObjectRBTreeMap(char[] keys, V[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Char2ObjectRBTreeMap(char[] keys, V[] values, CharComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Char2ObjectRBTreeMap(Map<? extends Character, ? extends V> map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Char2ObjectRBTreeMap(Char2ObjectMap<V> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Char2ObjectRBTreeMap(Char2ObjectMap<V> map, CharComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	public Char2ObjectRBTreeMap<V> copy() {
		Char2ObjectRBTreeMap<V> set = new Char2ObjectRBTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<V> extends AbstractCharSet implements CharNavigableSet
	{
		...
		public CharBidirectionalIterator iterator(char fromElement) {
			if(map instanceof Char2ObjectRBTreeMap) return ((Char2ObjectRBTreeMap<V>)map).keyIterator(fromElement);
		...
		public CharBidirectionalIterator iterator() {
			if(map instanceof Char2ObjectRBTreeMap) return ((Char2ObjectRBTreeMap<V>)map).keyIterator();
			return ((NavigableSubMap<V>)map).keyIterator();
		}
		
		@Override
		public CharBidirectionalIterator descendingIterator() {
			if(map instanceof Char2ObjectRBTreeMap) return ((Char2ObjectRBTreeMap<V>)map).descendingKeyIterator();
			return ((NavigableSubMap<V>)map).descendingKeyIterator();
		}
		
		protected Node<V> start() {
			if(map instanceof Char2ObjectRBTreeMap) return ((Char2ObjectRBTreeMap<V>)map).first;
			return ((NavigableSubMap<V>)map).subLowest();
		}
		
		protected Node<V> end() {
			if(map instanceof Char2ObjectRBTreeMap) return null;
			return ((NavigableSubMap<V>)map).subHighest();
		}
		
		protected Node<V> next(Node<V> entry) {
			if(map instanceof Char2ObjectRBTreeMap) return entry.next();
			return ((NavigableSubMap<V>)map).next(entry);
		}
		
		protected Node<V> previous(Node<V> entry) {
			if(map instanceof Char2ObjectRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Char2ObjectRBTreeMap<V> map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<V> extends NavigableSubMap<V>
	{
		...
		DescendingNaivgableSubMap(Char2ObjectRBTreeMap<V> map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Char2ObjectRBTreeMap<V> map;
		final char lo, hi;
		...
		NavigableSubMap(Char2ObjectRBTreeMap<V> map, boolean fromStart, char lo, boolean loInclusive, boolean toEnd, char hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractObjectCollection<V>
	{
		...
		@Override
		public int size() {
			return Char2ObjectRBTreeMap.this.size;
		}
