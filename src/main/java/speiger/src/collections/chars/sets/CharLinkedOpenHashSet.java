...
public class CharLinkedOpenHashSet extends CharOpenHashSet implements CharOrderedSet
{
	 ...
	 */
	@Deprecated
	public CharLinkedOpenHashSet(Collection<? extends Character> collection, float loadFactor) {
	...
	public boolean moveToFirst(char o) {
		if(isEmpty() || firstChar() == o) return false;
		if(o == (char)0) {
			if(containsNull) {
				moveToFirstIndex(nullIndex);
				return true;
	...
	public boolean moveToLast(char o) {
		if(isEmpty() || lastChar() == o) return false;
		if(o == (char)0) {
			if(containsNull) {
				moveToLastIndex(nullIndex);
				return true;
	...
	public CharLinkedOpenHashSet copy() {
		CharLinkedOpenHashSet set = new CharLinkedOpenHashSet(0, loadFactor);
