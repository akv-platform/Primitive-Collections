...
public class Object2BooleanAVLTreeMap<T> extends AbstractObject2BooleanMap<T> implements Object2BooleanNavigableMap<T>
{
	 ...
	 */
	public Object2BooleanAVLTreeMap(Comparator<T> comp) {
		comparator = comp;
	}
	 ...
	 */
	public Object2BooleanAVLTreeMap(T[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Object2BooleanAVLTreeMap(T[] keys, Boolean[] values, Comparator<T> comp) {
		comparator = comp;
	 ...
	 */
	public Object2BooleanAVLTreeMap(Map<? extends T, ? extends Boolean> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Object2BooleanAVLTreeMap(Map<? extends T, ? extends Boolean> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Object2BooleanAVLTreeMap(Object2BooleanMap<T> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Object2BooleanAVLTreeMap(Object2BooleanMap<T> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Object2BooleanAVLTreeMap<T> copy() {
		Object2BooleanAVLTreeMap<T> set = new Object2BooleanAVLTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	@Override
	public Object2BooleanMap.Entry<T> lowerEntry(T key) {
		Node<T> node = findLowerNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Object2BooleanMap.Entry<T> higherEntry(T key) {
		Node<T> node = findHigherNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Object2BooleanMap.Entry<T> floorEntry(T key) {
		Node<T> node = findFloorNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Object2BooleanMap.Entry<T> ceilingEntry(T key) {
	...
	static class KeySet<T> extends AbstractObjectSet<T> implements ObjectNavigableSet<T>
	{
		...
		public KeySet(Object2BooleanNavigableMap<T> map) {
			this.map = map;
		}
		...
		public ObjectBidirectionalIterator<T> iterator(T fromElement) {
			if(map instanceof Object2BooleanAVLTreeMap) return ((Object2BooleanAVLTreeMap<T>)map).keyIterator(fromElement);
		...
		public ObjectBidirectionalIterator<T> iterator() {
			if(map instanceof Object2BooleanAVLTreeMap) return ((Object2BooleanAVLTreeMap<T>)map).keyIterator();
			return ((NavigableSubMap<T>)map).keyIterator();
		}
		
		@Override
		public ObjectBidirectionalIterator<T> descendingIterator() {
			if(map instanceof Object2BooleanAVLTreeMap) return ((Object2BooleanAVLTreeMap<T>)map).descendingKeyIterator();
			return ((NavigableSubMap<T>)map).descendingKeyIterator();
		}
		
		protected Node<T> start() {
			if(map instanceof Object2BooleanAVLTreeMap) return ((Object2BooleanAVLTreeMap<T>)map).first;
			return ((NavigableSubMap<T>)map).subLowest();
		}
		
		protected Node<T> end() {
			if(map instanceof Object2BooleanAVLTreeMap) return null;
			return ((NavigableSubMap<T>)map).subHighest();
		}
		
		protected Node<T> next(Node<T> entry) {
			if(map instanceof Object2BooleanAVLTreeMap) return entry.next();
			return ((NavigableSubMap<T>)map).next(entry);
		}
		
		protected Node<T> previous(Node<T> entry) {
			if(map instanceof Object2BooleanAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Object2BooleanAVLTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<T> extends NavigableSubMap<T>
	{
		...
		DescendingNaivgableSubMap(Object2BooleanAVLTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap<T> extends AbstractObject2BooleanMap<T> implements Object2BooleanNavigableMap<T>
	{
		final Object2BooleanAVLTreeMap<T> map;
		final T lo, hi;
		...
		NavigableSubMap(Object2BooleanAVLTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public boolean getBoolean(T key) {
			return inRange(key) ? map.getBoolean(key) : getDefaultReturnValue();
		}
		...
		@Override
		public Object2BooleanMap.Entry<T> lowerEntry(T key) { return subLower(key); }
		@Override
		public Object2BooleanMap.Entry<T> floorEntry(T key) { return subFloor(key); }
		@Override
		public Object2BooleanMap.Entry<T> ceilingEntry(T key) { return subCeiling(key); }
		@Override
		public Object2BooleanMap.Entry<T> higherEntry(T key) { return subHigher(key); }
		...
		public Object2BooleanMap.Entry<T> pollFirstEntry() {
			Node<T> entry = subLowest();
			if(entry != null) {
				Object2BooleanMap.Entry<T> result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Object2BooleanMap.Entry<T>> {
			...
			public int size() {
				...
				int size = 0;
				for(ObjectIterator<Object2BooleanMap.Entry<T>> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
		...
		class DecsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Object2BooleanMap.Entry<T>>
		{
		...
		class AcsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Object2BooleanMap.Entry<T>>
		{
	...
	class Values extends AbstractBooleanCollection
	{
		...
		@Override
		public int size() {
			return Object2BooleanAVLTreeMap.this.size;
		}
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Object2BooleanMap.Entry<T>>
	{
	...
	private static final class Node<T> implements Object2BooleanMap.Entry<T>
	{
		T key;
		boolean value;
		int state;
		...
		Node(T key, boolean value, Node<T> parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherValue instanceof Boolean && Objects.equals(key, otherKey) && value == ((Boolean)otherValue).booleanValue();
			}
			return false;
		}
