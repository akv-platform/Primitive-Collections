 ...
 */
public class Object2FloatRBTreeMap<T> extends AbstractObject2FloatMap<T> implements Object2FloatNavigableMap<T>
{
	 ...
	 */
	public Object2FloatRBTreeMap(Comparator<T> comp) {
		comparator = comp;
	}
	 ...
	 */
	public Object2FloatRBTreeMap(T[] keys, Float[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Object2FloatRBTreeMap(T[] keys, Float[] values, Comparator<T> comp) {
		comparator = comp;
	 ...
	 */
	public Object2FloatRBTreeMap(Map<? extends T, ? extends Float> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Object2FloatRBTreeMap(Map<? extends T, ? extends Float> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Object2FloatRBTreeMap(Object2FloatMap<T> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Object2FloatRBTreeMap(Object2FloatMap<T> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Object2FloatRBTreeMap<T> copy() {
		Object2FloatRBTreeMap<T> set = new Object2FloatRBTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<T> extends AbstractObjectSet<T> implements ObjectNavigableSet<T>
	{
		...
		public ObjectBidirectionalIterator<T> iterator(T fromElement) {
			if(map instanceof Object2FloatRBTreeMap) return ((Object2FloatRBTreeMap<T>)map).keyIterator(fromElement);
		...
		public ObjectBidirectionalIterator<T> iterator() {
			if(map instanceof Object2FloatRBTreeMap) return ((Object2FloatRBTreeMap<T>)map).keyIterator();
			return ((NavigableSubMap<T>)map).keyIterator();
		}
		
		@Override
		public ObjectBidirectionalIterator<T> descendingIterator() {
			if(map instanceof Object2FloatRBTreeMap) return ((Object2FloatRBTreeMap<T>)map).descendingKeyIterator();
			return ((NavigableSubMap<T>)map).descendingKeyIterator();
		}
		
		protected Node<T> start() {
			if(map instanceof Object2FloatRBTreeMap) return ((Object2FloatRBTreeMap<T>)map).first;
			return ((NavigableSubMap<T>)map).subLowest();
		}
		
		protected Node<T> end() {
			if(map instanceof Object2FloatRBTreeMap) return null;
			return ((NavigableSubMap<T>)map).subHighest();
		}
		
		protected Node<T> next(Node<T> entry) {
			if(map instanceof Object2FloatRBTreeMap) return entry.next();
			return ((NavigableSubMap<T>)map).next(entry);
		}
		
		protected Node<T> previous(Node<T> entry) {
			if(map instanceof Object2FloatRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Object2FloatRBTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<T> extends NavigableSubMap<T>
	{
		...
		DescendingNaivgableSubMap(Object2FloatRBTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Object2FloatRBTreeMap<T> map;
		final T lo, hi;
		...
		NavigableSubMap(Object2FloatRBTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractFloatCollection
	{
		...
		@Override
		public int size() {
			return Object2FloatRBTreeMap.this.size;
		}
