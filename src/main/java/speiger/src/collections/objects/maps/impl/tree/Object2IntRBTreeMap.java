 ...
 */
public class Object2IntRBTreeMap<T> extends AbstractObject2IntMap<T> implements Object2IntNavigableMap<T>
{
	 ...
	 */
	public Object2IntRBTreeMap(Comparator<T> comp) {
		comparator = comp;
	}
	 ...
	 */
	public Object2IntRBTreeMap(T[] keys, Integer[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Object2IntRBTreeMap(T[] keys, Integer[] values, Comparator<T> comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Object2IntRBTreeMap(T[] keys, int[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Object2IntRBTreeMap(T[] keys, int[] values, Comparator<T> comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 */
	public Object2IntRBTreeMap(Map<? extends T, ? extends Integer> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Object2IntRBTreeMap(Map<? extends T, ? extends Integer> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Object2IntRBTreeMap(Object2IntMap<T> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Object2IntRBTreeMap(Object2IntMap<T> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Object2IntRBTreeMap<T> copy() {
		Object2IntRBTreeMap<T> set = new Object2IntRBTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<T> extends AbstractObjectSet<T> implements ObjectNavigableSet<T>
	{
		...
		public ObjectBidirectionalIterator<T> iterator(T fromElement) {
			if(map instanceof Object2IntRBTreeMap) return ((Object2IntRBTreeMap<T>)map).keyIterator(fromElement);
		...
		public ObjectBidirectionalIterator<T> iterator() {
			if(map instanceof Object2IntRBTreeMap) return ((Object2IntRBTreeMap<T>)map).keyIterator();
			return ((NavigableSubMap<T>)map).keyIterator();
		}
		
		@Override
		public ObjectBidirectionalIterator<T> descendingIterator() {
			if(map instanceof Object2IntRBTreeMap) return ((Object2IntRBTreeMap<T>)map).descendingKeyIterator();
			return ((NavigableSubMap<T>)map).descendingKeyIterator();
		}
		
		protected Node<T> start() {
			if(map instanceof Object2IntRBTreeMap) return ((Object2IntRBTreeMap<T>)map).first;
			return ((NavigableSubMap<T>)map).subLowest();
		}
		
		protected Node<T> end() {
			if(map instanceof Object2IntRBTreeMap) return null;
			return ((NavigableSubMap<T>)map).subHighest();
		}
		
		protected Node<T> next(Node<T> entry) {
			if(map instanceof Object2IntRBTreeMap) return entry.next();
			return ((NavigableSubMap<T>)map).next(entry);
		}
		
		protected Node<T> previous(Node<T> entry) {
			if(map instanceof Object2IntRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Object2IntRBTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<T> extends NavigableSubMap<T>
	{
		...
		DescendingNaivgableSubMap(Object2IntRBTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Object2IntRBTreeMap<T> map;
		final T lo, hi;
		...
		NavigableSubMap(Object2IntRBTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractIntCollection
	{
		...
		@Override
		public int size() {
			return Object2IntRBTreeMap.this.size;
		}
