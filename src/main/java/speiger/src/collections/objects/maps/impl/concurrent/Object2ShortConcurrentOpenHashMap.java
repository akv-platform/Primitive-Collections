 ...
 */
public class Object2ShortConcurrentOpenHashMap<T> extends AbstractObject2ShortMap<T> implements Object2ShortConcurrentMap<T>, ITrimmable
{
	/** Segment Limit */
	 ...
	 */
	protected Object2ShortConcurrentOpenHashMap(boolean unused) {}
	 ...
	 */
	public Object2ShortConcurrentOpenHashMap(int minCapacity) {
	 ...
	 */
	public Object2ShortConcurrentOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Object2ShortConcurrentOpenHashMap(int minCapacity, int concurrencyLevel) {
	 ...
	 * @throws IllegalStateException if the concurrencyLevel is either below/equal to 0 or above/equal to 65535
	 */
	public Object2ShortConcurrentOpenHashMap(float loadFactor, int concurrencyLevel) {
	 ...
	 * @throws IllegalStateException if the concurrencyLevel is either below/equal to 0 or above/equal to 65535
	 */
	public Object2ShortConcurrentOpenHashMap(int minCapacity, float loadFactor, int concurrencyLevel) {
	 ...
	 */
	public Object2ShortConcurrentOpenHashMap(T[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Object2ShortConcurrentOpenHashMap(T[] keys, Short[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the concurrencyLevel is either below/equal to 0 or above/equal to 65535
	 */
	public Object2ShortConcurrentOpenHashMap(T[] keys, Short[] values, int concurrencyLevel) {
	 ...
	 * @throws IllegalStateException if the concurrencyLevel is either below/equal to 0 or above/equal to 65535
	 */
	public Object2ShortConcurrentOpenHashMap(T[] keys, Short[] values, float loadFactor, int concurrencyLevel) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Object2ShortConcurrentOpenHashMap(Map<? extends T, ? extends Short> map, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the concurrencyLevel is either below/equal to 0 or above/equal to 65535
	 */
	public Object2ShortConcurrentOpenHashMap(Map<? extends T, ? extends Short> map, int concurrencyLevel) {
	 ...
	 * @throws IllegalStateException if the concurrencyLevel is either below/equal to 0 or above/equal to 65535
	 */
	public Object2ShortConcurrentOpenHashMap(Map<? extends T, ? extends Short> map, float loadFactor, int concurrencyLevel) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Object2ShortConcurrentOpenHashMap(Object2ShortMap<T> map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR, HashUtil.DEFAULT_MIN_CONCURRENCY);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Object2ShortConcurrentOpenHashMap(Object2ShortMap<T> map, float loadFactor) {
		this(map, loadFactor, HashUtil.DEFAULT_MIN_CONCURRENCY);
	}
	 ...
	 * @throws IllegalStateException if the concurrencyLevel is either below/equal to 0 or above/equal to 65535
 	 */
	public Object2ShortConcurrentOpenHashMap(Object2ShortMap<T> map, int concurrencyLevel) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR, concurrencyLevel);
	}
	 ...
	 * @throws IllegalStateException if the concurrencyLevel is either below/equal to 0 or above/equal to 65535
 	 */
	public Object2ShortConcurrentOpenHashMap(Object2ShortMap<T> map, float loadFactor, int concurrencyLevel) {
		this(map.size(), loadFactor, concurrencyLevel);
		putAll(map);
	}
	...
	public Object2ShortConcurrentOpenHashMap<T> copy() {
		Object2ShortConcurrentOpenHashMap<T> copy = new Object2ShortConcurrentOpenHashMap<>(false);
	...
	private class MapEntrySet extends AbstractObjectSet<Object2ShortMap.Entry<T>> implements Object2ShortMap.FastEntrySet<T> {
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2ShortMap.Entry) {
					Object2ShortMap.Entry<T> entry = (Object2ShortMap.Entry<T>)o;
					return Object2ShortConcurrentOpenHashMap.this.remove(entry.getKey(), entry.getShortValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Object2ShortConcurrentOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	...
	private final class KeySet extends AbstractObjectSet<T> implements ObjectSet<T> {
		...
		public boolean remove(Object o) {
			int oldSize = size();
			Object2ShortConcurrentOpenHashMap.this.remove(o);
			return size() != oldSize;
		}
	...
	private class FastEntryIterator extends MapIterator implements ObjectBidirectionalIterator<Object2ShortMap.Entry<T>> {
	...
	private class EntryIterator extends MapIterator implements ObjectBidirectionalIterator<Object2ShortMap.Entry<T>> {
		MapEntry entry;
	...
	protected static class Segment<T> extends StampedLock
	{
		...
		protected Segment(Object2ShortConcurrentOpenHashMap<T> map) {
			this.map = map;
		}
		
		protected Segment(Object2ShortConcurrentOpenHashMap<T> map, int minCapacity, float loadFactor, boolean isNullContainer) {
			this.map = map;
		...
		protected Segment<T> copy(Object2ShortConcurrentOpenHashMap<T> newMap) {
			long stamp = readLock();
			try
			{
		...
		protected short put(int hash, T key, short value) {
		...
		protected short putIfAbsent(int hash, T key, short value) {
			long stamp = writeLock();
			try {
				int slot = findIndex(hash, key);
				if(slot < 0) {
		...
		protected short addTo(int hash, T key, short value) {
			long stamp = writeLock();
			try {
				int slot = findIndex(hash, key);
				if(slot < 0) {
		...
		protected short subFrom(int hash, T key, short value) {
			long stamp = writeLock();
			try {
				int slot = findIndex(hash, key);
		...
		protected short getOrDefault(int hash, T key, short defaultValue) {
			long stamp = readLock();
			try {
				int slot = findIndex(hash, key);
		...
		protected void forEach(ObjectShortConsumer<T> action) {
			long stamp = readLock();
			try {
				int index = firstIndex;
				while(index != -1) {
		...
		protected short removeOrDefault(int hash, T key, short defaultValue) {
			long stamp = writeLock();
			try {
				int slot = findIndex(hash, key);
		...
		protected boolean remove(int hash, T key, short value) {
			long stamp = writeLock();
			try {
				if(key == null) {
		...
		protected boolean remove(int hash, Object key, Object value) {
			...
			try
			{
				...
				T current = keys[pos];
				if(current == null) return false;
				if(Objects.equals(key, current) && Objects.equals(value, Short.valueOf(values[pos]))) {
					removeIndex(pos);
					return true;
				}
				while(true) {
					if((current = keys[pos = (++pos & mask)]) == null) return false;
					else if(Objects.equals(key, current) && Objects.equals(value, Short.valueOf(values[pos]))){
						removeIndex(pos);
						return true;
					}
				}				
		...
		protected boolean replace(int hash, T key, short oldValue, short newValue) {
			long stamp = writeLock();
			try {
		...
		protected short replace(int hash, T key, short value) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
		...
		protected short compute(int hash, T key, ObjectShortUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfAbsent(int hash, T key, Object2ShortFunction<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short supplyIfAbsent(int hash, T key, ShortSupplier valueProvider) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected short computeIfPresent(int hash, T key, ObjectShortUnaryOperator<T> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
		...
		protected short merge(int hash, T key, short value, ShortShortUnaryOperator mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
