 ...
 */
public class Object2ObjectRBTreeMap<T, V> extends AbstractObject2ObjectMap<T, V> implements Object2ObjectNavigableMap<T, V>
{
	 ...
	 */
	public Object2ObjectRBTreeMap(Comparator<T> comp) {
		comparator = comp;
	}
	 ...
	 */
	public Object2ObjectRBTreeMap(T[] keys, V[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Object2ObjectRBTreeMap(T[] keys, V[] values, Comparator<T> comp) {
		comparator = comp;
	 ...
	 */
	public Object2ObjectRBTreeMap(Map<? extends T, ? extends V> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Object2ObjectRBTreeMap(Map<? extends T, ? extends V> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Object2ObjectRBTreeMap(Object2ObjectMap<T, V> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Object2ObjectRBTreeMap(Object2ObjectMap<T, V> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Object2ObjectRBTreeMap<T, V> copy() {
		Object2ObjectRBTreeMap<T, V> set = new Object2ObjectRBTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	protected static <T, V> boolean isBlack(Node<T, V> p) { return p == null || p.isBlack(); }
	protected static <T, V> Node<T, V> parentOf(Node<T, V> p) { return (p == null ? null : p.parent); }
	protected static <T, V> void setBlack(Node<T, V> p, boolean c) { if(p != null) p.setBlack(c); }
	protected static <T, V> Node<T, V> leftOf(Node<T, V> p) { return p == null ? null : p.left; }
	protected static <T, V> Node<T, V> rightOf(Node<T, V> p) { return (p == null) ? null : p.right; }
	...
	protected void fixAfterInsertion(Node<T, V> entry) {
		...
		while(entry != null && entry != tree && !entry.parent.isBlack()) {
			if(parentOf(entry) == leftOf(parentOf(parentOf(entry)))) {
				Node<T, V> y = rightOf(parentOf(parentOf(entry)));
				if(!isBlack(y)) {
				...
				Node<T, V> y = leftOf(parentOf(parentOf(entry)));
				if(!isBlack(y)) {
					setBlack(parentOf(entry), true);
					setBlack(y, true);
					setBlack(parentOf(parentOf(entry)), false);
	...
	protected void fixAfterDeletion(Node<T, V> entry) {
		while(entry != tree && isBlack(entry)) {
			if(entry == leftOf(parentOf(entry))) {
				Node<T, V> sib = rightOf(parentOf(entry));
				if(!isBlack(sib)) {
					setBlack(sib, true);
				...
				Node<T, V> sib = leftOf(parentOf(entry));
				if(!isBlack(sib)) {
					setBlack(sib, true);
					setBlack(parentOf(entry), false);
	...
	static class KeySet<T, V> extends AbstractObjectSet<T> implements ObjectNavigableSet<T>
	{
		...
		public ObjectBidirectionalIterator<T> iterator(T fromElement) {
			if(map instanceof Object2ObjectRBTreeMap) return ((Object2ObjectRBTreeMap<T, V>)map).keyIterator(fromElement);
		...
		public ObjectBidirectionalIterator<T> iterator() {
			if(map instanceof Object2ObjectRBTreeMap) return ((Object2ObjectRBTreeMap<T, V>)map).keyIterator();
			return ((NavigableSubMap<T, V>)map).keyIterator();
		}
		
		@Override
		public ObjectBidirectionalIterator<T> descendingIterator() {
			if(map instanceof Object2ObjectRBTreeMap) return ((Object2ObjectRBTreeMap<T, V>)map).descendingKeyIterator();
			return ((NavigableSubMap<T, V>)map).descendingKeyIterator();
		}
		
		protected Node<T, V> start() {
			if(map instanceof Object2ObjectRBTreeMap) return ((Object2ObjectRBTreeMap<T, V>)map).first;
			return ((NavigableSubMap<T, V>)map).subLowest();
		}
		
		protected Node<T, V> end() {
			if(map instanceof Object2ObjectRBTreeMap) return null;
			return ((NavigableSubMap<T, V>)map).subHighest();
		}
		
		protected Node<T, V> next(Node<T, V> entry) {
			if(map instanceof Object2ObjectRBTreeMap) return entry.next();
			return ((NavigableSubMap<T, V>)map).next(entry);
		}
		
		protected Node<T, V> previous(Node<T, V> entry) {
			if(map instanceof Object2ObjectRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Object2ObjectRBTreeMap<T, V> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<T, V> extends NavigableSubMap<T, V>
	{
		...
		DescendingNaivgableSubMap(Object2ObjectRBTreeMap<T, V> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Object2ObjectRBTreeMap<T, V> map;
		final T lo, hi;
		...
		NavigableSubMap(Object2ObjectRBTreeMap<T, V> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractObjectCollection<V>
	{
		...
		@Override
		public int size() {
			return Object2ObjectRBTreeMap.this.size;
		}
