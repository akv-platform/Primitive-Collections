 ...
 */
public class Object2ByteAVLTreeMap<T> extends AbstractObject2ByteMap<T> implements Object2ByteNavigableMap<T>
{
	...
	/** EntrySet Cache */
	protected ObjectSet<Object2ByteMap.Entry<T>> entrySet;
	 ...
	 */
	public Object2ByteAVLTreeMap(Comparator<T> comp) {
		comparator = comp;
	}
	 ...
	 */
	public Object2ByteAVLTreeMap(T[] keys, Byte[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Object2ByteAVLTreeMap(T[] keys, Byte[] values, Comparator<T> comp) {
		comparator = comp;
	 ...
	 */
	public Object2ByteAVLTreeMap(Map<? extends T, ? extends Byte> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Object2ByteAVLTreeMap(Map<? extends T, ? extends Byte> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Object2ByteAVLTreeMap(Object2ByteMap<T> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Object2ByteAVLTreeMap(Object2ByteMap<T> map, Comparator<T> comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Object2ByteAVLTreeMap<T> copy() {
		Object2ByteAVLTreeMap<T> set = new Object2ByteAVLTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	@Override
	public Object2ByteMap.Entry<T> lowerEntry(T key) {
		Node<T> node = findLowerNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Object2ByteMap.Entry<T> higherEntry(T key) {
		Node<T> node = findHigherNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Object2ByteMap.Entry<T> floorEntry(T key) {
		Node<T> node = findFloorNode(key);
		return node != null ? node.export() : null;
	}
	
	@Override
	public Object2ByteMap.Entry<T> ceilingEntry(T key) {
	...
	static class KeySet<T> extends AbstractObjectSet<T> implements ObjectNavigableSet<T>
	{
		...
		public KeySet(Object2ByteNavigableMap<T> map) {
			this.map = map;
		}
		...
		public ObjectBidirectionalIterator<T> iterator(T fromElement) {
			if(map instanceof Object2ByteAVLTreeMap) return ((Object2ByteAVLTreeMap<T>)map).keyIterator(fromElement);
		...
		public ObjectBidirectionalIterator<T> iterator() {
			if(map instanceof Object2ByteAVLTreeMap) return ((Object2ByteAVLTreeMap<T>)map).keyIterator();
			return ((NavigableSubMap<T>)map).keyIterator();
		}
		
		@Override
		public ObjectBidirectionalIterator<T> descendingIterator() {
			if(map instanceof Object2ByteAVLTreeMap) return ((Object2ByteAVLTreeMap<T>)map).descendingKeyIterator();
			return ((NavigableSubMap<T>)map).descendingKeyIterator();
		}
		
		protected Node<T> start() {
			if(map instanceof Object2ByteAVLTreeMap) return ((Object2ByteAVLTreeMap<T>)map).first;
			return ((NavigableSubMap<T>)map).subLowest();
		}
		
		protected Node<T> end() {
			if(map instanceof Object2ByteAVLTreeMap) return null;
			return ((NavigableSubMap<T>)map).subHighest();
		}
		
		protected Node<T> next(Node<T> entry) {
			if(map instanceof Object2ByteAVLTreeMap) return entry.next();
			return ((NavigableSubMap<T>)map).next(entry);
		}
		
		protected Node<T> previous(Node<T> entry) {
			if(map instanceof Object2ByteAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Object2ByteAVLTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<T> extends NavigableSubMap<T>
	{
		...
		DescendingNaivgableSubMap(Object2ByteAVLTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap<T> extends AbstractObject2ByteMap<T> implements Object2ByteNavigableMap<T>
	{
		final Object2ByteAVLTreeMap<T> map;
		final T lo, hi;
		final boolean fromStart, toEnd;
		final boolean loInclusive, hiInclusive;
		
		Object2ByteNavigableMap<T> inverse;
		ObjectNavigableSet<T> keySet;
		ObjectSet<Object2ByteMap.Entry<T>> entrySet;
		ByteCollection values;
		
		NavigableSubMap(Object2ByteAVLTreeMap<T> map, boolean fromStart, T lo, boolean loInclusive, boolean toEnd, T hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		@Override
		public byte getByte(T key) {
			return inRange(key) ? map.getByte(key) : getDefaultReturnValue();
		}
		...
		@Override
		public Object2ByteMap.Entry<T> lowerEntry(T key) { return subLower(key); }
		@Override
		public Object2ByteMap.Entry<T> floorEntry(T key) { return subFloor(key); }
		@Override
		public Object2ByteMap.Entry<T> ceilingEntry(T key) { return subCeiling(key); }
		@Override
		public Object2ByteMap.Entry<T> higherEntry(T key) { return subHigher(key); }
		...
		public Object2ByteMap.Entry<T> pollFirstEntry() {
			Node<T> entry = subLowest();
			if(entry != null) {
				Object2ByteMap.Entry<T> result = entry.export();
				map.removeNode(entry);
				return result;
			}
			return null;
		...
		abstract class SubEntrySet extends AbstractObjectSet<Object2ByteMap.Entry<T>> {
			@Override
			public int size() {
				if (fromStart && toEnd) return map.size();
				int size = 0;
				for(ObjectIterator<Object2ByteMap.Entry<T>> iter = iterator();iter.hasNext();iter.next(),size++);
				return size;
			}
		...
		class DecsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Object2ByteMap.Entry<T>>
		{
		...
		class AcsendingSubEntryIterator extends SubMapEntryIterator implements ObjectBidirectionalIterator<Object2ByteMap.Entry<T>>
		{
	...
	class Values extends AbstractByteCollection
	{
		...
		@Override
		public int size() {
			return Object2ByteAVLTreeMap.this.size;
		}
	...
	class AscendingMapEntryIterator extends MapEntryIterator implements ObjectBidirectionalIterator<Object2ByteMap.Entry<T>>
	{
	...
	private static final class Node<T> implements Object2ByteMap.Entry<T>
	{
		T key;
		byte value;
		int state;
		...
		Node(T key, byte value, Node<T> parent) {
			this.key = key;
			this.value = value;
			this.parent = parent;
		}
		...
		public boolean equals(Object obj) {
			if(obj instanceof Map.Entry) {
				...
				return otherValue instanceof Byte && Objects.equals(key, otherKey) && value == ((Byte)otherValue).byteValue();
			}
			return false;
		}
