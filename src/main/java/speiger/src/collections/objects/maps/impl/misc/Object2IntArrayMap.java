 ...
 */
public class Object2IntArrayMap<T> extends AbstractObject2IntMap<T> implements Object2IntOrderedMap<T>
{
	 ...
	 */
	public Object2IntArrayMap(int minCapacity) {
	 ...
	 */
	public Object2IntArrayMap(T[] keys, Integer[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in length
	 */
	public Object2IntArrayMap(T[] keys, Integer[] values, int length) {
		this(length);
	 ...
	 */
	public Object2IntArrayMap(T[] keys, int[] values) {
		this(keys, values, keys.length);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Object2IntArrayMap(T[] keys, int[] values, int length) {
		this(length);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		putAll(keys, values, 0, length);
	}
	 ...
	 */
	public Object2IntArrayMap(Map<? extends T, ? extends Integer> map) {
		this(map.size());
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Object2IntArrayMap(Object2IntMap<T> map) {
		this(map.size());
		for(ObjectIterator<Object2IntMap.Entry<T>> iter = Object2IntMaps.fastIterator(map);iter.hasNext();size++) {
	...
	public Object2IntArrayMap<T> copy() {
		Object2IntArrayMap<T> map = new Object2IntArrayMap<>();
		map.size = size;
	...
	protected void insertIndex(int index, T key, int value) {
		grow(size+1);
		if(index != size) {
	...
	protected int findIndex(T key, int value) {
	...
	protected int findIndex(Object key, Object value) {
		for(int i = size-1;i>=0;i--)
			if(Objects.equals(key, keys[i]) && Objects.equals(value, Integer.valueOf(values[i]))) return i;
		return -1;		
	}
	...
	private class MapEntrySet extends AbstractObjectSet<Object2IntMap.Entry<T>> implements Object2IntOrderedMap.FastOrderedSet<T> {
		...
		public boolean moveToFirst(Object2IntMap.Entry<T> o) {
			return Object2IntArrayMap.this.moveToFirst(o.getKey());
		}
		
		@Override
		public boolean moveToLast(Object2IntMap.Entry<T> o) {
			return Object2IntArrayMap.this.moveToLast(o.getKey());
		}
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2IntMap.Entry) {
					...
					int index = Object2IntArrayMap.this.findIndex(entry.getKey());
					if(index >= 0) return entry.getIntValue() == Object2IntArrayMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Object2IntArrayMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Integer.valueOf(Object2IntArrayMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Object2IntMap.Entry) {
					Object2IntMap.Entry<T> entry = (Object2IntMap.Entry<T>)o;
					return Object2IntArrayMap.this.remove(entry.getKey(), entry.getIntValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Object2IntArrayMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	...
	private class KeySet extends AbstractObjectSet<T> implements ObjectOrderedSet<T> {
		...
		public boolean remove(Object o) {
			int oldSize = size;
			Object2IntArrayMap.this.remove(o);
			return size != oldSize;
		}
		...
		@Override
		public boolean moveToFirst(T o) { return Object2IntArrayMap.this.moveToFirst(o); }
		@Override
		public boolean moveToLast(T o) { return Object2IntArrayMap.this.moveToLast(o); }
		@Override
		...
		@Override
		public int size() { return Object2IntArrayMap.this.size(); }
		@Override
		public void clear() { Object2IntArrayMap.this.clear(); }
		@Override
		...
		public boolean matchesAny(Object2BooleanFunction<T> filter) {
			...
			if(size() <= 0) return false;
			for(int i = 0;i<size;i++) {
				if(filter.getBoolean(keys[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(Object2BooleanFunction<T> filter) {
			Objects.requireNonNull(filter);
			for(int i = 0;i<size;i++) {
				if(filter.getBoolean(keys[i])) return false;
			}
			return true;
		}
		...
		public T findFirst(Object2BooleanFunction<T> filter) {
			Objects.requireNonNull(filter);
			for(int i = 0;i<size;i++) {
				if(filter.getBoolean(keys[i])) return keys[i];
			}
			return null;
		}
		...
		public int count(Object2BooleanFunction<T> filter) {
			...
			int result = 0;
			for(int i = 0;i<size;i++) {
				if(filter.getBoolean(keys[i])) result++;
			}
			return result;
		}
	}
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Object2IntMap.Entry<T>> {
		...
		@Override
		public void set(Object2IntMap.Entry<T> e) { throw new UnsupportedOperationException(); }
		@Override
		public void add(Object2IntMap.Entry<T> e) { throw new UnsupportedOperationException(); }
	}
	...
	private class MapEntry implements Object2IntMap.Entry<T>, Map.Entry<T, Integer> {
		int index = -1;
