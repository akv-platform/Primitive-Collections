...
public class ByteIterators
{
	 ...
	 */
	public static <E> ObjectIterator<E> map(Iterator<? extends Byte> iterator, Byte2ObjectFunction<E> mapper) {
	 ...
	 * @return a iterator that is mapped to a new result
	 */
	public static <E> ObjectIterator<E> map(ByteIterator iterator, Byte2ObjectFunction<E> mapper) {
	 ...
	 */
	public static <E, V extends Iterable<E>> ObjectIterator<E> flatMap(Iterator<? extends Byte> iterator, Byte2ObjectFunction<V> mapper) {
	 ...
	 * @return a iterator that is flatMapped to a new result
	 */
	public static <E, V extends Iterable<E>> ObjectIterator<E> flatMap(ByteIterator iterator, Byte2ObjectFunction<V> mapper) {
	 ...
	 * @return a iterator that is flatMapped to a new result
	 */
	public static <E> ObjectIterator<E> arrayFlatMap(Iterator<? extends Byte> iterator, Byte2ObjectFunction<E[]> mapper) {
	 ...
	 * @return a iterator that is flatMapped to a new result
	 */
	public static <E> ObjectIterator<E> arrayFlatMap(ByteIterator iterator, Byte2ObjectFunction<E[]> mapper) {
	 ...
	 */
	public static ByteIterator filter(Iterator<? extends Byte> iterator, Byte2BooleanFunction filter) {
	 ...
	 * @return a filtered iterator
	 */
	public static ByteIterator filter(ByteIterator iterator, Byte2BooleanFunction filter) {
	...
	private static class MappedIterator<T> implements ObjectIterator<T>
	{
		...
		MappedIterator(ByteIterator iterator, Byte2ObjectFunction<T> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		@Override
		public T next() {
			return mapper.get(iterator.nextByte());
		}
	...
	private static class FlatMappedIterator<T, V extends Iterable<T>> implements ObjectIterator<T>
	{
		...
		FlatMappedIterator(ByteIterator iterator, Byte2ObjectFunction<V> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				...
				last = mapper.get(iterator.nextByte()).iterator();
			}
		}
	...
	private static class FlatMappedArrayIterator<T> implements ObjectIterator<T>
	{
		...
		FlatMappedArrayIterator(ByteIterator iterator, Byte2ObjectFunction<T[]> mapper) {
			this.iterator = iterator;
			this.mapper = mapper;
		}
		...
		void compute() {
			...
			while(iterator.hasNext()) {
				if(last != null && last.hasNext()) return;
				last = ObjectIterators.wrap(mapper.get(iterator.nextByte()));
			}
		}
	...
	private static class SortedIterator implements ByteIterator
	{
		...
		public boolean hasNext() {
			if(sortedElements == null) {
				...
				sortedElements = hasNext ? pour(iterator) : ByteLists.empty();
	...
	private static class FilteredIterator implements ByteIterator
	{
		...
		public FilteredIterator(ByteIterator iterator, Byte2BooleanFunction filter) {
			this.iterator = iterator;
			this.filter = filter;
		}
