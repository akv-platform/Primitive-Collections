 ...
 */
public class Byte2DoubleRBTreeMap extends AbstractByte2DoubleMap implements Byte2DoubleNavigableMap
{
	 ...
	 */
	public Byte2DoubleRBTreeMap(ByteComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Byte2DoubleRBTreeMap(Byte[] keys, Double[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2DoubleRBTreeMap(Byte[] keys, Double[] values, ByteComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Byte2DoubleRBTreeMap(Map<? extends Byte, ? extends Double> map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Byte2DoubleRBTreeMap(Byte2DoubleMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Byte2DoubleRBTreeMap(Byte2DoubleMap map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Byte2DoubleRBTreeMap copy() {
		Byte2DoubleRBTreeMap set = new Byte2DoubleRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractByteSet implements ByteNavigableSet
	{
		...
		public ByteBidirectionalIterator iterator(byte fromElement) {
			if(map instanceof Byte2DoubleRBTreeMap) return ((Byte2DoubleRBTreeMap)map).keyIterator(fromElement);
		...
		public ByteBidirectionalIterator iterator() {
			if(map instanceof Byte2DoubleRBTreeMap) return ((Byte2DoubleRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ByteBidirectionalIterator descendingIterator() {
			if(map instanceof Byte2DoubleRBTreeMap) return ((Byte2DoubleRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Byte2DoubleRBTreeMap) return ((Byte2DoubleRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Byte2DoubleRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Byte2DoubleRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Byte2DoubleRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Byte2DoubleRBTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Byte2DoubleRBTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Byte2DoubleRBTreeMap map;
		final byte lo, hi;
		...
		NavigableSubMap(Byte2DoubleRBTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractDoubleCollection
	{
		...
		@Override
		public int size() {
			return Byte2DoubleRBTreeMap.this.size;
		}
