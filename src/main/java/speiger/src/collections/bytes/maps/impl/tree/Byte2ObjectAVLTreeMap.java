 ...
 */
public class Byte2ObjectAVLTreeMap<V> extends AbstractByte2ObjectMap<V> implements Byte2ObjectNavigableMap<V>
{
	 ...
	 */
	public Byte2ObjectAVLTreeMap(ByteComparator comp) {
		comparator = comp;
	}
	 ...
	 */
	public Byte2ObjectAVLTreeMap(Byte[] keys, V[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2ObjectAVLTreeMap(Byte[] keys, V[] values, ByteComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Byte2ObjectAVLTreeMap(Map<? extends Byte, ? extends V> map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Byte2ObjectAVLTreeMap(Byte2ObjectMap<V> map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Byte2ObjectAVLTreeMap(Byte2ObjectMap<V> map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Byte2ObjectAVLTreeMap<V> copy() {
		Byte2ObjectAVLTreeMap<V> set = new Byte2ObjectAVLTreeMap<>();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
	...
	static class KeySet<V> extends AbstractByteSet implements ByteNavigableSet
	{
		...
		public ByteBidirectionalIterator iterator(byte fromElement) {
			if(map instanceof Byte2ObjectAVLTreeMap) return ((Byte2ObjectAVLTreeMap<V>)map).keyIterator(fromElement);
		...
		public ByteBidirectionalIterator iterator() {
			if(map instanceof Byte2ObjectAVLTreeMap) return ((Byte2ObjectAVLTreeMap<V>)map).keyIterator();
			return ((NavigableSubMap<V>)map).keyIterator();
		}
		
		@Override
		public ByteBidirectionalIterator descendingIterator() {
			if(map instanceof Byte2ObjectAVLTreeMap) return ((Byte2ObjectAVLTreeMap<V>)map).descendingKeyIterator();
			return ((NavigableSubMap<V>)map).descendingKeyIterator();
		}
		
		protected Node<V> start() {
			if(map instanceof Byte2ObjectAVLTreeMap) return ((Byte2ObjectAVLTreeMap<V>)map).first;
			return ((NavigableSubMap<V>)map).subLowest();
		}
		
		protected Node<V> end() {
			if(map instanceof Byte2ObjectAVLTreeMap) return null;
			return ((NavigableSubMap<V>)map).subHighest();
		}
		
		protected Node<V> next(Node<V> entry) {
			if(map instanceof Byte2ObjectAVLTreeMap) return entry.next();
			return ((NavigableSubMap<V>)map).next(entry);
		}
		
		protected Node<V> previous(Node<V> entry) {
			if(map instanceof Byte2ObjectAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Byte2ObjectAVLTreeMap<V> map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap<V> extends NavigableSubMap<V>
	{
		...
		DescendingNaivgableSubMap(Byte2ObjectAVLTreeMap<V> map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Byte2ObjectAVLTreeMap<V> map;
		final byte lo, hi;
		...
		NavigableSubMap(Byte2ObjectAVLTreeMap<V> map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractObjectCollection<V>
	{
		...
		@Override
		public int size() {
			return Byte2ObjectAVLTreeMap.this.size;
		}
