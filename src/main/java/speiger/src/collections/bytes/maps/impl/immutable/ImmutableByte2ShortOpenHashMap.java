 ...
 */
public class ImmutableByte2ShortOpenHashMap extends AbstractByte2ShortMap implements Byte2ShortOrderedMap
{
	 ...
	 */
	protected ImmutableByte2ShortOpenHashMap() {}
	 ...
	 */
	public ImmutableByte2ShortOpenHashMap(Byte[] keys, Short[] values) {
	 ...
	 */
	public ImmutableByte2ShortOpenHashMap(Byte[] keys, Short[] values, float loadFactor) {
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		init(ByteArrays.unwrap(keys), ShortArrays.unwrap(values), 0, keys.length, loadFactor);
	}
	 ...
	 */
	public ImmutableByte2ShortOpenHashMap(Map<? extends Byte, ? extends Short> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public ImmutableByte2ShortOpenHashMap(Map<? extends Byte, ? extends Short> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public ImmutableByte2ShortOpenHashMap(Byte2ShortMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public ImmutableByte2ShortOpenHashMap(Byte2ShortMap map, float loadFactor) {
		byte[] keys = new byte[map.size()];
		short[] values = new short[keys.length];
		int index = 0;
		for(Byte2ShortMap.Entry entry : Byte2ShortMaps.fastIterable(map)) {
	...
	protected void init(byte[] a, short[] b, int offset, int length, float loadFactor) {
	...
	@Override
	public ImmutableByte2ShortOpenHashMap copy() {
		ImmutableByte2ShortOpenHashMap map = new ImmutableByte2ShortOpenHashMap();
		map.mask = mask;
	...
	@Override
	public boolean replace(byte key, short oldValue, short newValue) { throw new UnsupportedOperationException(); }
	
	@Override
	public short replace(byte key, short value) { throw new UnsupportedOperationException(); }
	...
	private class MapEntrySet extends AbstractObjectSet<Byte2ShortMap.Entry> implements Byte2ShortOrderedMap.FastOrderedSet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2ShortMap.Entry) {
					...
					int index = ImmutableByte2ShortOpenHashMap.this.findIndex(entry.getByteKey());
					if(index >= 0) return entry.getShortValue() == ImmutableByte2ShortOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = ImmutableByte2ShortOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short.valueOf(ImmutableByte2ShortOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		@Override
		public int size() {
			return ImmutableByte2ShortOpenHashMap.this.size();
		}
