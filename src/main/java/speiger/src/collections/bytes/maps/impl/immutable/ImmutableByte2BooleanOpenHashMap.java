...
public class ImmutableByte2BooleanOpenHashMap extends AbstractByte2BooleanMap implements Byte2BooleanOrderedMap
{
	 ...
	 */
	protected ImmutableByte2BooleanOpenHashMap() {}
	 ...
	 */
	public ImmutableByte2BooleanOpenHashMap(Byte[] keys, Boolean[] values) {
	 ...
	 */
	public ImmutableByte2BooleanOpenHashMap(Byte[] keys, Boolean[] values, float loadFactor) {
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
		init(ByteArrays.unwrap(keys), BooleanArrays.unwrap(values), 0, keys.length, loadFactor);
	}
	 ...
	 */
	public ImmutableByte2BooleanOpenHashMap(Map<? extends Byte, ? extends Boolean> map) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public ImmutableByte2BooleanOpenHashMap(Map<? extends Byte, ? extends Boolean> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public ImmutableByte2BooleanOpenHashMap(Byte2BooleanMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public ImmutableByte2BooleanOpenHashMap(Byte2BooleanMap map, float loadFactor) {
		byte[] keys = new byte[map.size()];
		boolean[] values = new boolean[keys.length];
		int index = 0;
		for(Byte2BooleanMap.Entry entry : Byte2BooleanMaps.fastIterable(map)) {
	...
	protected void init(byte[] a, boolean[] b, int offset, int length, float loadFactor) {
	...
	@Override
	public ImmutableByte2BooleanOpenHashMap copy() {
		ImmutableByte2BooleanOpenHashMap map = new ImmutableByte2BooleanOpenHashMap();
		map.mask = mask;
	...
	@Override
	public boolean replace(byte key, boolean oldValue, boolean newValue) { throw new UnsupportedOperationException(); }
	
	@Override
	public boolean replace(byte key, boolean value) { throw new UnsupportedOperationException(); }
	...
	protected class MapEntry implements Byte2BooleanMap.Entry, Map.Entry<Byte, Boolean> {
		...
		@Override
		public int hashCode() {
			return Byte.hashCode(keys[index]) ^ Boolean.hashCode(values[index]);
		}
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2BooleanMap.Entry) {
					...
					int index = ImmutableByte2BooleanOpenHashMap.this.findIndex(entry.getByteKey());
					if(index >= 0) return entry.getBooleanValue() == ImmutableByte2BooleanOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = ImmutableByte2BooleanOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Boolean.valueOf(ImmutableByte2BooleanOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
