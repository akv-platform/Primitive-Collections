...
public class ImmutableByte2DoubleOpenHashMap extends AbstractByte2DoubleMap implements Byte2DoubleOrderedMap
{
	 ...
	 */
	protected ImmutableByte2DoubleOpenHashMap() {}
	...
	public ImmutableByte2DoubleOpenHashMap(Byte[] keys, Double[] values, float loadFactor) {
		...
		init(ByteArrays.unwrap(keys), DoubleArrays.unwrap(values), 0, keys.length, loadFactor);
	}
	...
	public ImmutableByte2DoubleOpenHashMap(Byte2DoubleMap map, float loadFactor) {
		...
		double[] values = new double[keys.length];
		int index = 0;
		for(Byte2DoubleMap.Entry entry : Byte2DoubleMaps.fastIterable(map)) {
	...
	protected void init(byte[] a, double[] b, int offset, int length, float loadFactor) {
	...
	@Override
	public ImmutableByte2DoubleOpenHashMap copy() {
		ImmutableByte2DoubleOpenHashMap map = new ImmutableByte2DoubleOpenHashMap();
		map.mask = mask;
	...
	@Override
	public boolean replace(byte key, double oldValue, double newValue) { throw new UnsupportedOperationException(); }
	
	@Override
	public double replace(byte key, double value) { throw new UnsupportedOperationException(); }
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2DoubleMap.Entry) {
					...
					int index = ImmutableByte2DoubleOpenHashMap.this.findIndex(entry.getByteKey());
					if(index >= 0) return Double.doubleToLongBits(entry.getDoubleValue()) == Double.doubleToLongBits(ImmutableByte2DoubleOpenHashMap.this.values[index]);
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = ImmutableByte2DoubleOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Double.valueOf(ImmutableByte2DoubleOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
