 ...
 * 
 */
public class Byte2ByteConcurrentOpenHashMap extends AbstractByte2ByteMap implements Byte2ByteConcurrentMap
{
	/** Segment Limit */
	...
	protected static class Segment extends StampedLock
	{
		...
		protected byte computeIfAbsent(int hash, byte key, Byte2ByteFunction mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
