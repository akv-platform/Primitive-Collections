...
import speiger.src.collections.bytes.maps.interfaces.Byte2ObjectConcurrentMap;
 ...
 */
public class Byte2ObjectConcurrentOpenHashMap<V> extends AbstractByte2ObjectMap<V> implements Byte2ObjectConcurrentMap<V>
{
	/** Segment Limit */
	 ...
	 */
	protected Byte2ObjectConcurrentOpenHashMap(boolean unused) {}
	...
	@Override
	public Byte2ObjectConcurrentOpenHashMap<V> copy() {
		Byte2ObjectConcurrentOpenHashMap<V> copy = new Byte2ObjectConcurrentOpenHashMap<>(false);
	...
	private class MapEntrySet extends AbstractObjectSet<Byte2ObjectMap.Entry<V>> implements Byte2ObjectMap.FastEntrySet<V> {
		@Override
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2ObjectMap.Entry) {
					Byte2ObjectMap.Entry<V> entry = (Byte2ObjectMap.Entry<V>)o;
					return Byte2ObjectConcurrentOpenHashMap.this.remove(entry.getByteKey(), entry.getValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Byte2ObjectConcurrentOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
		...
		@Override
		public int size() {
			return Byte2ObjectConcurrentOpenHashMap.this.size();
		}
		
		@Override
		public void clear() {
			Byte2ObjectConcurrentOpenHashMap.this.clear();
		}
	}
	...
	private final class KeySet extends AbstractByteSet implements ByteSet {
		...
		public boolean remove(byte o) {
			int oldSize = size();
			Byte2ObjectConcurrentOpenHashMap.this.remove(o);
			return size() != oldSize;
		}
	...
	private class FastEntryIterator extends MapIterator implements ObjectBidirectionalIterator<Byte2ObjectMap.Entry<V>> {
	...
	private class EntryIterator extends MapIterator implements ObjectBidirectionalIterator<Byte2ObjectMap.Entry<V>> {
		MapEntry entry;
	...
	protected static class Segment<V> extends StampedLock
	{
		...
		protected final Byte2ObjectConcurrentOpenHashMap<V> map;
		...
		protected Segment(Byte2ObjectConcurrentOpenHashMap<V> map) {
			this.map = map;
		}
		
		protected Segment(Byte2ObjectConcurrentOpenHashMap<V> map, int minCapacity, float loadFactor, boolean isNullContainer) {
			this.map = map;
		...
		protected Segment<V> copy(Byte2ObjectConcurrentOpenHashMap<V> newMap) {
			long stamp = readLock();
			try
			{
		...
		protected V put(int hash, byte key, V value) {
		...
		protected V putIfAbsent(int hash, byte key, V value) {
			long stamp = writeLock();
			try {
				int slot = findIndex(hash, key);
				if(slot < 0) {
		...
		protected V get(int hash, byte key) {
			long stamp = readLock();
			try {
				int slot = findIndex(hash, key);
		...
		protected V getOrDefault(int hash, byte key, V defaultValue) {
			long stamp = readLock();
			try {
				int slot = findIndex(hash, key);
		...
		protected void forEach(ByteObjectConsumer<V> action) {
			long stamp = readLock();
			try {
				int index = firstIndex;
				while(index != -1) {
		...
		protected V remove(int hash, byte key) {
			long stamp = writeLock();
			try {
				int slot = findIndex(hash, key);
		...
		protected V removeOrDefault(int hash, byte key, V defaultValue) {
			long stamp = writeLock();
			try {
				int slot = findIndex(hash, key);
		...
		protected boolean remove(int hash, byte key, V value) {
			long stamp = writeLock();
			try {
				if(key == (byte)0) {
		...
		protected boolean replace(int hash, byte key, V oldValue, V newValue) {
			long stamp = writeLock();
			try {
		...
		protected V replace(int hash, byte key, V value) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
		...
		protected V compute(int hash, byte key, ByteObjectUnaryOperator<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V computeIfAbsent(int hash, byte key, Byte2ObjectFunction<V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
				if(index < 0) {
		...
		protected V merge(int hash, byte key, V value, ObjectObjectUnaryOperator<V, V> mappingFunction) {
			long stamp = writeLock();
			try {
				int index = findIndex(hash, key);
