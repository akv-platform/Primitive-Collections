...
public class Byte2ObjectOpenHashMap<V> extends AbstractByte2ObjectMap<V> implements ITrimmable
{
	...
	private class Values extends AbstractObjectCollection<V> {
		...
		public boolean matchesAny(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (byte)0 && filter.getBoolean(values[i])) return true;
			}
			return false;
		}
		...
		public boolean matchesNone(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (byte)0 && filter.getBoolean(values[i])) return false;
			}
			return true;
		}
		...
		public V findFirst(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (byte)0 && filter.getBoolean(values[i])) return values[i];
			}
			return null;
		}
		...
		public int count(Object2BooleanFunction<V> filter) {
			...
			for(int i = nullIndex-1;i>=0;i--) {
				if(keys[i] != (byte)0 && filter.getBoolean(values[i])) result++;
			}
			return result;
		}
	}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Byte2ObjectOpenHashMap.this.remove(wrapped.getByte(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
