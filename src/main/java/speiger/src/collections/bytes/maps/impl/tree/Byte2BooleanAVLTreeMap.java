...
public class Byte2BooleanAVLTreeMap extends AbstractByte2BooleanMap implements Byte2BooleanNavigableMap
{
	 ...
	 */
	public Byte2BooleanAVLTreeMap(Byte[] keys, Boolean[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2BooleanAVLTreeMap(Byte[] keys, Boolean[] values, ByteComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Byte2BooleanAVLTreeMap(Map<? extends Byte, ? extends Boolean> map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Byte2BooleanAVLTreeMap(Byte2BooleanMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Byte2BooleanAVLTreeMap(Byte2BooleanMap map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Byte2BooleanAVLTreeMap copy() {
		Byte2BooleanAVLTreeMap set = new Byte2BooleanAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractByteSet implements ByteNavigableSet
	{
		...
		public ByteBidirectionalIterator iterator(byte fromElement) {
			if(map instanceof Byte2BooleanAVLTreeMap) return ((Byte2BooleanAVLTreeMap)map).keyIterator(fromElement);
		...
		public ByteBidirectionalIterator iterator() {
			if(map instanceof Byte2BooleanAVLTreeMap) return ((Byte2BooleanAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ByteBidirectionalIterator descendingIterator() {
			if(map instanceof Byte2BooleanAVLTreeMap) return ((Byte2BooleanAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Byte2BooleanAVLTreeMap) return ((Byte2BooleanAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Byte2BooleanAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Byte2BooleanAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Byte2BooleanAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Byte2BooleanAVLTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Byte2BooleanAVLTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	{
		final Byte2BooleanAVLTreeMap map;
		final byte lo, hi;
		...
		NavigableSubMap(Byte2BooleanAVLTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractBooleanCollection
	{
		...
		@Override
		public int size() {
			return Byte2BooleanAVLTreeMap.this.size;
		}
