...
public class Byte2CharAVLTreeMap extends AbstractByte2CharMap implements Byte2CharNavigableMap
{
	 ...
	 */
	public Byte2CharAVLTreeMap(Byte[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2CharAVLTreeMap(Byte[] keys, Character[] values, ByteComparator comp) {
		comparator = comp;
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2CharAVLTreeMap(byte[] keys, char[] values) {
		this(keys, values, null);
	}
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2CharAVLTreeMap(byte[] keys, char[] values, ByteComparator comp) {
		comparator = comp;
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Byte2CharAVLTreeMap(Map<? extends Byte, ? extends Character> map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Byte2CharAVLTreeMap(Byte2CharMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Byte2CharAVLTreeMap(Byte2CharMap map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Byte2CharAVLTreeMap copy() {
		Byte2CharAVLTreeMap set = new Byte2CharAVLTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractByteSet implements ByteNavigableSet
	{
		...
		public ByteBidirectionalIterator iterator(byte fromElement) {
			if(map instanceof Byte2CharAVLTreeMap) return ((Byte2CharAVLTreeMap)map).keyIterator(fromElement);
		...
		public ByteBidirectionalIterator iterator() {
			if(map instanceof Byte2CharAVLTreeMap) return ((Byte2CharAVLTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ByteBidirectionalIterator descendingIterator() {
			if(map instanceof Byte2CharAVLTreeMap) return ((Byte2CharAVLTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Byte2CharAVLTreeMap) return ((Byte2CharAVLTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Byte2CharAVLTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Byte2CharAVLTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Byte2CharAVLTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Byte2CharAVLTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Byte2CharAVLTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractByte2CharMap implements Byte2CharNavigableMap
	{
		...
		NavigableSubMap(Byte2CharAVLTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractCharCollection
	{
		...
		@Override
		public int size() {
			return Byte2CharAVLTreeMap.this.size;
		}
