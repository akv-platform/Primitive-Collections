 ...
 */
public class Byte2CharOpenHashMap extends AbstractByte2CharMap implements ITrimmable
{
	 ...
	 */
	public Byte2CharOpenHashMap(int minCapacity) {
	 ...
	 */
	public Byte2CharOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Byte2CharOpenHashMap(Byte[] keys, Character[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Byte2CharOpenHashMap(Byte[] keys, Character[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2CharOpenHashMap(byte[] keys, char[] values) {
		this(keys, values, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Byte2CharOpenHashMap(byte[] keys, char[] values, float loadFactor) {
		this(keys.length, loadFactor);
		if(keys.length != values.length) throw new IllegalStateException("Input Arrays are not equal size");
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Byte2CharOpenHashMap(Map<? extends Byte, ? extends Character> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Byte2CharOpenHashMap(Byte2CharMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Byte2CharOpenHashMap(Byte2CharMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (byte)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Character.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Byte2CharOpenHashMap copy() {
		Byte2CharOpenHashMap map = new Byte2CharOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Byte2CharMap.Entry> implements Byte2CharMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2CharMap.Entry) {
					...
					int index = Byte2CharOpenHashMap.this.findIndex(entry.getByteKey());
					if(index >= 0) return entry.getCharValue() == Byte2CharOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Byte2CharOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Character.valueOf(Byte2CharOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2CharMap.Entry) {
					Byte2CharMap.Entry entry = (Byte2CharMap.Entry)o;
					return Byte2CharOpenHashMap.this.remove(entry.getByteKey(), entry.getCharValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Byte2CharOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractByteSet {
		...
		public boolean remove(byte o) {
			int oldSize = size;
			Byte2CharOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Byte2CharOpenHashMap.this.remove(wrapped.getByte(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
