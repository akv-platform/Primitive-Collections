...
public class Byte2LongOpenHashMap extends AbstractByte2LongMap implements ITrimmable
{
	 ...
	 */
	public Byte2LongOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Byte2LongOpenHashMap(Byte[] keys, Long[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Byte2LongOpenHashMap(Byte[] keys, Long[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Byte2LongOpenHashMap(Map<? extends Byte, ? extends Long> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Byte2LongOpenHashMap(Byte2LongMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Byte2LongOpenHashMap(Byte2LongMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (byte)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Long.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Byte2LongOpenHashMap copy() {
		Byte2LongOpenHashMap map = new Byte2LongOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Byte2LongMap.Entry> implements Byte2LongMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2LongMap.Entry) {
					...
					int index = Byte2LongOpenHashMap.this.findIndex(entry.getByteKey());
					if(index >= 0) return entry.getLongValue() == Byte2LongOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Byte2LongOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Long.valueOf(Byte2LongOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2LongMap.Entry) {
					Byte2LongMap.Entry entry = (Byte2LongMap.Entry)o;
					return Byte2LongOpenHashMap.this.remove(entry.getByteKey(), entry.getLongValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Byte2LongOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractByteSet {
		...
		public boolean remove(byte o) {
			int oldSize = size;
			Byte2LongOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Byte2LongOpenHashMap.this.remove(wrapped.getByte(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
