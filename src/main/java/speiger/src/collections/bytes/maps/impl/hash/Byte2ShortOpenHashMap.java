 ...
 */
public class Byte2ShortOpenHashMap extends AbstractByte2ShortMap implements ITrimmable
{
	 ...
	 */
	public Byte2ShortOpenHashMap(int minCapacity) {
	 ...
	 */
	public Byte2ShortOpenHashMap(int minCapacity, float loadFactor) {
	 ...
	 */
	public Byte2ShortOpenHashMap(Byte[] keys, Short[] values) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Byte2ShortOpenHashMap(Byte[] keys, Short[] values, float loadFactor) {
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
	 */
	public Byte2ShortOpenHashMap(Map<? extends Byte, ? extends Short> map, float loadFactor) {
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Byte2ShortOpenHashMap(Byte2ShortMap map) {
		this(map, HashUtil.DEFAULT_LOAD_FACTOR);
	}
	 ...
	 * @throws IllegalStateException if the loadfactor is either below/equal to 0 or above/equal to 1
 	 */
	public Byte2ShortOpenHashMap(Byte2ShortMap map, float loadFactor) {
		this(map.size(), loadFactor);
		putAll(map);
	}
	...
	public boolean containsValue(Object value) {
		...
		for(int i = nullIndex-1;i >= 0;i--)
			if(keys[i] != (byte)0 && ((value == null && values[i] == getDefaultReturnValue()) || Objects.equals(value, Short.valueOf(values[i])))) return true;
		return false;
	}
	...
	@Override
	public Byte2ShortOpenHashMap copy() {
		Byte2ShortOpenHashMap map = new Byte2ShortOpenHashMap(0, loadFactor);
	...
	private final class MapEntrySet extends AbstractObjectSet<Byte2ShortMap.Entry> implements Byte2ShortMap.FastEntrySet {
		...
		public boolean contains(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2ShortMap.Entry) {
					...
					int index = Byte2ShortOpenHashMap.this.findIndex(entry.getByteKey());
					if(index >= 0) return entry.getShortValue() == Byte2ShortOpenHashMap.this.values[index];
				}
				else {
					Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
					int index = Byte2ShortOpenHashMap.this.findIndex(entry.getKey());
					if(index >= 0) return Objects.equals(entry.getValue(), Short.valueOf(Byte2ShortOpenHashMap.this.values[index]));
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if(o instanceof Map.Entry) {
				if(o instanceof Byte2ShortMap.Entry) {
					Byte2ShortMap.Entry entry = (Byte2ShortMap.Entry)o;
					return Byte2ShortOpenHashMap.this.remove(entry.getByteKey(), entry.getShortValue());
				}
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>)o;
				return Byte2ShortOpenHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
	}
	...
	private final class KeySet extends AbstractByteSet {
		...
		public boolean remove(byte o) {
			int oldSize = size;
			Byte2ShortOpenHashMap.this.remove(o);
			return size != oldSize;
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if(lastReturned == nullIndex) {
				...
				Byte2ShortOpenHashMap.this.remove(wrapped.getByte(-returnedPos - 1));
				lastReturned = -1;
				return;
			}
			size--;
