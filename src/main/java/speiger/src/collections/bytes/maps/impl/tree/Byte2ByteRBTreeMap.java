...
public class Byte2ByteRBTreeMap extends AbstractByte2ByteMap implements Byte2ByteNavigableMap
{
	 ...
	 */
	public Byte2ByteRBTreeMap(Byte[] keys, Byte[] values) {
	 ...
	 * @throws IllegalStateException if the keys and values do not match in lenght
	 */
	public Byte2ByteRBTreeMap(Byte[] keys, Byte[] values, ByteComparator comp) {
		comparator = comp;
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
	 */
	public Byte2ByteRBTreeMap(Map<? extends Byte, ? extends Byte> map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	 ...
	 * @param map the values that should be present in the map
 	 */
	public Byte2ByteRBTreeMap(Byte2ByteMap map) {
		this(map, null);
	}
	 ...
	 * @param comp the function that decides how the tree is sorted, can be null
 	 */
	public Byte2ByteRBTreeMap(Byte2ByteMap map, ByteComparator comp) {
		comparator = comp;
		putAll(map);
	}
	...
	@Override
	public Byte2ByteRBTreeMap copy() {
		Byte2ByteRBTreeMap set = new Byte2ByteRBTreeMap();
		set.size = size;
		if(tree != null) {
			set.tree = tree.copy();
			Node lastFound = null;
	...
	static class KeySet extends AbstractByteSet implements ByteNavigableSet
	{
		...
		public ByteBidirectionalIterator iterator(byte fromElement) {
			if(map instanceof Byte2ByteRBTreeMap) return ((Byte2ByteRBTreeMap)map).keyIterator(fromElement);
		...
		public ByteBidirectionalIterator iterator() {
			if(map instanceof Byte2ByteRBTreeMap) return ((Byte2ByteRBTreeMap)map).keyIterator();
			return ((NavigableSubMap)map).keyIterator();
		}
		
		@Override
		public ByteBidirectionalIterator descendingIterator() {
			if(map instanceof Byte2ByteRBTreeMap) return ((Byte2ByteRBTreeMap)map).descendingKeyIterator();
			return ((NavigableSubMap)map).descendingKeyIterator();
		}
		
		protected Node start() {
			if(map instanceof Byte2ByteRBTreeMap) return ((Byte2ByteRBTreeMap)map).first;
			return ((NavigableSubMap)map).subLowest();
		}
		
		protected Node end() {
			if(map instanceof Byte2ByteRBTreeMap) return null;
			return ((NavigableSubMap)map).subHighest();
		}
		
		protected Node next(Node entry) {
			if(map instanceof Byte2ByteRBTreeMap) return entry.next();
			return ((NavigableSubMap)map).next(entry);
		}
		
		protected Node previous(Node entry) {
			if(map instanceof Byte2ByteRBTreeMap) return entry.previous();
	...
	{
		AscendingNaivgableSubMap(Byte2ByteRBTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
	...
	static class DescendingNaivgableSubMap extends NavigableSubMap
	{
		...
		DescendingNaivgableSubMap(Byte2ByteRBTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			super(map, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
	...
	static abstract class NavigableSubMap extends AbstractByte2ByteMap implements Byte2ByteNavigableMap
	{
		...
		NavigableSubMap(Byte2ByteRBTreeMap map, boolean fromStart, byte lo, boolean loInclusive, boolean toEnd, byte hi, boolean hiInclusive) {
			if (!fromStart && !toEnd) {
	...
	class Values extends AbstractByteCollection
	{
		...
		@Override
		public int size() {
			return Byte2ByteRBTreeMap.this.size;
		}
